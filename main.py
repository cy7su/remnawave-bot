...
class GracefulExit:
    ...
    def exit_gracefully(self, signum, frame):
        structlog.get_logger(__name__).info(
            "Получен сигнал, корректное завершение работы", signum=signum
        )
        self.exit = True
...
async def main():
    ...
    for insecure_default_warning in settings.collect_insecure_default_warnings():
        logger.warning(
            "⚠️ Insecure configuration default", detail=insecure_default_warning
        )
    ...
    try:
        ...
        if not skip_migration:
            async with timeline.stage(
                ...
                try:
                    ...
                    allow_failure = (
                        os.getenv("ALLOW_MIGRATION_FAILURE", "false").lower() == "true"
                    )
                    logger.error(
                        "Ошибка выполнения миграции", migration_error=migration_error
                    )
                    if not allow_failure:
                        raise
                    stage.warning(
                        f"Ошибка миграции: {migration_error} (ALLOW_MIGRATION_FAILURE=true)"
                    )
        else:
        ...
        async with timeline.stage(
            ...
            if not seq_ok:
                stage.warning(
                    "Не удалось синхронизировать последовательности PostgreSQL"
                )
            if not token_ok:
        ...
        async with timeline.stage(
            ...
            try:
                ...
                logger.error(
                    "❌ Не удалось инициализировать платёжные методы", error=error
                )
        ...
        dp = None
        async with timeline.stage(
            "Настройка бота", "🤖", success_message="Бот настроен"
        ) as stage:
            ...
            async with timeline.stage(
                ...
                try:
                    ...
                    if status.next_rotation:
                        ...
                        stage.log(
                            f'Следующая ротация: {next_dt.strftime("%d.%m.%Y %H:%M")}'
                        )
                except Exception as e:
        ...
        async with timeline.stage(
            ...
            try:
                ...
                if status.enabled:
                    times_text = (
                        ", ".join(t.strftime("%H:%M") for t in status.times) or "—"
                    )
                    if status.next_run:
        ...
        try:
            from app.services.subscription_dedup_service import (
                dedupe_expired_tariff_subscriptions,
            )
        ...
        async with timeline.stage(
            ...
            for method in SUPPORTED_MANUAL_CHECK_METHODS:
                if method == PaymentMethod.YOOKASSA and settings.is_yookassa_enabled():
                    ...
                    method == PaymentMethod.MULENPAY and settings.is_mulenpay_enabled()
                ):
                    ...
                    and settings.is_cryptobot_enabled()
                ):
            ...
            if verification_providers:
                ...
                stage.log(
                    f"Ожидающие пополнения автоматически отбираются не старше {hours}ч"
                )
                stage.log(
            ...
            if settings.is_payment_verification_auto_check_enabled():
                ...
                if auto_methods:
                    ...
                    stage.log(
                        f"Автопроверка каждые {interval_minutes} мин: {auto_labels}"
                    )
                else:
            ...
            if settings.is_nalogo_enabled():
                try:
                    ...
                    if nalogo_queue_service.is_running():
                        queue_len = (
                            await payment_service.nalogo_service.get_queue_length()
                        )
                        if queue_len > 0:
        ...
        async with timeline.stage(
            ...
            if should_start_web_app:
                ...
                base_url = (
                    ...
                    or f"http://{settings.WEB_API_HOST}:{settings.WEB_API_PORT}"
                )
        ...
        async with timeline.stage(
            ...
            if not settings.is_maintenance_monitoring_enabled():
                ...
                not maintenance_service._check_task
                or maintenance_service._check_task.done()
            ):
                maintenance_task = asyncio.create_task(
                    maintenance_service.start_monitoring()
                )
        ...
        async with timeline.stage(
            ...
            if daily_subscription_service.is_enabled():
                ...
                interval_minutes = (
                    daily_subscription_service.get_check_interval_minutes()
                )
                ...
                stage.log(
                    "Суточные тарифы выключены — запущен только сброс докупок трафика"
                )
        ...
        async with timeline.stage(
            ...
            if settings.is_version_check_enabled():
                version_check_task = asyncio.create_task(
                    version_service.start_periodic_check()
                )
                stage.log(
        ...
        async with timeline.stage(
            ...
            if polling_enabled:
                ...
                stage.log(
                    "skip_updates=False — накопившиеся обновления будут обработаны"
                )
            else:
                polling_task = None
        ...
        if settings.is_cloudpayments_enabled():
            webhook_lines.append(
                f"CloudPayments: {_fmt(settings.CLOUDPAYMENTS_WEBHOOK_PATH)}"
            )
        ...
        try:
            while not killer.exit:
                ...
                if monitoring_task.done():
                    ...
                    if exception:
                        logger.error(
                            "Служба мониторинга завершилась с ошибкой", error=exception
                        )
                ...
                if maintenance_task and maintenance_task.done():
                    ...
                    if exception:
                        logger.error(
                            "Служба техработ завершилась с ошибкой", error=exception
                        )
                        maintenance_task = asyncio.create_task(
                            maintenance_service.start_monitoring()
                        )
                ...
                if traffic_monitoring_task and traffic_monitoring_task.done():
                    ...
                    if exception:
                        logger.error(
                            "Мониторинг трафика завершился с ошибкой", error=exception
                        )
                        if traffic_monitoring_scheduler.is_enabled():
                ...
                if daily_subscription_task and daily_subscription_task.done():
                    ...
                    if exception:
                        if daily_subscription_service.is_enabled():
                            ...
                            logger.error(
                                "Цикл сброса докупок трафика завершился с ошибкой",
                                error=exception,
                            )
                ...
                if (
                    ...
                    await auto_payment_verification_service.start()
                    auto_verification_active = (
                        auto_payment_verification_service.is_running()
                    )
        ...
        try:
            ...
            logger.error(
                "Ошибка остановки сервиса автопроверки пополнений", error=error
            )
