...
def upgrade() -> None:
    ...
    if _has_table("referral_earnings") and not _has_column(
        "referral_earnings", "campaign_id"
    ):
        op.add_column(
            "referral_earnings", sa.Column("campaign_id", sa.Integer(), nullable=True)
        )
        ...
        op.create_index(
            "ix_referral_earnings_campaign_id", "referral_earnings", ["campaign_id"]
        )
    ...
    if not _has_column("users", "partner_status"):
        op.add_column(
            "users",
            sa.Column(
                "partner_status", sa.String(20), nullable=False, server_default="none"
            ),
        )
        op.create_index("ix_users_partner_status", "users", ["partner_status"])

    # --- From 0003: broadcast_history.blocked_count ---
    if _has_table("broadcast_history") and not _has_column(
    ...
    if _has_table("advertising_campaigns") and not _has_column(
        ...
        op.create_index(
            "ix_advertising_campaigns_partner_user_id",
            "advertising_campaigns",
            ["partner_user_id"],
        )
    ...
    if not _has_table("withdrawal_requests"):
        op.create_table(
            ...
            sa.Column(
                "processed_by", sa.Integer(), sa.ForeignKey("users.id"), nullable=True
            ),
    ...
    if not _has_table("email_templates"):
        ...
        op.create_index(
            "ix_email_templates_notification_type",
            "email_templates",
            ["notification_type"],
        )
