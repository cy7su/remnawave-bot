...
def upgrade() -> None:
    conn = op.get_bind()
    if conn.execute(
        sa.text("SELECT to_regclass('public.apple_transactions')")
    ).scalar():
        return
    op.create_table(
        "apple_transactions",
    ...
    op.create_index(
        "ix_apple_transactions_transaction_id", "apple_transactions", ["transaction_id"]
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_apple_transactions_original_transaction_id", table_name="apple_transactions"
    )
    op.drop_index(
        "ix_apple_transactions_transaction_id", table_name="apple_transactions"
    )
