...
def upgrade() -> None:
    ...
    if not _has_index("cabinet_refresh_tokens", "ix_cabinet_refresh_tokens_id"):
        op.create_index(
            "ix_cabinet_refresh_tokens_id", "cabinet_refresh_tokens", ["id"]
        )
    ...
    if not _has_index("cabinet_refresh_tokens", "ix_cabinet_refresh_tokens_user"):
        op.create_index(
            "ix_cabinet_refresh_tokens_user", "cabinet_refresh_tokens", ["user_id"]
        )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_cabinet_refresh_tokens_token_hash", table_name="cabinet_refresh_tokens"
    )
