...
def upgrade() -> None:
    conn = op.get_bind()
    if conn.execute(
        sa.text("SELECT to_regclass('public.inline_gift_subscriptions')")
    ).scalar():
        return
    op.create_table(
    ...
    op.create_index(
        "ix_inline_gifts_gift_code",
        "inline_gift_subscriptions",
        ["gift_code"],
        unique=True,
    )
    op.create_index(
        "ix_inline_gifts_recipient_tg_id",
        "inline_gift_subscriptions",
        ["recipient_telegram_id"],
    )
    op.create_index(
        "ix_inline_gifts_sender_id", "inline_gift_subscriptions", ["sender_user_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_inline_gifts_recipient_tg_id", table_name="inline_gift_subscriptions"
    )
