...
def upgrade() -> None:
    ...
    if "max_activations" not in existing_cols:
        op.add_column(
            ...
            sa.Column(
                "max_activations", sa.Integer(), nullable=False, server_default="1"
            ),
        )
    if "activated_count" not in existing_cols:
        op.add_column(
            "inline_gift_subscriptions",
            sa.Column(
                "activated_count", sa.Integer(), nullable=False, server_default="0"
            ),
        )
    if "add_extra_squad" not in existing_cols:
        op.add_column(
            "inline_gift_subscriptions",
            sa.Column(
                "add_extra_squad", sa.Boolean(), nullable=False, server_default="false"
            ),
        )
    # Sync activated_count for already-activated gifts
    op.execute(
        "UPDATE inline_gift_subscriptions SET activated_count = 1 WHERE is_activated = true"
    )
