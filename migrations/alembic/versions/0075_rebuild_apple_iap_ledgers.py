...
def upgrade() -> None:
    ...
    for col_name, col_type in new_at_cols.items():
        if col_name not in existing_at_cols:
            op.add_column('apple_transactions', sa.Column(col_name, col_type, nullable=True))
    ...
    # --- apple_iap_accounts ---
    if not conn.execute(sa.text("SELECT to_regclass('public.apple_iap_accounts')")).scalar():
        op.create_table(
            'apple_iap_accounts',
    ...
    # --- apple_notifications ---
    if not conn.execute(sa.text("SELECT to_regclass('public.apple_notifications')")).scalar():
        op.create_table(
            'apple_notifications',
            sa.Column('id', sa.Integer(), primary_key=True, autoincrement=True),
    ...
    if not conn.execute(sa.text("SELECT to_regclass('public.apple_iap_abuse_events')")).scalar():
        op.create_table(
            'apple_iap_abuse_events',
            sa.Column('id', sa.Integer(), primary_key=True, autoincrement=True),
