...
def upgrade() -> None:
    ...
    existing_cols = {
        c["name"] for c in inspector.get_columns("inline_gift_subscriptions")
    }
    ...
    op.execute(
        "UPDATE inline_gift_subscriptions SET traffic_limit_gb = NULL WHERE traffic_limit_gb = -2"
    )
    op.execute(
        "UPDATE inline_gift_subscriptions SET device_limit = NULL WHERE device_limit = 0 AND gift_type = 'subscription'"
    )


def downgrade() -> None:
    op.execute(
        "UPDATE inline_gift_subscriptions SET traffic_limit_gb = 0 WHERE traffic_limit_gb IS NULL"
    )
    op.execute(
        "UPDATE inline_gift_subscriptions SET device_limit = 1 WHERE device_limit IS NULL"
    )
