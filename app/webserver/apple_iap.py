...
def create_apple_iap_router(bot: Any = None) -> APIRouter:
    router = APIRouter()
    fulfillment_service = AppleIAPFulfillmentService(
    ...
    async def apple_iap_webhook(request: Request) -> JSONResponse:
        ...
        if content_type and "application/json" not in content_type.lower():
            return JSONResponse(
                {"status": "error", "reason": "unsupported_media_type"}, status_code=415
            )

        raw_body = await request.body()
        if not raw_body:
            return JSONResponse(
                {"status": "error", "reason": "empty_body"}, status_code=400
            )
        if len(raw_body) > 256_000:
            return JSONResponse(
                {"status": "error", "reason": "body_too_large"}, status_code=413
            )
        ...
        if not signed_payload:
            return JSONResponse(
                {"status": "error", "reason": "missing_signed_payload"}, status_code=400
            )

        ok, reason = await notification_service.process_signed_payload(
        ...
        return JSONResponse(
            {
                ...
                "root_certificates_count": len(
                    settings.get_apple_iap_root_cert_paths()
                ),
