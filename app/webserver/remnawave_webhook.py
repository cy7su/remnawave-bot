...
def create_remnawave_webhook_router(
    ...
    async def remnawave_webhook(request: Request) -> JSONResponse:
        ...
        if len(raw_body) > _MAX_BODY_SIZE:
            logger.warning(
                "RemnaWave webhook: payload too large", raw_body_count=len(raw_body)
            )
            return JSONResponse(
        ...
        if webhook_service.is_admin_event(
            event_name
        ) and not webhook_service.needs_db_session(event_name):
            try:
                processed = await webhook_service.process_event(None, event_name, data)
                return JSONResponse({"status": "ok", "processed": processed})
            except Exception:
                logger.exception(
                    "RemnaWave webhook processing error", event_name=event_name
                )
        ...
        try:
            async with AsyncSessionLocal() as db:
                try:
                    processed = await webhook_service.process_event(
                        db, event_name, data
                    )
                    await db.commit()
                    return JSONResponse({"status": "ok", "processed": processed})
