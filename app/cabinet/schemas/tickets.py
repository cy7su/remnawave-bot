...
class TicketMediaItem(BaseModel):
    ...
    token: str | None = Field(
        default=None, description="Signed media download token (response only)"
    )
...
class TicketCreateRequest(BaseModel):
    ...
    media_type: str | None = Field(
        None, description="Media type: photo, video, document"
    )
    media_file_id: str | None = Field(
        None, description="Telegram file_id of uploaded media"
    )
    media_caption: str | None = Field(
        None, max_length=1000, description="Media caption"
    )
    media_items: list[TicketMediaItem] | None = Field(
        None, description="Multi-media attachments"
    )
