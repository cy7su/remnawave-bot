...
async def get_template_override(
    ...
    try:
        ...
        logger.debug(
            "Не удалось получить override шаблона /",
...
async def get_overrides_for_type(
    notification_type: str, db: AsyncSession
) -> list[dict[str, Any]]:
...
async def get_rendered_override(
    ...
    if required_vars and context:
        missing = [
            ...
            if context.get(var) not in (None, "")
            and html.escape(str(context[var])) not in body_html
        ]
        if missing:
            logger.warning(
