...
def get_apple_iap_fulfillment_service(request: Request) -> AppleIAPFulfillmentService:
    ...
    return AppleIAPFulfillmentService(
        apple_iap_fulfillment_service.apple_service, bot=bot
    )
...
async def _increment_redis_counter(
    client: redis.Redis, key: str, ttl_seconds: int
) -> int:
...
async def _check_purchase_rate_limit(
    client: redis.Redis | None, user_id: int, ip_address: str | None
) -> bool:
    if client is None:
        allow_request = _rate_limit_error_allows_request()
        logger.error(
            "Apple IAP Redis client is not initialized", fail_open=allow_request
        )
    ...
    try:
        ...
        if (
            ...
            and _parse_redis_counter(failure_count) >= failure_limit
        ):
        ...
        for key in keys:
            ...
            if count > limit:
                logger.warning(
                    "Apple IAP purchase rate limit exceeded", key=key, user_id=user_id
                )
                return False
    except RedisError as error:
        allow_request = _rate_limit_error_allows_request()
        logger.error(
            "Apple IAP rate limiter unavailable", error=error, fail_open=allow_request
        )
        return allow_request
...
async def _record_purchase_failure(client: redis.Redis | None, user_id: int) -> None:
    if client is None:
        logger.warning(
            "Apple IAP Redis client is not initialized; skipping failure limiter"
        )
        return
    ...
    try:
        ...
        logger.warning(
            "Apple IAP failure limiter returned invalid counter", error=error
        )
...
async def apple_iap_account_token(
    ...
    fulfillment_service: AppleIAPFulfillmentService = Depends(
        get_apple_iap_fulfillment_service
    ),
):
...
async def reconcile_apple_iap_transactions(
    ...
    result = await apple_iap_reconciliation_service.reconcile_recent_transactions(
        db, limit=limit
    )
    return {
