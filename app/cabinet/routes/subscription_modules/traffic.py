...
async def get_traffic_packages(
    ...
    def _package_response(
        gb: int, base_price_kopeks: int, is_unlimited: bool
    ) -> TrafficPackageResponse:
        ...
        charged.
        """
        discount = _apply_addon_discount(
            user, "traffic", base_price_kopeks, period_hint_days
        )
    ...
    for pkg in packages:
        ...
        result.append(
            _package_response(pkg["gb"], pkg["price"], is_unlimited=pkg["gb"] == 0)
        )
...
async def purchase_traffic(
    ...
    if is_tariff_mode:
        ...
        matching_pkg = next(
            (
                pkg
                for pkg in packages
                if pkg["gb"] == request.gb and pkg.get("enabled", True)
            ),
            None,
        )
        if not matching_pkg:
    ...
    discount_result = _apply_addon_discount(
        user, "traffic", prorated_price, period_hint_days
    )
    ...
    if traffic_discount_percent > 0:
        traffic_description = (
            f"Докупка {request.gb} ГБ трафика (скидка {traffic_discount_percent}%)"
        )
    else:
...
async def switch_traffic_package(
    ...
    new_pkg = next(
        (p for p in packages if p["gb"] == new_traffic and p.get("enabled", True)), None
    )
    ...
    if new_price > current_price:
        ...
        price_diff, _discount_val, traffic_discount_percent = (
        ...
        final_price, days_charged = calculate_prorated_price(
            price_diff, subscription.end_date
        )
...
async def refresh_traffic(
    ...
    cache_suffix = (
        f"{user.id}_{subscription_id}" if subscription_id is not None else str(user.id)
    )
    ...
    try:
        ...
        if user.telegram_id and not settings.is_multi_tariff_enabled():
            traffic_stats = await remnawave_service.get_user_traffic_stats(
                user.telegram_id
            )
        elif _traffic_uuid:
            traffic_stats = await remnawave_service.get_user_traffic_stats_by_uuid(
                _traffic_uuid
            )
        else:
            traffic_stats = None
        ...
        if not traffic_stats:
            ...
            traffic_data = {
                "traffic_used_bytes": int(
                    (subscription.traffic_used_gb or 0) * (1024**3)
                ),
                "traffic_used_gb": round(subscription.traffic_used_gb or 0, 2),
                "traffic_limit_bytes": int(
                    (subscription.traffic_limit_gb or 0) * (1024**3)
                ),
                "traffic_limit_gb": subscription.traffic_limit_gb or 0,
                "traffic_used_percent": round(
                    (
                        (
                            (subscription.traffic_used_gb or 0)
                            / (subscription.traffic_limit_gb or 1)
                        )
                        * 100
        ...
        traffic_data = {
            ...
            "lifetime_used_gb": round(
                traffic_stats.get("lifetime_used_traffic_gb", 0), 2
            ),
        }
