...
async def _build_tariff_response(
    ...
    promo_group = (
        ...
        if user and hasattr(user, "get_primary_promo_group")
        else None
    )
    ...
    if subscription and subscription.tariff_id == tariff.id:
        extra_devices_count = max(
            0, (subscription.device_limit or 0) - (tariff.device_limit or 0)
        )
    ...
    if tariff.period_prices:
        for period_str, price_kopeks in sorted(
            ...
            extra_devices_cost = (
                extra_devices_count * extra_device_price_per_month * months
            )
            ...
            original_per_month = (
                original_price // months if months > 0 else original_price
            )
            ...
            if extra_devices_count > 0:
                ...
                period_data["extra_devices_cost_label"] = settings.format_price(
                    extra_devices_cost
                )
                period_data["base_tariff_price_kopeks"] = base_tariff_price
                period_data["base_tariff_price_label"] = settings.format_price(
                    base_tariff_price
                )
            ...
            if discount_percent > 0:
                ...
                period_data["original_price_label"] = settings.format_price(
                    original_price
                )
                period_data["original_per_month_kopeks"] = original_per_month
                period_data["original_per_month_label"] = settings.format_price(
                    original_per_month
                )
    ...
    traffic_label = (
        "Безлимит" if tariff.traffic_limit_gb == 0 else f"{tariff.traffic_limit_gb} ГБ"
    )
    ...
    if daily_price > 0:
        ...
        if daily_group_pct > 0 or daily_offer_pct > 0:
            daily_price, _, _ = PricingEngine.apply_stacked_discounts(
                daily_price, daily_group_pct, daily_offer_pct
            )
    ...
    if promo_group and price_per_day > 0:
        custom_days_discount_percent = promo_group.get_discount_percent(
            "period", 30
        )  # Use 30-day rate as base
        if custom_days_discount_percent > 0:
            price_per_day = pricing_engine.apply_discount(
                price_per_day, custom_days_discount_percent
            )

    # Apply discount to device price if applicable
    device_price = (
        tariff.device_price_kopeks if tariff.device_price_kopeks is not None else 0
    )
...
async def get_purchase_options(
    ...
    try:
        ...
        if settings.is_tariffs_mode():
            ...
            for tariff in tariffs:
                ...
                if (
                    ...
                    and tariff.id in purchased_tariff_ids
                ):
            ...
            return {
                ...
                "all_tariffs_purchased": (
                    len(purchased_tariff_ids) >= len(tariffs)
                    if settings.is_multi_tariff_enabled()
                    else False
                ),
        ...
        context = await purchase_service.build_options(
            db, user, subscription_id=subscription_id
        )
        ...
        logger.error(
            "Failed to build purchase options for user", user_id=user.id, error=e
        )
        raise HTTPException(
...
async def preview_purchase(
    ...
    try:
        ...
        logger.error(
            "Failed to calculate purchase preview for user", user_id=user.id, error=e
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def submit_purchase(
    ...
    try:
        ...
        if not user.telegram_id and user.email and user.email_verified:
            try:
                is_new_subscription = (
                    result.get("was_trial_conversion") or not context.subscription
                )
                notification_type = (
        ...
        try:
            ...
            if getattr(settings, "ADMIN_NOTIFICATIONS_ENABLED", False):
                ...
                try:
                    ...
                    await notification_service.send_subscription_purchase_notification(
                        ...
                        purchase_type=(
                            "renewal" if not is_new_subscription else "first_purchase"
                        ),
                    )
                finally:
            ...
            logger.error(
                "Failed to send admin notification for subscription purchase", error=e
            )
        ...
        try:
            ...
            logger.info(
                "Cart saved for auto-purchase (cabinet /purchase) user", user_id=user.id
            )
...
async def purchase_tariff(
    ...
    try:
        ...
        if request.traffic_gb is not None and tariff.can_purchase_custom_traffic():
            ...
            if (
                request.traffic_gb < tariff.min_traffic_gb
                or request.traffic_gb > tariff.max_traffic_gb
            ):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=(
        ...
        if settings.TRIAL_ADD_REMAINING_DAYS_TO_PAID:
            for _kt in killed_trials:
                if _kt.end_date and _kt.end_date > _now_trial:
                    _bonus_seconds += max(
                        0, (_kt.end_date - _now_trial).total_seconds()
                    )
        ...
        if not is_daily_tariff:
            try:
                ...
                await user_cart_service.save_user_cart(user.id, cart_data)
                logger.info(
                    "Tariff cart saved for auto-renewal (cabinet) user", user_id=user.id
                )
            except Exception as e:
        ...
        if discount_percent > 0:
            ...
            response["discount_amount_kopeks"] = (
                original_price - price_before_promo_offer
            )
            response["discount_label"] = settings.format_price(
                original_price - price_before_promo_offer
            )
            if promo_group:
        ...
        if promo_offer_discount_value > 0:
            ...
            response["promo_offer_discount_label"] = settings.format_price(
        ...
        if not user.telegram_id and user.email and user.email_verified:
            try:
                ...
                was_new_subscription = (
                    ...
                    and (datetime.now(UTC) - subscription.start_date).total_seconds()
                    < 60
                )
                notification_type = (
                    NotificationType.SUBSCRIPTION_ACTIVATED
        ...
        try:
            ...
            if getattr(settings, "ADMIN_NOTIFICATIONS_ENABLED", False):
                ...
                try:
                    ...
                    was_new_subscription = (
                        subscription.start_date
                        and (
                            datetime.now(UTC) - subscription.start_date
                        ).total_seconds()
                        < 60
                    )
                    await notification_service.send_subscription_purchase_notification(
                        ...
                        amount_kopeks=price_kopeks,
                        purchase_type=(
                            "renewal" if not was_new_subscription else "first_purchase"
                        ),
                    )
                finally:
                    await bot.session.close()
            ...
            logger.error(
                "Failed to send admin notification for tariff purchase", error=e
            )
...
async def get_trial_info(
    ...
    has_active = any(
        s.status == "active" and s.end_date and s.end_date > datetime.now(UTC)
        for s in subs
    )
    ...
    try:
        ...
        if getattr(settings, "ADMIN_NOTIFICATIONS_ENABLED", False):
            ...
            try:
                notification_service = AdminNotificationService(bot)
                charged_amount = (
                    settings.TRIAL_ACTIVATION_PRICE if requires_payment else None
                )
