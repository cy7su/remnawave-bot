    ...
    try:
        ...
        return DashboardStats(
            ...
            subscriptions=SubscriptionStats(
                ...
                expired=sub_stats.get("total_subscriptions", 0)
                - sub_stats.get("active_subscriptions", 0),
            ...
            financial=FinancialStats(
                ...
                income_month_kopeks=trans_stats.get("totals", {}).get(
                    "income_kopeks", 0
                ),
                income_month_rubles=trans_stats.get("totals", {}).get(
                    "income_kopeks", 0
                )
                / 100,
                income_total_kopeks=all_time_stats.get("totals", {}).get(
                    "income_kopeks", 0
                ),
                income_total_rubles=all_time_stats.get("totals", {}).get(
                    "income_kopeks", 0
                )
                / 100,
                subscription_income_kopeks=abs(
                    all_time_stats.get("totals", {}).get(
                        "subscription_income_kopeks", 0
                    )
                ),
            ...
            servers=ServerStats(
                ...
                servers_with_connections=server_stats.get(
...
async def toggle_node(
    ...
    try:
        ...
        if success:
            logger.info(
                "Admin d node", admin_id=admin.id, action=action, node_uuid=node_uuid
            )
            return {
                "success": True,
...
async def _get_nodes_overview() -> NodesOverview:
    ...
    try:
        ...
        total = len(nodes)
        online = sum(
            1 for n in nodes if n.get("is_connected") and not n.get("is_disabled")
        )
...
async def get_top_referrers(
    ...
    try:
        ...
        referrers_data = {
            row.referrer_id: {"total_invited": row.total_invited}
            for row in referrers_query
        }
        ...
        today_invited_query = await db.execute(
            ...
            .where(
                and_(User.referred_by_id.isnot(None), User.created_at >= today_start)
            )
            .group_by(User.referred_by_id)
        )
        ...
        by_earnings = sorted(
            referrer_items, key=lambda x: x.earnings_total_kopeks, reverse=True
        )[:limit]
        by_invited = sorted(
            referrer_items, key=lambda x: x.invited_count, reverse=True
        )[:limit]
...
async def get_top_campaigns(
    ...
    try:
        ...
        campaigns = await get_campaigns_list(
            db, offset=0, limit=100, include_inactive=True
        )
        ...
        for campaign in campaigns:
            ...
            campaign_items.append(
                TopCampaignItem(
                    ...
                    avg_revenue_per_user_kopeks=stats.get(
