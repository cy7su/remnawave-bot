...
async def upload_media(
    ...
    if declared_type in _BLOCKED_UPLOAD_CONTENT_TYPES or filename_lower.endswith(
        _BLOCKED_UPLOAD_EXTENSIONS
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        try:
            await bot.delete_message(
                chat_id=target_chat_id, message_id=message.message_id
            )
        except Exception:
        ...
        logger.error(
            "Failed to upload media for user", telegram_id=user.telegram_id, error=error
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
