...
async def _finalize_oauth_login(
    ...
    *,
    ...
    await _store_refresh_token(
        db, user.id, auth_response.refresh_token, device_info=f"oauth:{provider}"
    )
    ...
    auth_response.campaign_bonus = await _process_campaign_bonus(
        db, user, campaign_slug
    )
...
async def oauth_callback(
    ...
    if state_data.get("linking") == "true":
        logger.warning(
            "Linking-flow state token used in login callback", provider=provider
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if user:
        ...
        return await _finalize_oauth_login(
            db, user, provider, request.campaign_slug, request.referral_code
        )
    ...
    if user_info.email and user_info.email_verified:
        ...
        if user and user.email_verified:
            ...
            was_deleted = user.status == UserStatus.DELETED.value
            if was_deleted:
                await revive_deleted_user(
                    db, user, source=f"oauth_{provider}_email_merge"
                )
    ...
    if request.referral_code:
        try:
            ...
            logger.warning(
                "Failed to resolve referral code during OAuth",
                referral_code=request.referral_code,
                exc_info=True,
            )
    ...
    if user_info.email and user_info.email_verified:
        try:
            ...
            logger.warning(
                "Failed to sync panel subscription for new OAuth user",
                user_id=user.id,
                exc_info=True,
            )
