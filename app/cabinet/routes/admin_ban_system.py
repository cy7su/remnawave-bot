...
def _get_ban_api() -> BanSystemAPI:
    ...
    logger.debug(
        "Ban System URL", get_ban_system_api_url=settings.get_ban_system_api_url()
    )
...
async def get_users(
    ...
    status: str | None = Query(
        None, description="Filter: over_limit, with_limit, unlimited"
    ),
    admin: User = Depends(require_permission("ban_system:read")),
) -> BanUsersListResponse:
    ...
    data = await _api_request(
        api, "get_users", offset=offset, limit=limit, status=status
    )
...
async def unban_user(
    ...
    try:
        ...
        logger.info(
            "Admin unbanned user in Ban System", admin_id=admin.id, user_id=user_id
        )
...
async def get_agents(
    ...
    agent_status: str | None = Query(
        None, alias="status", description="online, offline"
    ),
    admin: User = Depends(require_permission("ban_system:read")),
) -> BanAgentsListResponse:
...
def _parse_setting_response(
    key: str, data: Any, default_type: str = "str"
) -> BanSettingDefinition:
...
async def set_setting(
    ...
    logger.info(
        "Admin changed Ban System setting to", admin_id=admin.id, key=key, value=value
    )
...
async def whitelist_add(
    ...
    try:
        ...
        return UnbanResponse(
            success=True, message=f"User {request.username} added to whitelist"
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def whitelist_remove(
    ...
    try:
        ...
        return UnbanResponse(
            success=True, message=f"User {request.username} removed from whitelist"
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def get_agent_history(
    ...
    api = _get_ban_api()
    data = await _api_request(
        api, "get_agent_history", node_name=node_name, hours=hours
    )
