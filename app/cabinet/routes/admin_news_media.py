...
def _build_response(request: Request, saved: SavedMedia) -> NewsMediaUploadResponse:
    ...
    thumbnail_url = (
        _build_media_url(request, saved.thumbnail_path)
...
async def upload_media(
    ...
    max_size_mb = (
        settings.MEDIA_MAX_IMAGE_SIZE_MB
        if media_type == "image"
        else settings.MEDIA_MAX_VIDEO_SIZE_MB
    )
    ...
    try:
        ...
        logger.warning(
            "Failed to save uploaded media", media_type=media_type, error=str(exc)
        )
        raise HTTPException(
