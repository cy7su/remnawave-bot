...
def _placeholder_context(notification_type: str) -> dict[str, Any]:
    ...
    return {
        var: f"{{{var}}}"
        for var in type_meta["context_vars"]
        if var not in _NUMERIC_FALLBACK_VARS
    }


def _get_default_template(
    notification_type: str, language: str, context: dict[str, Any]
) -> dict[str, str] | None:
...
async def get_templates_for_type(
    ...
    for lang in AVAILABLE_LANGUAGES:
        default_template = _get_default_template(
            notification_type, lang, editor_context
        )
...
async def preview_template(
    ...
    if data.body_html:
        ...
        subject = (
            substitute_context_vars(data.subject, sample_context, escape=False)
            or notification_type
        )
    else:
        # Preview default template
        default_template = _get_default_template(
            notification_type, language, sample_context
        )
        if default_template:
...
async def send_test_email(
    ...
    if data.body_html:
        ...
        rendered = await get_rendered_override(
            notification_type, language, sample_context, db
        )
