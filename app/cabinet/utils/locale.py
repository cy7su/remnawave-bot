...
def resolve_locale_text(
    data: dict[str, str] | str | None, lang: str = DEFAULT_LOCALE
) -> str:
    ...
    *,
    ...
    for locale, text in value.items():
        if locale not in SUPPORTED_LOCALES:
            raise ValueError(
                f'Unsupported locale "{locale}" in {field_name}. Allowed: {", ".join(SUPPORTED_LOCALES)}'
            )
        if not isinstance(text, str):
            raise ValueError(f"{field_name}[{locale}] must be a string")
        if max_length is not None and len(text) > max_length:
            raise ValueError(
                f"{field_name}[{locale}] exceeds max length {max_length} (got {len(text)})"
            )
    return value
