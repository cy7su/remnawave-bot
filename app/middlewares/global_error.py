...
class GlobalErrorMiddleware(BaseMiddleware):
    ...
    async def _handle_telegram_error(
        ...
        if self._is_topic_required_error(error_message):
            ...
            logger.debug(
                "[GlobalErrorMiddleware] Игнорируем ошибку топика", error=error
            )
            return None
        ...
        user_info = self._get_user_info(event)
        logger.error(
            "Неизвестная Telegram API ошибка", user_info=user_info, error=error
        )
        raise error
    ...
    async def _handle_old_query(self, event: TelegramObject, error: TelegramBadRequest):
        if isinstance(event, CallbackQuery):
            ...
            logger.warning(
                "[GlobalErrorMiddleware] Игнорируем устаревший запрос", error=error
            )
        ...
        if isinstance(event, CallbackQuery):
            try:
                ...
                if not self._is_old_query_error(str(answer_error).lower()):
                    logger.warning(
                        "Ошибка при ответе на callback", answer_error=answer_error
                    )
    ...
    async def _handle_bad_request(
        ...
        if USER_DEACTIVATED_PHRASE in error_message:
            ...
            return
        if (
            CHAT_NOT_FOUND_PHRASE in error_message
            or MESSAGE_NOT_FOUND_PHRASE in error_message
        ):
            logger.warning(
                "[GlobalErrorMiddleware] Чат или сообщение не найдено", error=error
            )
            return
        user_info = self._get_user_info(event)
        logger.error(
...
async def send_error_to_admin_chat(
    ...
    ) or getattr(settings, "ADMIN_NOTIFICATIONS_TOPIC_ID", None)
    ...
    if (
        ...
        logger.debug(
            "Ошибка добавлена в буфер, троттлинг активен", error_type=error_type
        )
        return False
