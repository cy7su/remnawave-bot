...
async def get_conversion_statistics(db: AsyncSession) -> dict:
    ...
    total_users_with_subscriptions_result = await db.execute(
    ...
    if total_users_with_subscriptions > 0:
        conversion_rate = round(
            (users_with_paid / total_users_with_subscriptions) * 100, 1
        )
    else:
        conversion_rate = 0.0
    ...
    month_conversions_result = await db.execute(
        select(func.count(SubscriptionConversion.id)).where(
            SubscriptionConversion.converted_at >= month_ago
        )
    )
    ...
    logger.info(
        "Всего пользователей с подписками",
        total_users_with_subscriptions=total_users_with_subscriptions,
    )
