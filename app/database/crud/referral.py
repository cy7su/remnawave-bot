...
async def get_commission_payment_count(
...
async def get_referral_earnings_by_referral(
    db: AsyncSession, referral_id: int
) -> list[ReferralEarning]:
    result = await db.execute(
        select(ReferralEarning)
...
async def get_referral_statistics(db: AsyncSession) -> dict:
    ...
    referrals_stats = {
        row.referrer_id: row.referrals_count for row in referrals_stats_result.all()
    }
    ...
    referral_earnings = {
        row.referrer_id: row.referral_earnings for row in referral_earnings_result.all()
    }
    ...
    today_earnings_result = await db.execute(
        select(func.coalesce(func.sum(ReferralEarning.amount_kopeks), 0)).where(
            ReferralEarning.created_at >= today
        )
    )
...
async def get_top_referrers_by_period(
    ...
    if sort_by == "invited":
        ...
        referral_earnings = {
            row.referrer_id: row.ref_earnings for row in referral_earnings_result
        }

        # Сортируем и берём топ
        sorted_referrers = sorted(
            referral_earnings.items(), key=lambda x: x[1], reverse=True
        )[:limit]
