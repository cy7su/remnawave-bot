...
async def start_simple_subscription_purchase(
    ...
    if settings.is_multi_tariff_enabled():
        await callback.answer(
            "Используйте выбор тарифа для управления подписками", show_alert=True
        )
        return
    ...
    if current_subscription:
        if (
            not getattr(current_subscription, "is_trial", False)
            ...
            try:
                days_left = max(
                    0, (current_subscription.end_date - datetime.now(UTC)).days
                )
            except Exception:
                days_left = 0
            key = (
    ...
    if has_active_paid_subscription:
        await state.set_state(
            SubscriptionStates.waiting_for_simple_subscription_confirmation
        )
    else:
        await state.set_state(
...
def _get_simple_subscription_payment_keyboard(
    language: str,
    ...
    if settings.TELEGRAM_STARS_ENABLED:
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text="Telegram Stars", callback_data="simple_subscription_stars"
                )
            ]
        )
    ...
    if settings.is_yookassa_enabled():
        ...
        yookassa_methods.append(
            types.InlineKeyboardButton(
                text="YooKassa (Карта)", callback_data="simple_subscription_yookassa"
            )
        )
        if yookassa_methods:
    ...
    if settings.is_cryptobot_enabled():
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text="CryptoBot", callback_data="simple_subscription_cryptobot"
                )
            ]
        )
    ...
    if settings.is_heleket_enabled():
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text="Heleket", callback_data="simple_subscription_heleket"
                )
            ]
        )
    ...
    if settings.is_pal24_enabled():
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text="PayPalych", callback_data="simple_subscription_pal24"
                )
            ]
        )
    ...
    if settings.is_wata_enabled():
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text="WATA", callback_data="simple_subscription_wata"
                )
            ]
        )
...
async def _ensure_simple_subscription_squad_uuid(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            "SIMPLE_SUBSCRIPTION_RANDOM_SQUAD_ERROR", user_id=user_id, error=error
        )
        return None
    ...
    if resolved_uuid:
        ...
        logger.info(
            "SIMPLE_SUBSCRIPTION_RANDOM_SQUAD_ASSIGNED",
            user_id=user_id,
...
async def handle_simple_subscription_pay_with_balance(
    ...
    if not subscription_params:
        await callback.answer(
            "Данные подписки устарели. Пожалуйста, начните сначала.", show_alert=True
        )
        return
    ...
    if (
        ...
        await callback.answer(
            "У вас уже есть активная платная подписка. Пожалуйста, подтвердите покупку.",
            show_alert=True,
        )
        return
    ...
    if total_required > 0 and user_balance_kopeks < total_required:
        await callback.answer(
            "Недостаточно средств на балансе для оплаты подписки", show_alert=True
        )
        return
    ...
    try:
        ...
        success_traffic_label = (
            "Безлимит" if success_traffic_gb == 0 else f"{success_traffic_gb} ГБ"
        )
...
async def handle_simple_subscription_other_payment_methods(
    ...
    payment_traffic_label = (
        "Безлимит" if payment_traffic_gb == 0 else f"{payment_traffic_gb} ГБ"
    )
...
async def handle_simple_subscription_payment_method(
    ...
    if (
        ...
        await callback.answer(
            "У вас уже есть активная платная подписка. Пожалуйста, подтвердите покупку через главное меню.",
            show_alert=True,
        )
        return
    ...
    try:
        ...
        if payment_method == "stars":
            ...
            stars_count = settings.rubles_to_stars(
                settings.kopeks_to_rubles(price_kopeks)
            )

            stars_traffic_gb = subscription_params["traffic_limit_gb"]
            stars_traffic_label = (
                "Безлимит" if stars_traffic_gb == 0 else f"{stars_traffic_gb} ГБ"
            )
            ...
            if not settings.is_yookassa_enabled():
                await callback.answer(
                    "Оплата через YooKassa временно недоступна", show_alert=True
                )
                return

            if payment_method == "yookassa_sbp" and not settings.YOOKASSA_SBP_ENABLED:
                await callback.answer(
                    "Оплата через СБП временно недоступна", show_alert=True
                )
                return
            ...
            order = await purchase_service.create_subscription_order(
                ...
                squad_uuid=resolved_squad_uuid,
                payment_method=(
                    "yookassa_sbp" if payment_method == "yookassa_sbp" else "yookassa"
                ),
                total_price_kopeks=price_kopeks,
            )
            ...
            if payment_method == "yookassa_sbp":
                payment_result = await payment_service.create_yookassa_sbp_payment(
                    ...
                    receipt_email=(
                        db_user.email
                        if hasattr(db_user, "email") and db_user.email
                        else None
                    ),
                    receipt_phone=(
                        db_user.phone
            ...
            if not confirmation_url and not qr_confirmation_data:
                await callback.answer(
                    "Ошибка получения данных для оплаты", show_alert=True
                )
                return
            ...
            if qr_confirmation_data or confirmation_url:
                try:
                    ...
                    qr_photo = BufferedInputFile(
                        img_bytes.getvalue(), filename="qrcode.png"
                    )
                except ImportError:
                    logger.warning(
                        "qrcode библиотека не установлена, QR-код не будет сгенерирован"
                    )
                except Exception as e:
            ...
            yookassa_traffic_label = (
                "Безлимит" if yookassa_traffic_gb == 0 else f"{yookassa_traffic_gb} ГБ"
            )
            ...
            if qr_photo:
                ...
                await callback.message.edit_media(
                    media=types.InputMediaPhoto(
                        media=qr_photo, caption=message_text, parse_mode="HTML"
                    ),
            ...
            if not settings.is_cryptobot_enabled():
                await callback.answer(
                    "Оплата через CryptoBot временно недоступна", show_alert=True
                )
                return
            ...
            if not settings.is_heleket_enabled():
                await callback.answer(
                    "Оплата через Heleket временно недоступна", show_alert=True
                )
                return
            ...
            if (
                price_kopeks < settings.MULENPAY_MIN_AMOUNT_KOPEKS
                or price_kopeks > settings.MULENPAY_MAX_AMOUNT_KOPEKS
            ):
                await callback.answer(
            ...
            payment_id_display = (
                mulen_result.get("mulen_payment_id") or local_payment_id
            )
            ...
            if not settings.is_pal24_enabled():
                await callback.answer(
                    "Оплата через PayPalych временно недоступна", show_alert=True
                )
                return
            ...
            fallback_url = pal24_result.get("link_page_url") or pal24_result.get(
                "link_url"
            )
            ...
            if (
                card_url
                and card_url != sbp_url
                and settings.is_pal24_card_button_visible()
            ):
                pay_buttons.append(
                    [
                        types.InlineKeyboardButton(
            ...
            if (
                not pay_buttons
                and fallback_url
                and settings.is_pal24_sbp_button_visible()
            ):
                pay_buttons.append(
                    [
                        types.InlineKeyboardButton(
            ...
            if not settings.is_wata_enabled():
                await callback.answer(
                    "Оплата через WATA временно недоступна", show_alert=True
                )
                return
            if (
                price_kopeks < settings.WATA_MIN_AMOUNT_KOPEKS
                or price_kopeks > settings.WATA_MAX_AMOUNT_KOPEKS
            ):
                await callback.answer(
...
async def check_simple_pal24_payment_status(
    ...
    try:
        ...
        payment_service = PaymentService(callback.bot)
        status_info = await payment_service.get_pal24_payment_status(
            db, local_payment_id
        )
        ...
        if (
            not card_link
            and payment.link_page_url
            and payment.link_page_url != sbp_link
        ):
        ...
        try:
            ...
            if "message is not modified" in str(error).lower():
                await callback.answer(
                    texts.t("CHECK_STATUS_NO_CHANGES", "Статус не изменился")
                )
            else:
                raise
        ...
        logger.error(
            "Ошибка проверки статуса PayPalych для простой подписки", error=error
        )
...
async def check_simple_mulenpay_payment_status(
    ...
    try:
        ...
        logger.debug(
            "Не удалось получить пользователя для MulenPay статуса", error=error
        )
...
async def check_simple_cryptobot_payment_status(
    ...
    try:
        ...
        logger.debug(
            "Не удалось получить пользователя для CryptoBot статуса", error=error
        )
...
async def check_simple_heleket_payment_status(
    ...
    try:
        ...
        logger.debug(
            "Не удалось получить пользователя для Heleket статуса", error=error
        )
...
async def check_simple_wata_payment_status(
    ...
    emoji, status_text = status_labels.get(
        payment.status, ("", texts.t("WATA_STATUS_UNKNOWN", "Неизвестно"))
    )
...
def register_simple_subscription_handlers(dp):
    ...
    dp.callback_query.register(
        start_simple_subscription_purchase, F.data == "simple_subscription_purchase"
    )
    ...
    dp.callback_query.register(
        check_simple_pal24_payment_status, F.data.startswith("check_simple_pal24_")
    )
    ...
    dp.callback_query.register(
        check_simple_heleket_payment_status, F.data.startswith("check_simple_heleket_")
    )

    dp.callback_query.register(
        check_simple_wata_payment_status, F.data.startswith("check_simple_wata_")
    )
