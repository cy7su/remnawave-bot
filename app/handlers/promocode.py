...
async def activate_promocode_for_registration(
    ...
    *,
    ...
    result = await promocode_service.activate_promocode(
        db, user_id, code, subscription_id=subscription_id
    )

    if result["success"]:
        logger.info(
            "Пользователь активировал промокод при регистрации",
            user_id=user_id,
            code=code,
        )
        ...
        if bot:
            try:
                ...
                logger.error(
                    "Ошибка отправки админ уведомления об активации промокода",
                    code=code,
...
async def process_promocode(
    ...
    if not validate_promo_format(code):
        await message.answer(
            texts.PROMOCODE_INVALID, reply_markup=get_back_keyboard(db_user.language)
        )
        return
    ...
    result = await activate_promocode_for_registration(
        db, db_user.id, code, message.bot
    )
    ...
    if result["success"]:
        ...
        error_messages = {
            ...
            "not_yet_valid": texts.t(
                "PROMOCODE_NOT_YET_VALID", "Промокод ещё не начал действовать"
            ),
            "used": texts.PROMOCODE_USED,
            "already_used_by_user": texts.PROMOCODE_USED,
            "user_not_found": texts.t(
                "PROMOCODE_USER_NOT_FOUND", "Пользователь не найден"
            ),
            ...
            "subscription_not_found": texts.t(
                "PROMOCODE_SUBSCRIPTION_NOT_FOUND", "Подписка не найдена."
            ),
            "daily_limit": texts.t(
                "PROMO_DAILY_LIMIT",
                "Достигнут лимит активаций промокодов на сегодня. Попробуйте завтра.",
        ...
        await message.answer(
            error_text, reply_markup=get_back_keyboard(db_user.language)
        )
        await _restore_previous_state(state)
...
async def handle_promo_subscription_select(
    ...
    texts = get_texts(db_user.language)
    result = await activate_promocode_for_registration(
        db, db_user.id, code, callback.bot, subscription_id=sub_id
    )
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        handle_promo_subscription_select, F.data.startswith("promo_sub:")
    )
