...
async def show_main_menu(
    ...
    *,
    ...
    has_active_subscription = any(
        sub.is_active or getattr(sub, "actual_status", None) == "limited"
        for sub in _subs
    )
...
async def show_info_menu(
    ...
    faq_enabled = is_visible_in_bot(
        settings.FAQ_DISPLAY_MODE
    ) and await FaqService.is_enabled(db, db_user.language)
    ...
    custom_pages = [
        (page.id, _resolve_info_page_title(page, db_user.language))
        for page in bot_pages
        if page.replaces_tab is None
    ]
...
async def show_promo_groups_info(
    ...
    if next_group:
        remaining_kopeks = (
            next_group.auto_assign_total_spent_kopeks or 0
        ) - total_spent_kopeks
        lines.append(
            texts.t(
    ...
    lines.extend(
        ["", texts.t("PROMO_GROUPS_INFO_LEVELS_HEADER", "Уровни с автовыдачей:")]
    )
...
async def get_main_menu_text(user, texts, db: AsyncSession):
    ...
    if settings.is_multi_tariff_enabled():
        subscriptions_status, tariff_info_block = await _get_multi_tariff_status(
            user, texts, db
        )
...
async def handle_activate_button(
    ...
    if not connected_squads:
        available_servers = await get_available_server_squads(
            db, promo_group_id=db_user.promo_group_id
        )
        connected_squads = [
            s.squad_uuid
    ...
    available_periods = sorted(
        settings.get_available_subscription_periods(), reverse=True
    )
    ...
    try:
        for period in available_periods:
            if subscription:
                pricing_result = await pricing_engine.calculate_renewal_price(
                    db, subscription, period, user=db_user
                )
                price = pricing_result.final_total
            else:
                new_pricing = (
                    await pricing_engine.calculate_classic_new_subscription_price(
                        db,
                        period,
                        connected_squads,
                        traffic_limit_gb,
        ...
        if not best_period:
            ...
            if subscription:
                min_pricing = await pricing_engine.calculate_renewal_price(
                    db, subscription, min_period, user=db_user
                )
