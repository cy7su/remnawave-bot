...
async def _prepare_subscription_summary(
    ...
    if "period_days" not in summary_data:
        raise KeyError(
            "period_days missing from subscription data — FSM state likely expired"
        )
    ...
    traffic_discounted_per_month = PricingEngine.apply_discount(
        traffic_price_per_month, traffic_discount_percent
    )
    servers_discounted_per_month = PricingEngine.apply_discount(
        servers_price_per_month, servers_discount_percent
    )
    devices_discounted_per_month = PricingEngine.apply_discount(
        devices_price_per_month, devices_discount_percent
    )
    discounted_monthly_additions = (
        traffic_discounted_per_month
        + servers_discounted_per_month
        + devices_discounted_per_month
    )
...
async def get_subscription_cost(subscription, db: AsyncSession) -> int:
    try:
        ...
        result = await pricing_engine.calculate_renewal_price(
            db, subscription, 30, user=owner
        )
...
async def get_subscription_info_text(subscription, texts, db_user, db: AsyncSession):
    ...
    subscription_url = (
        getattr(subscription, "subscription_url", None) or "Генерируется..."
    )
