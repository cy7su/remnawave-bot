...
async def _resolve_subscription(
    callback: types.CallbackQuery, db_user: User, db: AsyncSession, state=None
):
...
async def handle_connect_subscription(
    ...
    back_cb = (
        f"sm:{sub_id}" if settings.is_multi_tariff_enabled() else "menu_subscription"
    )
...
<b>Выберите ваше устройство</b> для получения подробной инструкции по настройке:""",
        ...
        await callback.message.edit_text(
            ...
            reply_markup=get_device_selection_keyboard(
                db_user.language, platforms=platforms, sub_id=sub_id
            ),
            parse_mode="HTML",
        )
