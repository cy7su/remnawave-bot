    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="← Назад", callback_data="admin_campaigns"
                    )
                ]
            ]
        ),
...
async def show_campaign_detail(
    ...
    text.append(
        f'• Получили триал: <b>{stats["trial_users_count"]}</b> (активно: {stats["active_trials_count"]})'
    )
    text.append(
        "• Конверсий в оплату: "
    ...
    text.append(
        f'• Средний доход на пользователя: <b>{texts.format_price(stats["avg_revenue_per_user_kopeks"])}</b>'
    )
    text.append(
        f'• Средний первый платеж: <b>{texts.format_price(stats["avg_first_payment_kopeks"])}</b>'
    )
...
async def toggle_campaign_status(
    ...
    logger.info(
        "Кампания переключена", campaign_id=campaign_id, status_text=status_text
    )
...
async def select_campaign_tariff(
    ...
    await state.update_data(
        campaign_tariff_id=tariff_id, campaign_tariff_name=tariff.name
    )
...
async def start_edit_campaign_tariff(
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text="← Назад", callback_data=f"admin_campaign_edit_{campaign_id}"
            )
        ]
    )
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        start_edit_campaign_tariff_days,
        F.data.startswith("admin_campaign_edit_tariff_days_"),
    )
    dp.callback_query.register(
