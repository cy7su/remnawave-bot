...
async def show_blacklist_settings(
    ...
    """
    logger.info(
        "Вызван обработчик show_blacklist_settings для пользователя",
...
"""
    ...
    keyboard = [
        ...
        [
            types.InlineKeyboardButton(
                text="URL к GitHub" if not github_url else "Изменить URL",
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Обновить статус", callback_data="admin_blacklist_settings"
                    )
                ],
                [
                    types.InlineKeyboardButton(
                        text="← Назад", callback_data="admin_blacklist_settings"
                    )
                ],
            ]
        ),
        ...
        await callback.message.edit_text(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="Повторить", callback_data="admin_blacklist_update"
                        )
                    ],
                    [
                        types.InlineKeyboardButton(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Обновить", callback_data="admin_blacklist_view"
                    )
                ],
                [
                    types.InlineKeyboardButton(
    ...
    await message.answer(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Обновить список", callback_data="admin_blacklist_update"
                    )
                ],
                [
                    types.InlineKeyboardButton(
...
def register_blacklist_handlers(dp):
    ...
    dp.callback_query.register(
        show_blacklist_settings, lambda c: c.data == "admin_blacklist_settings"
    )

    # Обработчики для взаимодействия с черным списком
    dp.callback_query.register(
        toggle_blacklist, lambda c: c.data == "admin_blacklist_toggle"
    )

    dp.callback_query.register(
        update_blacklist, lambda c: c.data == "admin_blacklist_update"
    )

    dp.callback_query.register(
        show_blacklist_users, lambda c: c.data == "admin_blacklist_view"
    )

    dp.callback_query.register(
        start_set_blacklist_url, lambda c: c.data == "admin_blacklist_set_url"
    )

    # Обработчик сообщений для установки URL (работает только в нужном состоянии)
    dp.message.register(
        process_blacklist_url, StateFilter(BlacklistStates.waiting_for_blacklist_url)
    )
