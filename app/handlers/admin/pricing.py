...
TRAFFIC_PACKAGE_FIELD_MAP: dict[int, str] = {
    gb: field for gb, field in TRAFFIC_PACKAGE_FIELDS
}
...
SETTING_ENTRY_BY_KEY: dict[str, SettingEntry] = {
    entry.key: entry
    for entries in SETTING_ENTRIES_BY_SECTION.values()
...
async def _save_traffic_packages(
    ...
    *,
    ...
    current_value = (
        bot_configuration_service.get_current_value("TRAFFIC_PACKAGES_CONFIG") or ""
    )
...
def _build_period_summary(
    items: Iterable[PriceItem], lang_code: str, fallback: str
) -> str:
    parts: list[str] = []
...
def _build_period_options_summary(lang_code: str) -> str:
    suffix = "д" if lang_code == "ru" else "d"
    available = ", ".join(
        f"{days}{suffix}" for days in settings.get_available_subscription_periods()
    )
    renewal = ", ".join(
...
def _build_settings_section(
    ...
    if entries:
        ...
        lines.append(
            texts.t("ADMIN_PRICING_SECTION_EMPTY", "Нет параметров для изменения.")
        )
...
def _build_period_options_section(
    ...
    sub_list = (
        ", ".join(f"{days}{suffix}" for days in sorted(available_subscription)) or "—"
    )
    renew_list = (
        ", ".join(f"{days}{suffix}" for days in sorted(available_renewal)) or "—"
    )
...
async def start_setting_edit(
    ...
    if mode == "price":
        ...
        if warning:
            prompt_parts.append(
                f'{texts.t("ADMIN_PRICING_SETTING_WARNING", "Важно")}: {warning}'
            )
        prompt_parts.extend(
            [
                "",
...
async def process_pricing_input(
    ...
    if not key:
        await message.answer(
            texts.t("ADMIN_PRICING_EDIT_EXPIRED", "Сессия редактирования истекла.")
        )
        await state.clear()
        return
    ...
    if raw_value.strip().lower() in {"cancel", "отмена"}:
        ...
        await message.answer(
            texts.t("ADMIN_PRICING_EDIT_CANCELLED", "Изменения отменены.")
        )
        return
...
async def toggle_traffic_package(
    ...
    target_index = next(
        (index for index, pkg in enumerate(packages) if pkg["gb"] == gb_value), None
    )
