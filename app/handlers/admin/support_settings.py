...
def _get_support_settings_keyboard(language: str) -> types.InlineKeyboardMarkup:
    ...
    rows.append(
        [
            types.InlineKeyboardButton(
                text=mode_button(
                    "ADMIN_SUPPORT_SETTINGS_MODE_TICKETS", "Тикеты", mode == "tickets"
                ),
                callback_data="admin_support_mode_tickets",
            ),
            types.InlineKeyboardButton(
                text=mode_button(
                    "ADMIN_SUPPORT_SETTINGS_MODE_CONTACT", "Контакт", mode == "contact"
                ),
                callback_data="admin_support_mode_contact",
            ),
            types.InlineKeyboardButton(
                text=mode_button(
                    "ADMIN_SUPPORT_SETTINGS_MODE_BOTH", "Оба", mode == "both"
                ),
    ...
    rows.append(
        [
            types.InlineKeyboardButton(
                text=texts.t(
                    "ADMIN_SUPPORT_SETTINGS_MODERATORS_COUNT", "‍Модераторы: {count}"
                ).format(count=mod_count),
    ...
    rows.append(
        [
            types.InlineKeyboardButton(
                text=texts.BACK, callback_data="admin_submenu_support"
            )
        ]
    )
...
@admin_required
@error_handler
async def start_set_sla_minutes(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.BACK, callback_data="admin_support_settings"
                    )
                ]
            ]
        ),
...
async def list_moderators(
    ...
    if not moderators:
        await callback.answer(
            texts.t("ADMIN_SUPPORT_MODERATORS_EMPTY", "Список пуст"), show_alert=True
        )
        return
    text = (
    ...
    if len(current_desc_plain) <= 4000:
        ...
        while chunk < len(current_desc_plain):
            ...
            await callback.message.answer(
                next_chunk, reply_markup=(markup if is_last else None)
            )
            chunk += 4000
    await callback.answer(
        texts.t("ADMIN_SUPPORT_DESCRIPTION_SENT", "Текст отправлен ниже")
    )
...
async def delete_sent_message(
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            await callback.answer(
                texts.t("ADMIN_SUPPORT_MESSAGE_DELETED", "Сообщение удалено")
            )


def register_handlers(dp: Dispatcher):
    dp.callback_query.register(
        show_support_settings, F.data == "admin_support_settings"
    )
    dp.callback_query.register(
        toggle_support_menu, F.data == "admin_support_toggle_menu"
    )
    ...
    dp.callback_query.register(
        delete_sent_message, F.data == "admin_support_delete_msg"
    )
    dp.callback_query.register(
        toggle_admin_notifications, F.data == "admin_support_toggle_admin_notifications"
    )
    dp.callback_query.register(
        toggle_user_notifications, F.data == "admin_support_toggle_user_notifications"
    )
    dp.callback_query.register(toggle_sla, F.data == "admin_support_toggle_sla")
    dp.callback_query.register(
        start_set_sla_minutes, F.data == "admin_support_set_sla_minutes"
    )
    dp.callback_query.register(
        start_add_moderator, F.data == "admin_support_add_moderator"
    )
    dp.callback_query.register(
        start_remove_moderator, F.data == "admin_support_remove_moderator"
    )
    dp.callback_query.register(
        list_moderators, F.data == "admin_support_list_moderators"
    )
    dp.message.register(handle_new_desc, SupportSettingsStates.waiting_for_desc)
    dp.message.register(
        handle_sla_minutes, SupportAdvancedStates.waiting_for_sla_minutes
    )
    dp.message.register(
        handle_moderator_id, SupportAdvancedStates.waiting_for_moderator_id
    )
