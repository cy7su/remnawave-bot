...
def _split_text_into_pages(
    header: str, message_blocks: list[str], max_len: int = MAX_MESSAGE_LEN
) -> list[str]:
...
async def show_admin_tickets(
    ...
    if not (
        settings.is_admin(callback.from_user.id)
        or SupportSettingsService.is_moderator(callback.from_user.id)
    ):
    ...
    statuses = (
        ...
        if scope == "open"
        else [TicketStatus.CLOSED.value]
    )
    page_size = 10
    ...
    tickets = await TicketCRUD.get_tickets_by_statuses(
        db, statuses=statuses, limit=page_size, offset=offset
    )
...
async def view_admin_ticket(
    ...
    if data_str.startswith("admin_ticket_page_"):
        ...
        try:
            ...
            texts = get_texts(db_user.language)
            await callback.answer(
                texts.t("TICKET_NOT_FOUND", "Тикет не найден."), show_alert=True
            )
            return
    ...
    ticket = await TicketCRUD.get_ticket_by_id(
        db, ticket_id, load_messages=True, load_user=True
    )
    ...
    telegram_id_display = (
        html.escape(
            str(ticket.user.telegram_id or ticket.user.email or f"#{ticket.user.id}")
        )
        if ticket.user
        else "—"
    )
    ...
    if ticket.messages:
        ...
        for msg in ticket.messages:
            ...
            if (
                ...
                and getattr(msg, "media_type", None) == "photo"
            ):
                block += "Вложение: фото\n\n"
    ...
    try:
        if ticket.user and ticket.user.telegram_id and ticket.user.username:
            ...
            buttons_row.append(
                types.InlineKeyboardButton(text="Профиль", url=profile_url)
            )
            if buttons_row:
    ...
    if total_pages > 1:
        ...
        if page > 1:
            nav_row.append(
                types.InlineKeyboardButton(
                    text="←", callback_data=f"admin_ticket_page_{ticket_id}_{page - 1}"
                )
            )
        nav_row.append(
            types.InlineKeyboardButton(
    ...
    try:
        await callback.message.edit_text(
            page_text, reply_markup=keyboard, parse_mode="HTML"
        )
...
async def handle_admin_ticket_reply(
    ...
    if not (
        ...
        or SupportSettingsService.is_moderator(message.from_user.id)
    ):
        texts = get_texts(db_user.language)
    ...
    try:
        ...
        if not data.get("reply_mode"):
            ...
            ok = await TicketCRUD.set_user_reply_block(
                db, ticket_id, permanent=False, until=until
            )
            if ok:
        ...
        ticket = await TicketCRUD.get_ticket_by_id(
            db, ticket_id, load_messages=False, load_user=True
        )
        if not ticket:
            texts = get_texts(db_user.language)
...
async def close_all_open_admin_tickets(
    ...
    if closed_count == 0:
        await callback.answer(
            texts.t(
                "ADMIN_CLOSE_ALL_OPEN_TICKETS_EMPTY",
                "Нет открытых тикетов для закрытия.",
            ),
            show_alert=True,
        )
        return
    ...
    try:
        is_moderator = not settings.is_admin(
            callback.from_user.id
        ) and SupportSettingsService.is_moderator(callback.from_user.id)
        ...
        logger.warning(
            "Failed to add support audit for bulk close", audit_error=audit_error
        )
    ...
    notification_keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="Удалить", callback_data="admin_support_delete_msg"
                )
            ]
        ]
    )
    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                await callback.answer(
                    texts.t("TICKET_CLOSED", "Тикет закрыт."), show_alert=True
                )

            # Обновляем inline-клавиатуру в текущем сообщении без кнопок действий
            await callback.message.edit_reply_markup(
                reply_markup=get_admin_ticket_view_keyboard(
                    ticket_id, True, db_user.language
                )
            )
        else:
            texts = get_texts(db_user.language)
...
async def block_user_in_ticket(
    ...
    try:
        await state.update_data(
            origin_chat_id=message.chat.id, origin_message_id=message.message_id
        )
    except Exception as e:
        logger.warning(
            "Failed to save origin message ids for ticket block",
            ticket_id=ticket_id,
            error=e,
        )
    block_prompt = texts.t(
        "ENTER_BLOCK_MINUTES",
        "Введите количество минут для блокировки пользователя (например, 15):",
    )
...
async def handle_admin_block_duration_input(
    ...
    try:
        ...
        try:
            ...
            ) and SupportSettingsService.is_moderator(message.from_user.id)
            await TicketCRUD.add_support_audit(
                db,
                actor_user_id=db_user.id if db_user else None,
        ...
        try:
            updated = await TicketCRUD.get_ticket_by_id(
                db, ticket_id, load_messages=True, load_user=True
            )
            texts = get_texts(db_user.language)
            status_text = {
                TicketStatus.OPEN.value: texts.t("TICKET_STATUS_OPEN", "Открыт"),
                TicketStatus.ANSWERED.value: texts.t(
                    "TICKET_STATUS_ANSWERED", "Отвечен"
                ),
                TicketStatus.CLOSED.value: texts.t("TICKET_STATUS_CLOSED", "Закрыт"),
                TicketStatus.PENDING.value: texts.t(
                    "TICKET_STATUS_PENDING", "В ожидании"
                ),
            }.get(updated.status, updated.status)
            user_name = (
                html.escape(updated.user.full_name) if updated.user else "Unknown"
            )
            ...
            ticket_text += (
                f'Обновлен: {updated.updated_at.strftime("%d.%m.%Y %H:%M")}\n'
            )
            if updated.user and updated.user.telegram_id:
                ...
                if updated.user.username:
                    ...
                    ticket_text += f'ЛС: <a href="tg://resolve?domain={safe_username}">tg://resolve?domain={safe_username}</a>\n'
                else:
                ...
                # Email-only user
                user_id_display = html.escape(
                    str(updated.user.email or f"#{updated.user.id}")
                )
            ...
            kb = get_admin_ticket_view_keyboard(
                ...
                updated.is_closed,
                db_user.language,
                is_user_blocked=updated.is_user_reply_blocked,
            )
            ...
            try:
                if updated.user and updated.user.telegram_id and updated.user.username:
                    ...
                    pm_url = f"tg://resolve?domain={safe_username}"
                    buttons_row.append(
                        types.InlineKeyboardButton(text="Написать в ЛС", url=pm_url)
                    )
            ...
            has_photos = any(
                ...
                and getattr(m, "media_type", None) == "photo"
            ...
            if origin_chat_id and origin_message_id:
                try:
                    ...
                    try:
                        ...
                        await message.answer(
                            f"Пользователь заблокирован на {minutes} минут"
                        )
            else:
                await message.answer(f"Пользователь заблокирован на {minutes} минут")
        except Exception:
        ...
        texts = get_texts(db_user.language)
        await message.answer(
            texts.t("TICKET_REPLY_ERROR", "Произошла ошибка. Попробуйте позже.")
        )
...
async def unblock_user_in_ticket(
    ...
    ok = await TicketCRUD.set_user_reply_block(
        db, ticket_id, permanent=False, until=None
    )
    if ok:
        try:
            await callback.message.answer(
...
async def block_user_permanently(
    ...
    ok = await TicketCRUD.set_user_reply_block(
        db, ticket_id, permanent=True, until=None
    )
    if ok:
        try:
            await callback.message.answer(
...
async def notify_user_about_ticket_reply(
    bot: Bot, ticket: Ticket, reply_text: str, db: AsyncSession
):
    ...
    try:
        ...
        if not getattr(ticket_with_user, "user", None):
            ticket_with_user = await TicketCRUD.get_ticket_by_id(
                db, ticket.id, load_user=True
            )
        ...
        logger.info(
            "Ticket reply notification sent to user",
            ticket_id=ticket.id,
            chat_id=chat_id,
        )
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_admin_tickets, F.data == "admin_tickets_scope_closed"
    )
    dp.callback_query.register(
        close_all_open_admin_tickets, F.data == "admin_tickets_close_all_open"
    )

    dp.callback_query.register(
        view_admin_ticket, F.data.startswith("admin_view_ticket_")
    )
    dp.callback_query.register(
        view_admin_ticket, F.data.startswith("admin_ticket_page_")
    )

    # Ответы на тикеты
    dp.callback_query.register(
        reply_to_admin_ticket, F.data.startswith("admin_reply_ticket_")
    )

    dp.message.register(handle_admin_ticket_reply, AdminTicketStates.waiting_for_reply)
    dp.message.register(
        handle_admin_block_duration_input, AdminTicketStates.waiting_for_block_duration
    )

    # Управление статусами: явная кнопка больше не используется (статус меняется автоматически)

    dp.callback_query.register(
        close_admin_ticket, F.data.startswith("admin_close_ticket_")
    )
    dp.callback_query.register(
        block_user_in_ticket, F.data.startswith("admin_block_user_ticket_")
    )
    dp.callback_query.register(
        unblock_user_in_ticket, F.data.startswith("admin_unblock_user_ticket_")
    )
    dp.callback_query.register(
        block_user_permanently, F.data.startswith("admin_block_user_perm_ticket_")
    )

    # Отмена операций
    dp.callback_query.register(
        cancel_admin_ticket_reply, F.data == "cancel_admin_ticket_reply"
    )

    # Пагинация админских тикетов
    dp.callback_query.register(
        show_admin_tickets, F.data.startswith("admin_tickets_page_")
    )
    ...
    async def send_admin_ticket_attachments(
        ...
        if not photos:
            await callback.answer(
                texts.t("NO_ATTACHMENTS", "Вложений нет."), show_alert=True
            )
            return
        ...
        if last_group_message:
            try:
                ...
                await callback.answer(
                    texts.t("ATTACHMENTS_SENT", "Вложения отправлены.")
                )
        else:
            await callback.answer(texts.t("ATTACHMENTS_SENT", "Вложения отправлены."))

    dp.callback_query.register(
        send_admin_ticket_attachments, F.data.startswith("admin_ticket_attachments_")
    )
    ...
    dp.callback_query.register(
        admin_delete_message, F.data.startswith("admin_delete_message_")
    )
