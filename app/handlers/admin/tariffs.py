...
def get_tariffs_list_keyboard(
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    text=button_text, callback_data=f"admin_tariff_view:{tariff.id}"
                )
            ]
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="", callback_data=f"admin_tariffs_page:{page - 1}"
            )
        )
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text="Создать тариф", callback_data="admin_tariff_create"
            )
        ]
    )
...
def get_tariff_view_keyboard(
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text="Название", callback_data=f"admin_tariff_edit_name:{tariff.id}"
            ),
            InlineKeyboardButton(
                text="Описание", callback_data=f"admin_tariff_edit_desc:{tariff.id}"
            ),
        ]
    )
    buttons.append(
        [
            InlineKeyboardButton(
                text="Трафик", callback_data=f"admin_tariff_edit_traffic:{tariff.id}"
            ),
            InlineKeyboardButton(
                text="Устройства",
    ...
    if not is_daily:
        buttons.append(
            [
                InlineKeyboardButton(
                    text="Цены", callback_data=f"admin_tariff_edit_prices:{tariff.id}"
                ),
                InlineKeyboardButton(
                    text="Уровень", callback_data=f"admin_tariff_edit_tier:{tariff.id}"
                ),
            ]
        )
    else:
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text="Серверы", callback_data=f"admin_tariff_edit_squads:{tariff.id}"
            ),
            InlineKeyboardButton(
                text="Промогруппы", callback_data=f"admin_tariff_edit_promo:{tariff.id}"
            ),
        ]
    )
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text="Удалить", callback_data=f"admin_tariff_delete:{tariff.id}"
            )
        ]
    )
...
def format_tariff_info(tariff: Tariff, language: str, subs_count: int = 0) -> str:
    ...
    if is_daily:
        price_block = (
            f"<b>Суточная цена:</b> {format_price_kopeks(daily_price_kopeks)}/день"
        )
...
async def show_tariffs_list(
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    ...
                    [
                        InlineKeyboardButton(
                            text=texts.BACK, callback_data="admin_panel"
                        )
                    ],
                ]
            ),
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_tariffs_list_keyboard(
            page_data, db_user.language, 0, total_pages
        ),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def show_tariffs_page(
    ...
    await callback.message.edit_text(
        ...
        "Выберите тариф для просмотра и редактирования:",
        reply_markup=get_tariffs_list_keyboard(
            page_data, db_user.language, page, total_pages
        ),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def toggle_trial_tariff(
    ...
    if tariff.is_trial_available:
        ...
        await callback.answer(
            f"Тариф «{tariff.name}» установлен как триальный", show_alert=True
        )
...
async def toggle_daily_tariff(
    ...
    if is_daily:
        ...
        tariff = await update_tariff(
            db, tariff, is_daily=True, daily_price_kopeks=5000
        )  # 50 руб по умолчанию
        await callback.answer(
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Периодный (месяцы)", callback_data="tariff_type_periodic"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="Суточный (оплата за день)",
                        callback_data="tariff_type_daily",
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=texts.CANCEL, callback_data="admin_tariffs"
                    )
                ],
            ]
        ),
...
async def start_edit_tariff_traffic_topup(
    ...
    if tariff.is_unlimited_traffic:
        await callback.answer(
            "Докупка недоступна для безлимитного тарифа", show_alert=True
        )
        return
    ...
    if is_enabled:
        ...
        if packages:
            packages_display = "\n".join(
                f"  • {gb} ГБ: {format_price_kopeks(price)}"
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text=texts.BACK, callback_data=f"admin_tariff_view:{tariff_id}"
            )
        ]
    )
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.CANCEL,
                        callback_data=f"admin_tariff_edit_traffic_topup:{tariff_id}",
                    )
                ]
            ]
        ),
...
async def process_edit_traffic_topup_packages(
    ...
    max_topup_traffic = getattr(tariff, "max_topup_traffic_gb", 0) or 0
    max_limit_display = (
        f"{max_topup_traffic} ГБ" if max_topup_traffic > 0 else "Без ограничений"
    )
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    ...
                    InlineKeyboardButton(
                        text="Отмена", callback_data=f"admin_tariff_view:{tariff_id}"
                    ),
                ]
            ]
        ),
...
async def toggle_tariff_squad(
    ...
    propagate_result = await SubscriptionService().propagate_tariff_squads(
        db, tariff.id, list(current_squads)
    )
...
async def toggle_tariff_promo_group(
    ...
    from app.database.crud.tariff import (
        add_promo_group_to_tariff,
        remove_promo_group_from_tariff,
    )
...
TRAFFIC_RESET_MODES = [
    ...
    (
        "MONTH_ROLLING",
        "Скользящий месяц",
        "Трафик сбрасывается через 30 дней от первого подключения",
    ),
    ("NO_RESET", "Никогда", "Трафик не сбрасывается автоматически"),
]


def get_traffic_reset_mode_keyboard(
    tariff_id: int, current_mode: str | None, language: str
) -> InlineKeyboardMarkup:
...
async def start_edit_traffic_reset_mode(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_traffic_reset_mode_keyboard(
            tariff_id, current_mode, db_user.language
        ),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def set_traffic_reset_mode(
    ...
    await callback.message.edit_text(
        ...
        "• <b>Никогда</b> — трафик накапливается за весь период подписки",
        reply_markup=get_traffic_reset_mode_keyboard(
            tariff_id, new_mode, db_user.language
        ),
        parse_mode="HTML",
    )
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_tariffs_page, F.data.startswith("admin_tariffs_page:")
    )
    ...
    dp.callback_query.register(
        toggle_trial_tariff, F.data.startswith("admin_tariff_toggle_trial:")
    )
    ...
    dp.callback_query.register(
        select_tariff_type_periodic, F.data == "tariff_type_periodic"
    )
    ...
    dp.callback_query.register(
        start_edit_tariff_name, F.data.startswith("admin_tariff_edit_name:")
    )
    dp.message.register(process_edit_tariff_name, AdminStates.editing_tariff_name)

    # Редактирование описания
    dp.callback_query.register(
        start_edit_tariff_description, F.data.startswith("admin_tariff_edit_desc:")
    )
    dp.message.register(
        process_edit_tariff_description, AdminStates.editing_tariff_description
    )

    # Редактирование трафика (traffic_topup BEFORE traffic to avoid prefix conflict)
    dp.callback_query.register(
        start_edit_tariff_traffic_topup,
        F.data.startswith("admin_tariff_edit_traffic_topup:"),
    )
    dp.callback_query.register(
        start_edit_tariff_traffic, F.data.startswith("admin_tariff_edit_traffic:")
    )
    dp.message.register(process_edit_tariff_traffic, AdminStates.editing_tariff_traffic)

    # Редактирование устройств
    dp.callback_query.register(
        start_edit_tariff_devices, F.data.startswith("admin_tariff_edit_devices:")
    )
    dp.message.register(process_edit_tariff_devices, AdminStates.editing_tariff_devices)

    # Редактирование уровня
    dp.callback_query.register(
        start_edit_tariff_tier, F.data.startswith("admin_tariff_edit_tier:")
    )
    dp.message.register(process_edit_tariff_tier, AdminStates.editing_tariff_tier)

    # Редактирование цен
    dp.callback_query.register(
        start_edit_tariff_prices, F.data.startswith("admin_tariff_edit_prices:")
    )
    dp.message.register(process_edit_tariff_prices, AdminStates.editing_tariff_prices)

    # Редактирование цены за устройство
    dp.callback_query.register(
        start_edit_tariff_device_price,
        F.data.startswith("admin_tariff_edit_device_price:"),
    )
    dp.message.register(
        process_edit_tariff_device_price, AdminStates.editing_tariff_device_price
    )

    # Редактирование макс. устройств
    dp.callback_query.register(
        start_edit_tariff_max_devices,
        F.data.startswith("admin_tariff_edit_max_devices:"),
    )
    dp.message.register(
        process_edit_tariff_max_devices, AdminStates.editing_tariff_max_devices
    )

    # Редактирование дней триала
    dp.callback_query.register(
        start_edit_tariff_trial_days, F.data.startswith("admin_tariff_edit_trial_days:")
    )
    dp.message.register(
        process_edit_tariff_trial_days, AdminStates.editing_tariff_trial_days
    )
    ...
    dp.callback_query.register(
        start_edit_traffic_topup_packages,
        F.data.startswith("admin_tariff_edit_topup_packages:"),
    )
    dp.message.register(
        process_edit_traffic_topup_packages,
        AdminStates.editing_tariff_traffic_topup_packages,
    )

    # Редактирование макс. лимита докупки трафика
    dp.callback_query.register(
        start_edit_max_topup_traffic, F.data.startswith("admin_tariff_edit_max_topup:")
    )
    dp.message.register(
        process_edit_max_topup_traffic, AdminStates.editing_tariff_max_topup_traffic
    )

    # Удаление (delete_confirm BEFORE delete to avoid prefix conflict)
    dp.callback_query.register(
        delete_tariff_confirmed, F.data.startswith("admin_tariff_delete_confirm:")
    )
    dp.callback_query.register(
        confirm_delete_tariff, F.data.startswith("admin_tariff_delete:")
    )

    # Редактирование серверов
    dp.callback_query.register(
        start_edit_tariff_squads, F.data.startswith("admin_tariff_edit_squads:")
    )
    dp.callback_query.register(toggle_tariff_squad, F.data.startswith("trf_sq:"))
    dp.callback_query.register(
        clear_tariff_squads, F.data.startswith("admin_tariff_clear_squads:")
    )
    dp.callback_query.register(
        select_all_tariff_squads, F.data.startswith("admin_tariff_select_all_squads:")
    )

    # Редактирование промогрупп
    dp.callback_query.register(
        start_edit_tariff_promo_groups, F.data.startswith("admin_tariff_edit_promo:")
    )
    dp.callback_query.register(
        toggle_tariff_promo_group, F.data.startswith("admin_tariff_toggle_promo:")
    )
    dp.callback_query.register(
        clear_tariff_promo_groups, F.data.startswith("admin_tariff_clear_promo:")
    )

    # Суточный режим
    dp.callback_query.register(
        toggle_daily_tariff, F.data.startswith("admin_tariff_toggle_daily:")
    )
    dp.callback_query.register(
        start_edit_daily_price, F.data.startswith("admin_tariff_edit_daily_price:")
    )
    dp.message.register(
        process_daily_price_input, AdminStates.editing_tariff_daily_price
    )

    # Режим сброса трафика
    dp.callback_query.register(
        start_edit_traffic_reset_mode,
        F.data.startswith("admin_tariff_edit_reset_mode:"),
    )
    dp.callback_query.register(
        set_traffic_reset_mode, F.data.startswith("admin_tariff_set_reset_mode:")
    )
