    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text="Название", callback_data=f"admin_server_edit_name_{server.id}"
            ),
            types.InlineKeyboardButton(
        ...
        [
            types.InlineKeyboardButton(
                text="Удалить", callback_data=f"admin_server_delete_{server.id}"
            ),
            types.InlineKeyboardButton(
                text="Назад", callback_data="admin_servers_list"
            ),
        ],
    ]
...
def _build_server_promo_groups_keyboard(server_id: int, promo_groups, selected_ids):
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text="Назад", callback_data=f"admin_server_edit_{server_id}"
            )
        ]
    )
...
"""
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text="Список серверов", callback_data="admin_servers_list"
            ),
            types.InlineKeyboardButton(
                text="Синхронизация", callback_data="admin_servers_sync"
            ),
        ],
        [
            types.InlineKeyboardButton(
                text="Синхронизировать счетчики",
                callback_data="admin_servers_sync_counts",
            ),
            types.InlineKeyboardButton(
                text="Подробная статистика", callback_data="admin_servers_stats"
            ),
        ],
...
async def show_servers_list(
    callback: types.CallbackQuery, db_user: User, db: AsyncSession, page: int = 1
):
    ...
    if total_pages > 1:
        ...
        if page > 1:
            nav_row.append(
                types.InlineKeyboardButton(
                    text="←", callback_data=f"admin_servers_list_page_{page - 1}"
                )
            )
            ...
            await callback.message.edit_text(
                ...
                reply_markup=types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text="Назад", callback_data="admin_servers"
                            )
                        ]
                    ]
                ),
...
"""
        ...
        keyboard = [
            [
                ...
                types.InlineKeyboardButton(
                    text="Повторить", callback_data="admin_servers_sync"
                ),
            ],
            [types.InlineKeyboardButton(text="Назад", callback_data="admin_servers")],
        ]
...
async def show_server_users(
    ...
    if users:
        ...
        for index, user in enumerate(page_users, start=start_index + 1):
            ...
            if user.telegram_id:
                user_link = (
                    f'<a href="tg://user?id={user.telegram_id}">{safe_user_name}</a>'
                )
            else:
    ...
    for user in page_users:
        ...
        if (
            ...
            and hasattr(user, "subscriptions")
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text="К списку", callback_data="admin_servers_list"
            )
        ]
    )
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Отмена", callback_data=f"admin_server_edit_{server_id}"
                    )
                ]
            ]
        ),
...
async def process_server_price_edit(
    ...
    try:
        ...
        await message.answer(
            "Неверный формат цены. Используйте числа (например: 15.50)"
        )
        ...
        await callback.message.edit_text(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="К списку серверов", callback_data="admin_servers_list"
                        )
                    ]
                ]
            ),
...
"""
    ...
    sorted_servers = sorted(
        available_servers, key=lambda x: x.price_kopeks, reverse=True
    )
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text="Обновить", callback_data="admin_servers_stats"
            ),
            types.InlineKeyboardButton(
                text="Список", callback_data="admin_servers_list"
            ),
        ],
        [types.InlineKeyboardButton(text="Назад", callback_data="admin_servers")],
    ]
...
async def start_server_edit_promo_groups(
    ...
    await callback.message.edit_text(
        text,
        reply_markup=_build_server_promo_groups_keyboard(
            server_id, promo_groups, selected_ids
        ),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def toggle_server_promo_group(
    ...
    if group_id in selected:
        if len(selected) == 1:
            await callback.answer(
                "Нельзя отключить последнюю промогруппу", show_alert=True
            )
            return
    ...
    await callback.message.edit_reply_markup(
        reply_markup=_build_server_promo_groups_keyboard(
            server_id, promo_groups, selected
        )
    )
...
async def sync_server_user_counts_handler(
    ...
    await callback.message.edit_text(
        "Синхронизация счетчиков пользователей...", reply_markup=None
    )
...
"""
        ...
        keyboard = [
            [
                ...
                types.InlineKeyboardButton(
                    text="Повторить", callback_data="admin_servers_sync_counts"
                ),
            ],
            [types.InlineKeyboardButton(text="Назад", callback_data="admin_servers")],
        ]
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        sync_servers_with_remnawave, F.data == "admin_servers_sync"
    )
    dp.callback_query.register(
        sync_server_user_counts_handler, F.data == "admin_servers_sync_counts"
    )
    dp.callback_query.register(
        show_server_detailed_stats, F.data == "admin_servers_stats"
    )
    ...
    dp.callback_query.register(
        toggle_server_availability, F.data.startswith("admin_server_toggle_")
    )
    dp.callback_query.register(
        toggle_server_trial_assignment, F.data.startswith("admin_server_trial_")
    )
    dp.callback_query.register(
        show_server_users, F.data.startswith("admin_server_users_")
    )

    dp.callback_query.register(
        start_server_edit_name, F.data.startswith("admin_server_edit_name_")
    )
    dp.callback_query.register(
        start_server_edit_price, F.data.startswith("admin_server_edit_price_")
    )
    dp.callback_query.register(
        start_server_edit_country, F.data.startswith("admin_server_edit_country_")
    )
    dp.callback_query.register(
        start_server_edit_promo_groups, F.data.startswith("admin_server_edit_promo_")
    )
    dp.callback_query.register(
        start_server_edit_limit, F.data.startswith("admin_server_edit_limit_")
    )
    dp.callback_query.register(
        start_server_edit_description, F.data.startswith("admin_server_edit_desc_")
    )
    ...
    dp.message.register(
        process_server_description_edit, AdminStates.editing_server_description
    )
    dp.callback_query.register(
        toggle_server_promo_group, F.data.startswith("admin_server_promo_toggle_")
    )
    dp.callback_query.register(
        save_server_promo_groups, F.data.startswith("admin_server_promo_save_")
    )

    dp.callback_query.register(
        delete_server_confirm,
        F.data.startswith("admin_server_delete_") & ~F.data.contains("confirm"),
    )
    dp.callback_query.register(
        delete_server_execute, F.data.startswith("admin_server_delete_confirm_")
    )

    dp.callback_query.register(
        handle_servers_pagination, F.data.startswith("admin_servers_list_page_")
    )
