...
def _format_period_discounts_value(discounts: dict[int, int]) -> str:
    ...
    return ", ".join(
        f"{period}:{percent}" for period, percent in sorted(discounts.items())
    )
...
async def show_promo_groups_menu(
    ...
    if groups:
        ...
        for group, member_count in groups:
            ...
            lines.append(
                f"{icon} <b>{html.escape(group.name)}</b>{default_suffix} — {members_label}"
            )
    ...
    keyboard_rows.append(
        [
            types.InlineKeyboardButton(
                text="Создать", callback_data="admin_promo_group_create"
            )
        ]
    )
    keyboard_rows.append(
        [
            types.InlineKeyboardButton(
                text=texts.BACK, callback_data="admin_submenu_promo"
            )
        ]
    )
...
async def show_promo_group_details(
    ...
    keyboard_rows.append(
        [
            types.InlineKeyboardButton(
                text=texts.BACK, callback_data="admin_promo_groups"
            )
        ]
    )
...
async def process_create_group_name(message: types.Message, state: FSMContext):
    ...
    if not name:
        ...
        await message.answer(
            texts.t("ADMIN_PROMO_GROUP_INVALID_NAME", "Название не может быть пустым.")
        )
        return
...
async def process_create_group_traffic(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            texts.t("ADMIN_PROMO_GROUP_INVALID_PERCENT", "Введите число от 0 до 100.")
        )
        return
...
async def prompt_edit_promo_group_field(
    ...
    if field == "name":
        ...
        current_discounts = _normalize_periods_dict(
...
async def process_edit_group_name(
    ...
    await _send_edit_menu_after_update(
        ...
        texts.t("ADMIN_PROMO_GROUP_UPDATED", "Промогруппа «{name}» обновлена.").format(
            name=html.escape(group.name)
        ),
    )
...
async def show_promo_group_members(
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text=texts.BACK, callback_data=f"promo_group_manage_{group_id}"
            )
        ]
    )
...
async def delete_promo_group_confirmed(
    ...
    await callback.message.edit_text(
        texts.t("ADMIN_PROMO_GROUP_DELETED", "Промогруппа «{name}» удалена.").format(
            name=html.escape(group.name)
        ),
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_promo_group_details, F.data.startswith("promo_group_manage_")
    )
    dp.callback_query.register(
        start_create_promo_group, F.data == "admin_promo_group_create"
    )
    dp.callback_query.register(
    ...
    dp.message.register(
        process_create_group_name, AdminStates.creating_promo_group_name
    )
    dp.message.register(
