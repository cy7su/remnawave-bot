...
def _parse_query(query_text: str) -> ParsedQuery:
    ...
    if not rest:
        return ParsedQuery('subscription', username, target_id, 0, None, None, None, 0, 0)
    ...
    if flag == '-d':
        ...
        return ParsedQuery('discount', username, target_id, 0, None, None, None, pct, 0)

    if flag == '-b':
        rub = int(args[0]) if args and args[0].isdigit() else 0
        return ParsedQuery('balance', username, target_id, 0, None, None, None, 0, rub)

    if flag == '-t':
        gb = int(args[0]) if args and args[0].lstrip('-').isdigit() else 0
        gb = max(-999, min(999, gb))
        return ParsedQuery('temp_traffic', username, target_id, 0, None, None, None, 0, 0, gb)

    if flag == '-p':
        days, traffic, devices = _parse_sub_args(args)
        return ParsedQuery('subscription', username, target_id, 0, days, traffic, devices, 0, 0)
