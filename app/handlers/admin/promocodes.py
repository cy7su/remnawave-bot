...
"""
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        ...
        if promo.promo_group:
            text += (
                f'<b>Промогруппа:</b> {html.escape(promo.promo_group.name)} (приоритет: {promo.promo_group.priority})\n'
            )
...
"""
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        keyboard.insert(
            1,
            [types.InlineKeyboardButton(text='Сумма бонуса', callback_data=f'promo_edit_amount_{promo.id}')],
        )
    elif promo.type in [
        PromoCodeType.SUBSCRIPTION_DAYS.value,
        PromoCodeType.TRIAL_SUBSCRIPTION.value,
        ...
        keyboard.insert(
            1,
            [types.InlineKeyboardButton(text='Количество дней', callback_data=f'promo_edit_days_{promo.id}')],
        )
