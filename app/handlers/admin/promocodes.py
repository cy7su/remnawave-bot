...
"""
    ...
    await callback.message.edit_text(
        text, reply_markup=get_admin_promocodes_keyboard(db_user.language)
    )
...
async def show_promocodes_list(
    ...
    for promo in promocodes:
        ...
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text=f"{promo.code}", callback_data=f"promo_manage_{promo.id}"
                )
            ]
        )
...
"""
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        ...
        if discount_hours > 0:
            ...
            text += (
                f"<b>Скидка:</b> {promo.balance_bonus_kopeks}% (до первой покупки)\n"
            )
    ...
    first_purchase_btn_text = (
        "Первая покупка: " if first_purchase_only else "Первая покупка: "
    )

    keyboard = [
        [
            types.InlineKeyboardButton(
                text="Редактировать", callback_data=f"promo_edit_{promo.id}"
            ),
            types.InlineKeyboardButton(
                text="Переключить статус", callback_data=f"promo_toggle_{promo.id}"
            ),
        ],
        [
            types.InlineKeyboardButton(
        ...
        [
            types.InlineKeyboardButton(
                text="Статистика", callback_data=f"promo_stats_{promo.id}"
            ),
            types.InlineKeyboardButton(
                text="Удалить", callback_data=f"promo_delete_{promo.id}"
            ),
        ],
        [
            types.InlineKeyboardButton(
                text="← К списку", callback_data="admin_promo_list"
            )
        ],
    ]
...
"""
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text="Дата окончания", callback_data=f"promo_edit_date_{promo.id}"
            )
        ],
        [
            types.InlineKeyboardButton(
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        keyboard.insert(
            ...
            [
                types.InlineKeyboardButton(
                    text="Сумма бонуса", callback_data=f"promo_edit_amount_{promo.id}"
                )
            ],
        )
    elif promo.type in [
        ...
        keyboard.insert(
            ...
            [
                types.InlineKeyboardButton(
                    text="Количество дней", callback_data=f"promo_edit_days_{promo.id}"
                )
            ],
        )
    ...
    keyboard.extend(
        [
            [
                types.InlineKeyboardButton(
                    text="← Назад", callback_data=f"promo_manage_{promo.id}"
                )
            ]
        ]
    )
...
"""
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="Отмена", callback_data=f"promo_edit_{promo_id}"
                )
            ]
        ]
    )
...
async def process_promo_group_selection(
    ...
    await state.update_data(
        promo_group_id=promo_group_id, promo_group_name=promo_group.name
    )
...
async def process_promocode_uses(
    ...
    try:
        ...
        await message.answer(
            "Введите срок действия промокода в днях (или 0 для бессрочного):"
        )
        await state.set_state(AdminStates.setting_promocode_expiry)
        ...
        await message.answer(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="К промокоду", callback_data=f"promo_manage_{promo_id}"
                        )
                    ]
                ]
            ),
...
"""
        ...
        if promocode.valid_until:
            summary_text += (
                f"<b>Действует до:</b> {format_datetime(promocode.valid_until)}\n"
            )
        ...
        await state.clear()
        logger.info(
            "Создан промокод администратором",
            code=code,
            telegram_id=db_user.telegram_id,
        )
...
"""
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="Да, удалить", callback_data=f"promo_delete_confirm_{promo.id}"
                ),
                types.InlineKeyboardButton(
                    text="Отмена", callback_data=f"promo_manage_{promo.id}"
                ),
            ]
        ]
    )
...
"""
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="К промокодам", callback_data="admin_promo_list"
                )
            ],
            [
                types.InlineKeyboardButton(
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_promocodes_list_page, F.data.startswith("admin_promo_list_page_")
    )
    ...
    dp.callback_query.register(
        process_promo_group_selection, F.data.startswith("promo_select_group_")
    )

    dp.callback_query.register(
        show_promocode_management, F.data.startswith("promo_manage_")
    )
    dp.callback_query.register(
        toggle_promocode_first_purchase, F.data.startswith("promo_toggle_first_")
    )
    dp.callback_query.register(
        toggle_promocode_status, F.data.startswith("promo_toggle_")
    )
    dp.callback_query.register(show_promocode_stats, F.data.startswith("promo_stats_"))

    dp.callback_query.register(
        start_edit_promocode_date, F.data.startswith("promo_edit_date_")
    )
    dp.callback_query.register(
        start_edit_promocode_amount, F.data.startswith("promo_edit_amount_")
    )
    dp.callback_query.register(
        start_edit_promocode_days, F.data.startswith("promo_edit_days_")
    )
    dp.callback_query.register(
        start_edit_promocode_uses, F.data.startswith("promo_edit_uses_")
    )
    dp.callback_query.register(
        show_general_promocode_stats, F.data == "admin_promo_general_stats"
    )

    dp.callback_query.register(
        show_promocode_edit_menu, F.data.regexp(r"^promo_edit_\d+$")
    )

    dp.callback_query.register(
        delete_promocode_confirmed, F.data.startswith("promo_delete_confirm_")
    )
    dp.callback_query.register(
        confirm_delete_promocode, F.data.startswith("promo_delete_")
    )
