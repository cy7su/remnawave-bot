...
def get_message_buttons_selector_keyboard(
    language: str = "ru",
) -> types.InlineKeyboardMarkup:
    return get_updated_message_buttons_selector_keyboard(
        list(DEFAULT_SELECTED_BUTTONS), language
    )
...
def get_updated_message_buttons_selector_keyboard(
    ...
    return get_updated_message_buttons_selector_keyboard_with_media(
        selected_buttons, False, language
    )
...
def create_broadcast_keyboard(
    ...
    if custom_buttons:
        for btn in custom_buttons:
            ...
            if action_type == "url":
                keyboard.append(
                    [types.InlineKeyboardButton(text=label, url=action_value)]
                )
            else:
                # callback type
                keyboard.append(
                    [types.InlineKeyboardButton(text=label, callback_data=action_value)]
                )
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                if not broadcast_history:
                    logger.critical(
                        "Не удалось найти запись BroadcastHistory для записи результатов",
...
async def show_pinned_message_menu(
    ...
    if pinned_message:
        ...
        timestamp_text = (
            last_updated.strftime("%d.%m.%Y %H:%M") if last_updated else "—"
        )
        media_line = ""
        ...
        position_line = (
            "↑ Отправлять перед меню"
            if pinned_message.send_before_menu
            else "↓ Отправлять после меню"
        )
        start_mode_line = (
            "При каждом /start"
...
async def prompt_pinned_message_update(
    ...
    await callback.message.edit_text(
        "<b>Новое закрепленное сообщение</b>\n\n"
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Отмена", callback_data="admin_pinned_message"
                    )
                ]
            ]
        ),
...
async def process_pinned_message_update(
    ...
    await message.answer(
        ...
        reply_markup=get_pinned_broadcast_confirm_keyboard(
            db_user.language, pinned_message.id
        ),
        parse_mode="HTML",
    )
...
async def show_tariff_filter(
        ...
        await callback.message.edit_text(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="← Назад", callback_data="admin_msg_by_sub"
                        )
                    ]
                ]
            ),
    ...
    buttons.append(
        [types.InlineKeyboardButton(text="← Назад", callback_data="admin_msg_by_sub")]
    )
...
"""
        keyboard = [
            [types.InlineKeyboardButton(text="← Назад", callback_data="admin_messages")]
        ]
    else:
        ...
        for broadcast in broadcasts:
            status_emoji = (
                ...
                else "" if broadcast.status == "failed" else ""
            )
            success_rate = (
                round((broadcast.sent_count / broadcast.total_count * 100), 1)
...
"""
        ...
        keyboard = get_broadcast_history_keyboard(
            page, total_pages, db_user.language
        ).inline_keyboard
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Отмена", callback_data="admin_messages"
                    )
                ]
            ]
        ),
...
async def handle_media_selection(
    ...
    instruction_text = f'{media_instructions.get(media_type, "Отправьте медиафайл:")}\n\n<i>Размер файла не должен превышать 50 МБ</i>'
    ...
    if is_media_message:
        ...
        await callback.message.answer(
            instruction_text, reply_markup=instruction_keyboard, parse_mode="HTML"
        )
    else:
        await callback.message.edit_text(
            instruction_text, reply_markup=instruction_keyboard, parse_mode="HTML"
...
async def show_media_preview(message: types.Message, db_user: User, state: FSMContext):
    ...
    preview_text = f"<b>Медиафайл добавлен</b>\n\n<b>Тип:</b> {media_type}\nФайл сохранен и готов к отправке\n\nЧто делать дальше?"
...
async def handle_media_confirmation(
    ...
    if action == "confirm_media":
        ...
        await state.update_data(
            has_media=False, media_file_id=None, media_type=None, media_caption=None
        )
        await show_button_selector_callback(callback, db_user, state)
...
async def show_button_selector_callback(
    ...
    if has_media:
        ...
        media_info = f"\n<b>Медиафайл:</b> {media_type} добавлен"
...
<b>Реферальная программа</b> — откроет реферальную программу
...
️ <b>Поддержка</b> — свяжет с поддержкой
...
async def confirm_button_selection(
    ...
    if has_media:
        ...
        media_info = (
            f"\n<b>Медиафайл:</b> {media_type_names.get(media_type, media_type)}"
        )
    ...
    selected_names = [
        button_labels[key] for key in ordered_keys if key in selected_buttons
    ]
    if selected_names:
...
"""
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text="Отправить", callback_data="admin_confirm_broadcast"
            ),
            types.InlineKeyboardButton(
                text="Изменить кнопки", callback_data="edit_buttons"
            ),
        ]
    ]
    ...
    if has_media:
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text="Изменить медиа", callback_data="change_media"
                )
            ]
        )
...
async def confirm_broadcast(
        ...
        for attempt in range(_MAX_SEND_RETRIES):
            ...
            try:
                ...
                if (
                    ...
                    return "blocked"
                logger.debug(
                    "BadRequest при рассылке пользователю", telegram_id=telegram_id, e=e
                )
                return "failed"
    ...
    def _build_progress_text(
        ...
        if phase == "sending":
            blocked_line = (
                f"• Заблокировали бота: {current_blocked}\n" if current_blocked else ""
            )
            return (
    ...
    async def _update_progress_message(
        current_sent: int, current_failed: int, current_blocked: int = 0
    ) -> None:
        ...
        try:
            ...
            logger.debug(
                "FloodWait при обновлении прогресса, пропускаем: сек",
                retry_after=e.retry_after,
            )
    ...
    if skipped_email_users > 0:
        logger.info(
            "Пропущено email-only пользователей при рассылке",
            skipped_email_users=skipped_email_users,
        )
    ...
    success_rate = (
        round(sent_count / total_users_count * 100, 1) if total_users_count else 0
    )
    media_info = f"\n<b>Медиафайл:</b> {media_type}" if has_media else ""
    ...
    back_keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="К рассылкам", callback_data="admin_messages"
                )
            ]
        ]
    )
    ...
    try:
        await progress_message.edit_text(
            result_text, reply_markup=back_keyboard, parse_mode="HTML"
        )
    except TelegramBadRequest as e:
        ...
        query = (
            ...
            .where(
                ...
                or_(
                    ...
                    and_(
                        Subscription.id == None, User.has_had_paid_subscription == True
                    ),
                ),
            )
        )
    ...
    if target == "active_zero":
        ...
        query = (
            ...
            .where(
                ...
                Subscription.is_trial == False,
                or_(
                    Subscription.traffic_used_gb == None,
    ...
    if target.startswith("custom_"):
        ...
        if criteria == "today":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.created_at >= today
            )
        elif criteria == "week":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.created_at >= now - timedelta(days=7)
            )
        elif criteria == "month":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.created_at >= now - timedelta(days=30)
            )
        elif criteria == "active_today":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.last_activity >= today
            )
        elif criteria == "inactive_week":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.last_activity < now - timedelta(days=7)
            )
        elif criteria == "inactive_month":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.last_activity < now - timedelta(days=30)
            )
        elif criteria == "referrals":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.referred_by_id.isnot(None)
            )
        elif criteria == "direct":
            query = select(sql_func.count(User.id)).where(
                base_filter, User.referred_by_id.is_(None)
            )
        else:
            return 0
...
async def get_target_users(db: AsyncSession, target: str) -> list:
    ...
    if target == "active":
        return [
            ...
            for user in users
            if any(
                s.is_active and not s.is_trial
    ...
    if target == "trial":
        return [
            user
            for user in users
            if any(s.is_trial for s in (getattr(user, "subscriptions", None) or []))
        ]
    ...
    if target == "expired":
        ...
        for user in users:
            ...
            if subs:
                ...
                has_expired = any(
                    s.status in expired_statuses
                    or (s.end_date <= now and not s.is_active)
                    for s in subs
                )
                if has_expired:
    ...
    if target == "zero":
        return [
            ...
            for user in users
            if any(
                s.is_active and (s.traffic_used_gb or 0) <= 0
                for s in (getattr(user, "subscriptions", None) or [])
            )
        ]
    ...
    if target == "canceled_subscribers":
        return [
            ...
            for user in users
            if any(
                s.status == SubscriptionStatus.DISABLED.value
                for s in (getattr(user, "subscriptions", None) or [])
            )
        ]
    ...
    if target.startswith("tariff_"):
        ...
        return [
            ...
            for user in users
            if any(
                s.is_active and s.tariff_id == tariff_id
                for s in (getattr(user, "subscriptions", None) or [])
            )
        ]
...
async def get_custom_users(db: AsyncSession, criteria: str) -> list:
    ...
    if criteria == "today":
        ...
        stmt = select(User).where(
            and_(User.status == "active", User.referred_by_id.is_(None))
        )
    else:
        return []
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_pinned_message_menu, F.data == "admin_pinned_message"
    )
    dp.callback_query.register(
        toggle_pinned_message_position, F.data == "admin_pinned_message_position"
    )
    dp.callback_query.register(
        toggle_pinned_message_start_mode, F.data == "admin_pinned_message_start_mode"
    )
    dp.callback_query.register(
        delete_pinned_message, F.data == "admin_pinned_message_delete"
    )
    dp.callback_query.register(
        prompt_pinned_message_update, F.data == "admin_pinned_message_edit"
    )
    dp.callback_query.register(
        handle_pinned_broadcast_now, F.data.startswith("admin_pinned_broadcast_now:")
    )
    dp.callback_query.register(
        handle_pinned_broadcast_skip, F.data.startswith("admin_pinned_broadcast_skip:")
    )
    dp.callback_query.register(
        show_broadcast_targets, F.data.in_(["admin_msg_all", "admin_msg_by_sub"])
    )
    ...
    dp.callback_query.register(
        show_messages_history, F.data.startswith("admin_msg_history")
    )
    ...
    dp.callback_query.register(
        handle_media_confirmation, F.data.in_(["confirm_media", "replace_media"])
    )
    dp.callback_query.register(handle_change_media, F.data == "change_media")
    dp.message.register(
        process_broadcast_message, AdminStates.waiting_for_broadcast_message
    )
    dp.message.register(
        process_broadcast_media, AdminStates.waiting_for_broadcast_media
    )
    dp.message.register(
        process_pinned_message_update, AdminStates.editing_pinned_message
    )
