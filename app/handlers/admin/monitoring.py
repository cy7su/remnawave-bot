...
def _build_notification_settings_view(language: str):
    ...
    trial_channel_status = _format_toggle(
        config.get("trial_channel_unsubscribed", {}).get("enabled", True)
    )
    expired_1d_status = _format_toggle(config["expired_1d"].get("enabled", True))
    second_wave_status = _format_toggle(
        config["expired_second_wave"].get("enabled", True)
    )
    third_wave_status = _format_toggle(
        config["expired_third_wave"].get("enabled", True)
    )
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"Скидка 2-3 дня: {second_percent}%",
                    callback_data="admin_mon_notify_edit_2d_percent",
                )
            ],
            [
                InlineKeyboardButton(
                    text=f"Срок скидки 2-3 дня: {second_hours} ч",
...
async def _send_notification_preview(
    bot, chat_id: int, language: str, notification_type: str
) -> None:
    message, keyboard = await _build_notification_preview_message(
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    InlineKeyboardButton(
                        text="← Назад", callback_data="admin_submenu_settings"
                    )
                ],
            ]
        )
...
async def preview_trial_channel_notification(callback: CallbackQuery):
    try:
        language = callback.from_user.language_code or settings.DEFAULT_LANGUAGE
        await _send_notification_preview(
            callback.bot, callback.from_user.id, language, "trial_channel_unsubscribed"
        )
...
async def preview_expired_1d_notification(callback: CallbackQuery):
    try:
        language = callback.from_user.language_code or settings.DEFAULT_LANGUAGE
        await _send_notification_preview(
            callback.bot, callback.from_user.id, language, "expired_1d"
        )
        await callback.answer("Пример отправлен")
    except Exception as exc:
...
async def preview_second_wave_notification(callback: CallbackQuery):
    try:
        language = callback.from_user.language_code or settings.DEFAULT_LANGUAGE
        await _send_notification_preview(
            callback.bot, callback.from_user.id, language, "expired_2d"
        )
        await callback.answer("Пример отправлен")
    except Exception as exc:
...
async def preview_third_wave_notification(callback: CallbackQuery):
    try:
        language = callback.from_user.language_code or settings.DEFAULT_LANGUAGE
        await _send_notification_preview(
            callback.bot, callback.from_user.id, language, "expired_nd"
        )
        await callback.answer("Пример отправлен")
    except Exception as exc:
...
async def preview_all_notifications(callback: CallbackQuery):
    try:
        ...
        for notification_type in [
            ...
            await _send_notification_preview(
                callback.bot, chat_id, language, notification_type
            )
...
"""
            ...
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="← Назад", callback_data="admin_monitoring"
                        )
                    ]
                ]
            )
...
async def traffic_check_callback(callback: CallbackQuery):
    ...
    try:
        ...
        snapshot_age = (
            await traffic_monitoring_scheduler_v2.service.get_snapshot_age_minutes()
        )
        threshold_gb = (
            traffic_monitoring_scheduler_v2.service.get_fast_check_threshold_gb()
        )
...
"""
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Повторить", callback_data="admin_mon_traffic_check"
                    )
                ],
                [
                    InlineKeyboardButton(
...
async def monitoring_logs_callback(callback: CallbackQuery):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            success_rate = (
                round(total_success / len(all_logs) * 100, 1) if all_logs else 0
            )
...
"""
        ...
        await callback.bot.send_message(
            callback.from_user.id, test_message, parse_mode="HTML"
        )
...
• В очереди: {queue_len} чек(ов)"""
            ...
            if settings.is_nalogo_enabled():
                ...
                if pending_count > 0:
                    nalogo_buttons.append(
                        InlineKeyboardButton(
                            text=f"Проверить ({pending_count})",
                            callback_data="admin_mon_nalogo_pending",
                        )
                    )
                nalogo_buttons.append(
                    InlineKeyboardButton(
                        text="Сверка чеков", callback_data="admin_mon_receipts_missing"
                    )
                )
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="← Назад", callback_data="admin_mon_statistics"
                        )
                    ]
                ]
            ),
...
async def receipts_link_old_callback(callback: CallbackQuery):
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not transactions:
                await callback.answer(
                    "Нет старых транзакций для привязки", show_alert=True
                )
                return
            ...
            if not incomes:
                await callback.answer(
                    "Не удалось получить чеки из NaloGO", show_alert=True
                )
                return
                            ...
                            if operation_time:
                                try:
                                    ...
                                    t.receipt_created_at = (
                                        ...
                                        else parsed_time.replace(tzinfo=UTC)
                                    )
                                except Exception:
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Обновить", callback_data="admin_mon_reconcile_logs"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="Детали", callback_data="admin_mon_reconcile_logs_details"
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="← Назад", callback_data="admin_mon_reconcile_logs"
                    )
                ],
            ]
        )
    ...
    keyboard.extend(
        [
            [
                ...
                InlineKeyboardButton(
                    text="Очистить", callback_data="admin_mon_clear_logs"
                ),
            ],
            [InlineKeyboardButton(text="← Назад", callback_data="admin_monitoring")],
        ]
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Обновить", callback_data="admin_mon_logs"),
                InlineKeyboardButton(
                    text="Фильтры", callback_data="admin_mon_logs_filters"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="Очистить логи", callback_data="admin_mon_clear_logs"
                )
            ],
            [InlineKeyboardButton(text="← Назад", callback_data="admin_monitoring")],
        ]
...
async def process_notification_value_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        language = (
            data.get("settings_language")
            or message.from_user.language_code
            or settings.DEFAULT_LANGUAGE
        )
        texts = get_texts(language)
        await message.answer(
            texts.get("NOTIFICATION_VALUE_INVALID", "Введите целое число.")
        )
        return
    ...
    if (key == "expired_second_wave" and field == "percent") or (
        key == "expired_third_wave" and field == "percent"
    ):
        if value < 0 or value > 100:
            await message.answer("Процент скидки должен быть от 0 до 100.")
            return
    elif (key == "expired_second_wave" and field == "hours") or (
        key == "expired_third_wave" and field == "hours"
    ):
    ...
    if not success:
        await message.answer(
            texts.get(
                "NOTIFICATION_VALUE_INVALID", "Некорректное значение, попробуйте снова."
            )
        )
        return
...
async def toggle_fast_check(callback: CallbackQuery):
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            await BotConfigurationService.set_value(
                db, "TRAFFIC_FAST_CHECK_ENABLED", new_value
            )
            await db.commit()
...
async def toggle_daily_check(callback: CallbackQuery):
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            await BotConfigurationService.set_value(
                db, "TRAFFIC_DAILY_CHECK_ENABLED", new_value
            )
            await db.commit()
