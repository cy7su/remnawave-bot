...
def validate_html_tags(text: str) -> tuple[bool, str]:
    ...
    tags_with_pos = [
        ...
        for m in re.finditer(tag_pattern, clean_text)
    ]
        ...
        if tag_lower == "a":
            ...
            if href_match:
                ...
                if not re.match(r"^https?://|^tg://", url, re.IGNORECASE):
                    return (
                        False,
                        f"URL в теге <a> должен начинаться с http://, https:// или tg://. Найдено: {url}",
                    )
            else:
    ...
    for closing, tag, attrs, start_pos, end_pos in tags_with_pos:
        ...
        if closing:
            ...
            if last_opening_tag.lower() != tag_lower:
                return (
                    False,
                    f"Тег </{tag}> не соответствует открывающему тегу <{last_opening_tag}>",
                )
        else:
            # Это открывающий тег
            stack.append(tag)
...
async def show_welcome_text_panel(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_welcome_text_keyboard(
            db_user.language, welcome_settings["is_enabled"]
        ),
        parse_mode="HTML",
    )
...
async def show_current_welcome_text(
    ...
    placeholders_text = "\n".join(
        [f"• <code>{key}</code> - {desc}" for key, desc in placeholders.items()]
    )
...
async def process_welcome_text_edit(
    ...
    if success:
        ...
        placeholders = get_available_placeholders()
        placeholders_text = "\n".join(
            [f"• <code>{key}</code>" for key in placeholders.keys()]
        )
...
def register_welcome_text_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_placeholders_help, F.data == "show_placeholders_help"
    )

    dp.callback_query.register(show_formatting_help, F.data == "show_formatting_help")

    dp.callback_query.register(
        show_preview_welcome_text, F.data == "preview_welcome_text"
    )
