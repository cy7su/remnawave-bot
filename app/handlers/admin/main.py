...
async def show_admin_panel(
    ...
    try:
        ...
        logger.error(
            "Не удалось получить статистику Remnawave для админ-панели", error=e
        )

    await callback.message.edit_text(
        admin_text, reply_markup=get_admin_main_keyboard(db_user.language)
    )
...
async def show_communications_submenu(
    ...
    await callback.message.edit_text(
        ...
        + texts.t(
            "ADMIN_COMMUNICATIONS_SUBMENU_DESCRIPTION",
...
async def show_support_submenu(
    ...
    is_moderator_only = not settings.is_admin(
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_SUPPORT_TICKETS", "Тикеты поддержки"),
    ...
    await callback.message.edit_text(
        ...
        + (
            ...
            if is_moderator_only
                ...
                "ADMIN_SUPPORT_SUBMENU_DESCRIPTION",
                "Управление тикетами и настройками поддержки:",
            )
        ),
        reply_markup=kb,
        parse_mode="Markdown",
...
async def show_support_audit(
    ...
    if not logs:
        ...
        for log in logs:
            ...
            actor_id_display = (
                log.actor_telegram_id or f"user#{log.actor_user_id}"
                if log.actor_user_id
                else "unknown"
            )
            lines.append(
                f"{ts} • {role} <code>{actor_id_display}</code> — {action_text}{ticket_part}{extra}"
            )
    ...
    if total_pages > 1:
        if page > 1:
            nav_row.append(
                InlineKeyboardButton(
                    text="←", callback_data=f"admin_support_audit_page_{page - 1}"
                )
            )
        nav_row.append(
            InlineKeyboardButton(
...
async def show_system_submenu(
    ...
    await callback.message.edit_text(
        ...
        + texts.t(
            "ADMIN_SYSTEM_SUBMENU_DESCRIPTION",
...
async def clear_rules_command(message: types.Message, db_user: User, db: AsyncSession):
    try:
        ...
        if success:
            ...
            logger.info(
                "Правила очищены командой администратором",
                telegram_id=db_user.telegram_id,
                full_name=db_user.full_name,
            )
        else:
            await message.reply(
                "<b>Нет правил для очистки</b>\n\nАктивные правила не найдены."
            )
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_communications_submenu, F.data == "admin_submenu_communications"
    )

    dp.callback_query.register(show_support_submenu, F.data == "admin_submenu_support")
    dp.callback_query.register(
        show_support_audit,
        F.data.in_(["admin_support_audit"])
        | F.data.startswith("admin_support_audit_page_"),
    )

    dp.callback_query.register(
        show_settings_submenu, F.data == "admin_submenu_settings"
    )
