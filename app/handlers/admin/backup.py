        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Создать бекап", callback_data="backup_create"
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    text="Список бекапов", callback_data="backup_list"
                ),
                InlineKeyboardButton(text="Настройки", callback_data="backup_settings"),
            ],
...
def get_backup_list_keyboard(backups: list, page: int = 1, per_page: int = 5):
    ...
    if len(backups) > per_page:
        ...
        if page > 1:
            nav_row.append(
                InlineKeyboardButton(
                    text="←", callback_data=f"backup_list_page_{page - 1}"
                )
            )
    ...
    keyboard.extend(
        [[InlineKeyboardButton(text="Назад", callback_data="backup_panel")]]
    )
...
def get_backup_manage_keyboard(backup_filename: str):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="Удалить", callback_data=f"backup_delete_{backup_filename}"
                )
            ],
            [InlineKeyboardButton(text="К списку", callback_data="backup_list")],
        ]
    )
...
def get_backup_settings_keyboard(settings_obj):
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"Сжатие: {compression_status}",
...
"""
    ...
    await callback.message.edit_text(
        text, parse_mode="HTML", reply_markup=get_backup_main_keyboard(db_user.language)
    )
...
async def create_backup_handler(
    ...
    success, message, file_path = await backup_service.create_backup(
        created_by=created_by_id, compress=True
    )
    ...
    if not backups:
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Создать первый бекап", callback_data="backup_create"
                    )
                ],
                [InlineKeyboardButton(text="Назад", callback_data="backup_panel")],
            ]
...
<b>Сжатие:</b> {'Да' if backup_info.get('compressed') else 'Нет'}
<b>БД:</b> {backup_info.get('database_type', 'unknown')}
"""

    if backup_info.get("error"):
        text += f'\n<b>Ошибка:</b> {backup_info["error"]}'

    await callback.message.edit_text(
        text, parse_mode="HTML", reply_markup=get_backup_manage_keyboard(filename)
    )
    await callback.answer()
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                ...
                InlineKeyboardButton(
                    text="Отмена", callback_data=f"backup_manage_{filename}"
                ),
            ]
        ]
    )
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="К списку бекапов", callback_data="backup_list"
                        )
                    ]
                ]
            ),
...
Или выберите из существующих бекапов ниже."""
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Выбрать из списка", callback_data="backup_list"
                    )
                ],
...
async def restore_backup_execute(
    ...
    success, message = await backup_service.restore_backup(
        str(backup_path), clear_existing=clear_existing
    )
...
async def handle_backup_file_upload(
    ...
    if not document.file_name or not any(
        document.file_name.endswith(ext) for ext in allowed_extensions
    ):
        await message.answer(
...
async def show_backup_settings(
    ...
    text = f"""<b>Настройки системы бекапов</b>
...
"""
    ...
    await callback.message.edit_text(
        text, parse_mode="HTML", reply_markup=get_backup_settings_keyboard(settings_obj)
    )
    await callback.answer()
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        ...
        F.data.startswith("backup_delete_")
        & ~F.data.startswith("backup_delete_confirm_"),
    )

    dp.callback_query.register(
        delete_backup_execute, F.data.startswith("backup_delete_confirm_")
    )
    ...
    dp.callback_query.register(
        ...
        F.data.startswith("backup_restore_execute_")
        | F.data.startswith("backup_restore_clear_"),
    )
    ...
    dp.callback_query.register(
        ...
        F.data.in_(
            ["backup_toggle_auto", "backup_toggle_compression", "backup_toggle_logs"]
        ),
    )
