    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="Проверить обновления", callback_data="admin_updates_check"
            )
        ],
        [
            InlineKeyboardButton(
                text="Информация о версии", callback_data="admin_updates_info"
            )
        ],
        [
            InlineKeyboardButton(
...
async def show_updates_menu(
    ...
    try:
        ...
        status_text = (
            f"Доступно {total_newer} обновлений" if has_updates else "Актуальная версия"
        )
...
Система автоматически проверяет обновления каждый час и отправляет уведомления о новых версиях."""
...
async def check_updates(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        has_updates, newer_releases = await version_service.check_for_updates(
            force=True
        )
...
<b>Репозиторий:</b> {version_service.repo}"""
            ...
            updates_text = "\n".join(updates_list)
            more_text = (
                f"\n\nИ еще {len(newer_releases) - 5} обновлений..."
...
<b>Репозиторий:</b> {version_service.repo}"""
        ...
        if has_updates:
            keyboard.inline_keyboard.insert(
                ...
                [
                    InlineKeyboardButton(
                        text="Подробнее о версиях", callback_data="admin_updates_info"
                    )
                ],
            )
...
async def show_version_info(
    ...
    try:
        ...
        if last_check:
            system_info += (
                f'<b>Последняя проверка:</b> {last_check.strftime("%d.%m.%Y %H:%M")}\n'
            )
