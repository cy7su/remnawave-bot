...
async def _persist_maintenance_mode(db: AsyncSession, enabled: bool) -> None:
    try:
        ...
        logger.error(
            "Не удалось сохранить MAINTENANCE_MODE после переключения из бота",
            enabled=enabled,
            error=error,
        )
                ...
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="Отмена", callback_data="maintenance_panel"
                        )
                    ]
                ]
            ),
    ...
    await message.answer(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Панель техработ", callback_data="maintenance_panel"
                    )
                ]
            ]
        ),
...
async def send_manual_notification(
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="Онлайн", callback_data="manual_notify_online"
                ),
                types.InlineKeyboardButton(
                    text="Офлайн", callback_data="manual_notify_offline"
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text="Проблемы", callback_data="manual_notify_degraded"
                ),
                types.InlineKeyboardButton(
                    text="Обслуживание", callback_data="manual_notify_maintenance"
                ),
            ],
            [
                types.InlineKeyboardButton(
...
async def process_notification_message(
    ...
    try:
        ...
        success = await rw_service.send_manual_status_notification(
            message.bot, status, notification_message
        )
...
async def back_to_admin_panel(
    ...
    await callback.message.edit_text(
        texts.ADMIN_PANEL, reply_markup=get_admin_main_keyboard(db_user.language)
    )
    await callback.answer()
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        send_manual_notification, F.data == "maintenance_manual_notify"
    )

    dp.callback_query.register(
        handle_manual_notification, F.data.startswith("manual_notify_")
    )

    dp.callback_query.register(back_to_admin_panel, F.data == "admin_panel")

    dp.message.register(
        process_maintenance_reason, MaintenanceStates.waiting_for_reason
    )

    dp.message.register(
        process_notification_message, MaintenanceStates.waiting_for_notification_message
    )
