...
async def show_referral_statistics(
    ...
    try:
        ...
        if stats.get("active_referrers", 0) > 0:
            avg_per_referrer = (
                stats.get("total_paid_kopeks", 0) / stats["active_referrers"]
            )
...
"""
        ...
        keyboard_rows = [
            [
                types.InlineKeyboardButton(
                    text="Обновить", callback_data="admin_referrals"
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="Топ рефереров", callback_data="admin_referrals_top"
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="Диагностика логов", callback_data="admin_referral_diagnostics"
                )
            ],
        ]
        ...
        keyboard_rows.extend(
            [
                [
                    types.InlineKeyboardButton(
                        text="Настройки", callback_data="admin_referrals_settings"
                    )
                ],
...
"""
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Повторить", callback_data="admin_referrals"
                    )
                ],
                [types.InlineKeyboardButton(text="Назад", callback_data="admin_panel")],
            ]
...
def _get_top_keyboard(period: str, sort_by: str) -> types.InlineKeyboardMarkup:
    ...
    return types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=period_week, callback_data=f"admin_top_ref:week:{sort_by}"
                ),
                types.InlineKeyboardButton(
                    text=period_month, callback_data=f"admin_top_ref:month:{sort_by}"
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=sort_earnings, callback_data=f"admin_top_ref:{period}:earnings"
                ),
                types.InlineKeyboardButton(
                    text=sort_invited, callback_data=f"admin_top_ref:{period}:invited"
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text="Обновить", callback_data=f"admin_top_ref:{period}:{sort_by}"
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="К статистике", callback_data="admin_referrals"
                )
            ],
        ]
    )
...
async def _show_top_referrers_filtered(
    callback: types.CallbackQuery, db: AsyncSession, period: str, sort_by: str
):
    """Внутренняя функция отображения топа с фильтрами."""
    try:
        top_referrers = await get_top_referrers_by_period(
            db, period=period, sort_by=sort_by
        )
        ...
        if top_referrers:
            for i, referrer in enumerate(top_referrers[:20], 1):
                ...
                id_display = (
                    telegram_id or user_email or f"#{user_id}" if user_id else "N/A"
                )
...
async def show_pending_withdrawal_requests(
    ...
    if not requests:
        ...
        keyboard_rows.append(
            [types.InlineKeyboardButton(text="Назад", callback_data="admin_referrals")]
        )

        await callback.message.edit_text(
            text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=keyboard_rows)
        )
        await callback.answer()
        return
    ...
    for req in requests[:10]:
        ...
        user_name = (
            html.escape(user.full_name) if user and user.full_name else "Неизвестно"
        )
        ...
        risk_emoji = (
            ...
            if req.risk_score < 30
                ...
                else "high" if req.risk_score < 70 else "MAX"
            )
        )
...
"""
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text="К списку", callback_data="admin_withdrawal_requests"
            )
        ]
    )
            ...
            try:
                ...
                await callback.bot.send_message(
                    ...
                    texts.t(
                        ...
                        id=request.id, amount=texts.format_price(request.amount_kopeks)
                    ),
                )
            except Exception as e:
...
"""
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="Отмена", callback_data="admin_withdrawal_requests"
                )
            ]
        ]
    )
    ...
    await message.answer(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="К заявкам", callback_data="admin_withdrawal_requests"
                    )
                ],
                [
                    types.InlineKeyboardButton(
...
async def _show_diagnostics_for_period(
    ...
    try:
        ...
        # Анализируем логи
        report = await referral_diagnostics_service.analyze_period(
...
"""
        ...
        keyboard_rows = [
            ...
            [
                types.InlineKeyboardButton(
                    text="Загрузить лог-файл", callback_data="admin_ref_diag_upload"
                )
            ],
            [
                types.InlineKeyboardButton(
        ...
        keyboard_rows.extend(
            [
                [
                    types.InlineKeyboardButton(
                        text="Обновить", callback_data=f"admin_ref_diag:{period}"
                    )
                ],
                [
                    types.InlineKeyboardButton(
...
@admin_required
@error_handler
async def show_referral_diagnostics(
    callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext
):
...
@admin_required
@error_handler
async def preview_referral_fixes(
    callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext
):
    ...
    try:
        ...
        if period == "uploaded_file":
            ...
            if not report_data:
                await callback.answer(
                    "Отчёт загруженного файла не найден", show_alert=True
                )
                return
        ...
        if not report.lost_referrals:
            await callback.answer(
                "Нет потерянных рефералов для исправления", show_alert=True
            )
            return

        # Запускаем предпросмотр исправлений
        fix_report = await referral_diagnostics_service.fix_lost_referrals(
            db, report.lost_referrals, apply=False
        )
...
"""
        ...
        for i, detail in enumerate(fix_report.details[:10], 1):
            ...
            if detail.error:
                ...
                if detail.referred_by_set:
                    referrer_display = (
                        html.escape(detail.referrer_name)
                        if detail.referrer_name
                        else f"ID{detail.referrer_id}"
                    )
        ...
        back_button_callback = (
            f"admin_ref_diag:{period}"
            if period != "uploaded_file"
            else "admin_referral_diagnostics"
        )
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    types.InlineKeyboardButton(
                        text=back_button_text, callback_data=back_button_callback
                    )
                ],
            ]
        )
...
@admin_required
@error_handler
async def apply_referral_fixes(
    callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext
):
    ...
    try:
        ...
        fix_report = await referral_diagnostics_service.fix_lost_referrals(
            db, report.lost_referrals, apply=True
        )
...
"""
            for i, mb in enumerate(report.missing_bonuses[:15], 1):
                referral_name = html.escape(
                    mb.referral_full_name
                    or mb.referral_username
                    or str(mb.referral_telegram_id)
                )
                referrer_name = html.escape(
                    mb.referrer_full_name
                    or mb.referrer_username
                    or str(mb.referrer_telegram_id)
                )
                text += f"\n{i}. <b>{referral_name}</b>"
            ...
            keyboard = types.InlineKeyboardMarkup(
                inline_keyboard=[
                    ...
                    [
                        types.InlineKeyboardButton(
                            text="Обновить", callback_data="admin_ref_check_bonuses"
                        )
                    ],
                    [
                        types.InlineKeyboardButton(
...
async def apply_missing_bonuses(
    ...
    try:
        ...
        if not report_dict:
            await callback.answer(
                "Отчёт не найден. Обновите проверку.", show_alert=True
            )
            return
...
"""
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Проверить снова", callback_data="admin_ref_check_bonuses"
                    )
                ],
                [
                    types.InlineKeyboardButton(
...
async def sync_referrals_with_contest(
    ...
    try:
        ...
        paid_contests = await get_contests_for_events(
        ...
        for contest in all_contests:
            ...
            if "error" not in stats:
                ...
                contest_results.append(
                    f'• {html.escape(contest.title)}: +{stats.get("created", 0)} новых'
                )
            else:
...
"""
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="Отмена", callback_data="admin_referral_diagnostics"
                )
            ]
        ]
    )
...
async def receive_log_file(
    ...
    try:
        ...
        temp_file_path = str(
            Path(temp_dir) / f"ref_diagnostics_{message.from_user.id}_{file_name}"
        )
            ...
            await message.answer(
                ...
                reply_markup=types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text="Назад", callback_data="admin_referral_diagnostics"
                            )
                        ]
                    ]
                ),
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_top_referrers_filtered, F.data.startswith("admin_top_ref:")
    )
    dp.callback_query.register(
        show_referral_settings, F.data == "admin_referrals_settings"
    )
    dp.callback_query.register(
        show_referral_diagnostics, F.data == "admin_referral_diagnostics"
    )
    dp.callback_query.register(
        show_referral_diagnostics, F.data.startswith("admin_ref_diag:")
    )
    dp.callback_query.register(
        preview_referral_fixes, F.data == "admin_ref_fix_preview"
    )
    dp.callback_query.register(apply_referral_fixes, F.data == "admin_ref_fix_apply")

    # Загрузка лог-файла
    dp.callback_query.register(
        request_log_file_upload, F.data == "admin_ref_diag_upload"
    )
    dp.message.register(receive_log_file, AdminStates.waiting_for_log_file)

    # Проверка бонусов по БД
    dp.callback_query.register(
        check_missing_bonuses, F.data == "admin_ref_check_bonuses"
    )
    dp.callback_query.register(apply_missing_bonuses, F.data == "admin_ref_bonus_apply")
    dp.callback_query.register(
        sync_referrals_with_contest, F.data == "admin_ref_sync_contest"
    )

    # Хендлеры заявок на вывод
    dp.callback_query.register(
        show_pending_withdrawal_requests, F.data == "admin_withdrawal_requests"
    )
    dp.callback_query.register(
        view_withdrawal_request, F.data.startswith("admin_withdrawal_view_")
    )
    dp.callback_query.register(
        approve_withdrawal_request, F.data.startswith("admin_withdrawal_approve_")
    )
    dp.callback_query.register(
        reject_withdrawal_request, F.data.startswith("admin_withdrawal_reject_")
    )
    dp.callback_query.register(
        complete_withdrawal_request, F.data.startswith("admin_withdrawal_complete_")
    )

    # Тестовое начисление
    dp.callback_query.register(
        start_test_referral_earning, F.data == "admin_test_referral_earning"
    )
    dp.message.register(
        process_test_referral_earning, AdminStates.test_referral_earning_input
    )
