        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Добавить сообщение", callback_data="add_user_message"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Список сообщений", callback_data="list_user_messages:0"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Статистика", callback_data="user_messages_stats"
                )
            ],
...
def get_message_actions_keyboard(
    message_id: int, is_active: bool, language: str = "ru"
):
    from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=status_text, callback_data=f"toggle_user_message:{message_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Удалить", callback_data=f"delete_user_message:{message_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="К списку", callback_data="list_user_messages:0"
                )
            ],
        ]
    )
...
async def process_new_message_text(
    ...
    try:
        new_message = await create_user_message(
            db=db, message_text=message_text, created_by=db_user.id, is_active=True
        )
...
async def list_user_messages(
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="← Назад", callback_data=f"list_user_messages:{page - 1}"
            )
        )

    nav_buttons.append(
        InlineKeyboardButton(text="Добавить", callback_data="add_user_message")
    )

    if len(messages) == limit:
        nav_buttons.append(
            InlineKeyboardButton(
                text="Вперед ", callback_data=f"list_user_messages:{page + 1}"
            )
        )
    ...
    keyboard.append(
        [InlineKeyboardButton(text="Назад", callback_data="user_messages_panel")]
    )
...
async def process_edit_message_text(
    ...
    try:
        updated_message = await update_user_message(
            db=db, message_id=message_id, message_text=new_text
        )
...
def register_handlers(dp: Dispatcher):
    dp.callback_query.register(
        show_user_messages_panel, F.data == "user_messages_panel"
    )
    ...
    dp.callback_query.register(
        edit_user_message_start, F.data.startswith("edit_user_message:")
    )

    dp.message.register(
        process_edit_message_text, StateFilter(UserMessageStates.waiting_for_edit_text)
    )

    dp.callback_query.register(
        list_user_messages, F.data.startswith("list_user_messages")
    )

    dp.callback_query.register(
        view_user_message, F.data.startswith("view_user_message:")
    )

    dp.callback_query.register(
        toggle_message_status, F.data.startswith("toggle_user_message:")
    )

    dp.callback_query.register(
        delete_message_confirm, F.data.startswith("delete_user_message:")
    )
