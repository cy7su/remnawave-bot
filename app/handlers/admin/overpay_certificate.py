...
ENV_LOCK_NOTE = "OVERPAY_P12_PATH или OVERPAY_P12_PASSPHRASE заданы через переменные окружения — значения из БД не применяются."
...
def _back_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="К сертификату", callback_data="overpay_cert")]
        ]
    )
...
def _status_view() -> tuple[str, InlineKeyboardMarkup]:
    ...
    buttons = [
        [InlineKeyboardButton(text="Загрузить", callback_data="overpay_cert:upload")]
    ]
    if status["uploaded"]:
        buttons.append(
            [InlineKeyboardButton(text="Удалить", callback_data="overpay_cert:delete")]
        )
    buttons.append(
...
@admin_required
@error_handler
async def show_certificate_status(
...
@admin_required
@error_handler
async def start_certificate_upload(
    callback: CallbackQuery, state: FSMContext, **kwargs
) -> None:
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Удалить", callback_data="overpay_cert:delete_confirm"
                    ),
                    InlineKeyboardButton(text="Отмена", callback_data="overpay_cert"),
                ]
            ]
...
@admin_required
@error_handler
async def process_certificate_passphrase(
    message: Message, state: FSMContext, **kwargs
) -> None:
    if not message.text:
        await message.answer(
    ...
    if not file_id:
        await message.answer(
            "Файл не найден. Начните загрузку заново.", reply_markup=_back_keyboard()
        )
        return
    ...
    try:
        ...
        await message.answer(
            "Не удалось скачать файл. Начните загрузку заново.",
    ...
    async with AsyncSessionLocal() as db:
        try:
            metadata = await cert_service.store_certificate(
                db, buffer.getvalue(), passphrase
            )
