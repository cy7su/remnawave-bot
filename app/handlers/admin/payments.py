...
def _status_info(
    ...
    *,
    ...
    if record.method == PaymentMethod.PAL24:
        ...
        return mapping.get(
            status, ("", texts.t("ADMIN_PAYMENT_STATUS_UNKNOWN", "Unknown"))
        )
...
def _build_payment_details_text(record: PendingPayment, *, texts, language: str) -> str:
    ...
    if record.method == PaymentMethod.HELEKET:
        ...
        if getattr(payment, "payer_amount", None) and getattr(
            payment, "payer_currency", None
        ):
            lines.append(
    ...
    if record.method == PaymentMethod.CRYPTOBOT:
        ...
        if getattr(payment, "bot_invoice_url", None) or getattr(
            payment, "mini_app_invoice_url", None
        ):
            lines.append(
                texts.t("ADMIN_PAYMENT_HAS_LINK", "Payment link is available above.")
            )
...
def _parse_method_and_id(
    payload: str, *, prefix: str
) -> tuple[PaymentMethod, int] | None:
...
async def show_payments_overview(
    ...
    if page_records:
        for idx, record in enumerate(page_records, start=start_index + 1):
            lines.extend(
                _build_record_lines(
                    record, index=idx, texts=texts, language=db_user.language
                )
            )
            lines.append("")
        lines.append(notice)
        if has_checkable:
            ...
            lines.append(
                texts.t(
                    "ADMIN_PAYMENTS_CHECKABLE_COUNT", "Доступно для проверки: {count}"
                ).format(count=len(checkable_records))
            )
    else:
        empty_text = texts.t(
            "ADMIN_PAYMENTS_EMPTY", "No pending top-ups in the last 24 hours."
        )
        lines.append("")
...
async def manual_check_payment(
    ...
    if not record:
        ...
        await callback.answer(
            texts.t("ADMIN_PAYMENT_NOT_FOUND", "Payment not found."), show_alert=True
        )
        return
...
async def check_all_payments(
    ...
    checkable_records = [r for r in records if _is_checkable(r) and not r.is_paid]
    logger.info(
        "Found checkable records", checkable_records_count=len(checkable_records)
    )
    ...
    await callback.answer(
        texts.t("ADMIN_PAYMENTS_CHECKING_ALL", "Проверяю {count} платежей...").format(
    ...
    for record in checkable_records:
        try:
            logger.info(
                "Checking payment", method=record.method.value, local_id=record.local_id
            )
            updated = await run_manual_check(
                db, record.method, record.local_id, payment_service
            )
            checked += 1
            logger.info(
                "Check result: is_paid", is_paid=updated.is_paid if updated else None
            )
    ...
    result_lines = [
        ...
        "",
        texts.t("ADMIN_PAYMENTS_CHECK_ALL_CHECKED", "Проверено: {count}").format(
            count=checked
        ),
        texts.t("ADMIN_PAYMENTS_CHECK_ALL_CONFIRMED", "Подтверждено: {count}").format(
            count=confirmed
        ),
    ]
    if failed:
        result_lines.append(
            texts.t("ADMIN_PAYMENTS_CHECK_ALL_FAILED", "Ошибок: {count}").format(
                count=failed
            )
        )
...
def register_handlers(dp: Dispatcher) -> None:
    ...
    dp.callback_query.register(
        manual_check_payment, F.data.startswith("admin_payment_check_")
    )
    dp.callback_query.register(
        show_payment_details,
        F.data.startswith("admin_payment_")
        & ~F.data.startswith("admin_payment_check_"),
    )
    dp.callback_query.register(
        show_payments_overview, F.data.startswith("admin_payments_page_")
    )
