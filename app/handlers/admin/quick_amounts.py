...
def _format_amounts_line(quick_amounts: list[int] | None) -> str:
    ...
    defaults = ", ".join(
        f"{_format_rubles(amount)} ₽" for amount in DEFAULT_QUICK_AMOUNTS
    )
...
def _method_title(config, defaults: dict) -> str:
    ...
    return config.display_name or method_def.get(
        "default_display_name", config.method_id
    )
...
def _list_keyboard(configs: list, defaults: dict) -> InlineKeyboardMarkup:
    ...
    for config in configs:
        marker = "" if config.quick_amounts else ""
        buttons.append(
            [
                InlineKeyboardButton(
...
def _view_keyboard(method_id: str, has_custom: bool) -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                text="Изменить", callback_data=f"qamounts:edit:{method_id}"
            )
        ]
    ]
    if has_custom:
        buttons.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"qamounts:reset:{method_id}",
                )
            ]
        )
    buttons.append(
        [InlineKeyboardButton(text="К списку", callback_data="qamounts:list")]
    )
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
@admin_required
async def show_quick_amounts_list(
    ...
    await callback.message.edit_text(
        text, reply_markup=_list_keyboard(configs, defaults)
    )
...
@admin_required
async def start_edit_quick_amounts(
    callback: CallbackQuery, state: FSMContext, **kwargs
) -> None:
    method_id = callback.data.split(":", 2)[2]
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Отмена", callback_data=f"qamounts:view:{method_id}"
                    )
                ]
            ]
        ),
    ...
    async with AsyncSessionLocal() as db:
        try:
            config = await update_config(
                db, method_id, {"quick_amounts": amounts_kopeks}
            )
        except ValueError as error:
            logger.warning(
                "Некорректные быстрые суммы", method_id=method_id, error=error
            )
            await message.answer(
