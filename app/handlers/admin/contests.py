...
def _format_contest_summary(contest, texts, tz: ZoneInfo) -> str:
    start_local = (
        ...
        else contest.start_at.replace(tzinfo=UTC)
    )
    end_local = (
        contest.end_at if contest.end_at.tzinfo else contest.end_at.replace(tzinfo=UTC)
    )
    ...
    summary_time = (
        contest.daily_summary_time.strftime("%H:%M")
        if contest.daily_summary_time
        else "12:00"
    )
    ...
    if contest.prize_text:
        parts.append(
            texts.t("ADMIN_CONTEST_PRIZE", "Приз: {prize}").format(
...
async def list_contests(
    ...
    for contest in contests:
        title = (
            contest.title if len(contest.title) <= 25 else contest.title[:22] + "..."
        )
...
async def show_contest_details(
    ...
    if not settings.is_contests_enabled():
        await callback.answer(
            get_texts(db_user.language).t(
                "ADMIN_CONTESTS_DISABLED", "Конкурсы отключены."
            ),
            show_alert=True,
        )
        return
    ...
    if not contest:
        await callback.answer(
            texts.t("ADMIN_CONTEST_NOT_FOUND", "Конкурс не найден."), show_alert=True
        )
        return
    ...
    lines = [
        ...
        texts.t("ADMIN_CONTEST_TOTAL_EVENTS", "Зачётов: <b>{count}</b>").format(
            count=total_events
        ),
    ]
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_referral_contest_manage_keyboard(
            ...
            can_delete=(
                ...
                and (
                    contest.end_at.replace(tzinfo=UTC)
                    if contest.end_at.tzinfo is None
                    else contest.end_at
                )
                < datetime.now(UTC)
            ),
...
async def process_edit_summary_times(
    ...
    if not summary_time:
        await message.answer(
            texts.t(
                "ADMIN_CONTEST_INVALID_TIME",
                "Не удалось распознать время. Формат: 12:00 или 12:00,18:00",
            )
        )
        await state.clear()
        return
    ...
    await update_referral_contest(
        ...
        daily_summary_times=(
            ",".join(t.strftime("%H:%M") for t in times) if times else None
        ),
    )
...
async def delete_contest(
    ...
    if contest.is_active or contest.end_at > now_utc:
        await callback.answer(
            texts.t(
                "ADMIN_CONTEST_DELETE_RESTRICT",
                "Удалять можно только завершённые конкурсы.",
            ),
            show_alert=True,
        )
        return
    ...
    await callback.answer(
        texts.t("ADMIN_CONTEST_DELETED", "Конкурс удалён."), show_alert=True
    )
...
async def show_leaderboard(
    ...
    if not leaderboard:
        await callback.answer(
            texts.t("ADMIN_CONTEST_EMPTY_LEADERBOARD", "Пока нет участников."),
            show_alert=True,
        )
        return
...
async def select_contest_mode(
    ...
    mode = (
        "referral_paid"
        if callback.data == "admin_contest_mode_paid"
        else "referral_registered"
    )
...
async def process_title(
    ...
    await message.answer(
        texts.t(
            "ADMIN_CONTEST_ENTER_DESCRIPTION",
            "Опишите конкурс (или отправьте '-' чтобы пропустить):",
        )
    )
...
async def process_description(
    ...
    await message.answer(
        texts.t(
            "ADMIN_CONTEST_ENTER_PRIZE",
            "Укажите призы/выгоды конкурса (или '-' чтобы пропустить):",
        )
    )
...
async def process_start_date(
    ...
    if not start_dt:
        await message.answer(
            texts.t(
                "ADMIN_CONTEST_INVALID_DATE",
                "Не удалось распознать дату. Формат: 01.06.2024 12:00",
            )
        )
        return
...
@admin_required
@error_handler
async def finalize_contest_creation(
    message: types.Message, state: FSMContext, db_user, db: AsyncSession
):
    times = _parse_times(message.text or "")
    ...
    if not start_at_raw or not end_at_raw:
        await message.answer(
            texts.t("ADMIN_CONTEST_INVALID_DATE", "Не удалось распознать дату.")
        )
        return
...
async def show_detailed_stats(
    ...
    if virtual_count > 0:
        ...
        general_lines.append(
            f"Виртуальных: <b>{virtual_count}</b> (рефералов: {virtual_referrals})"
        )
...
async def show_detailed_stats_page(
    ...
    if contest_id is None or stats is None:
        ...
        stats = await referral_contest_service.get_detailed_contest_stats(
            db, contest_id
        )
...
async def sync_contest(
    ...
    detailed_stats = await referral_contest_service.get_detailed_contest_stats(
        db, contest_id
    )
    general_lines = [
        f"<b>{html.escape(contest.title)}</b>",
...
async def debug_contest_transactions(
    ...
    from app.database.crud.referral_contest import (
        debug_contest_transactions as debug_txs,
    )
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Отмена", callback_data=f"admin_contest_vp_{contest_id}"
                    )
                ],
            ]
        ),
...
async def process_virtual_participant_name(
    ...
    if not name or len(name) > 200:
        await message.answer(
            "Имя должно быть от 1 до 200 символов. Попробуйте ещё раз:"
        )
        return
    ...
    await message.answer(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="К списку", callback_data=f"admin_contest_vp_{contest_id}"
                    )
                ],
                [
                    types.InlineKeyboardButton(
...
async def delete_virtual_participant_handler(
    ...
    result = await db.execute(
        sa_select(ReferralContestVirtualParticipant).where(
            ReferralContestVirtualParticipant.id == vp_id
        )
    )
    ...
    rows = [
        [
            types.InlineKeyboardButton(
                text="Добавить", callback_data=f"admin_contest_vp_add_{contest_id}"
            ),
            types.InlineKeyboardButton(
                text="Массовка", callback_data=f"admin_contest_vp_mass_{contest_id}"
            ),
        ],
    ]
    if vps:
            ...
            rows.append(
                [
                    types.InlineKeyboardButton(
                        ...
                        callback_data=f"admin_contest_vp_edit_{v.id}",
                    ),
                    types.InlineKeyboardButton(
                        text="", callback_data=f"admin_contest_vp_del_{v.id}"
                    ),
                ]
            )
    rows.append(
        [
            types.InlineKeyboardButton(
                text="← Назад", callback_data=f"admin_contest_view_{contest_id}"
            )
        ]
    )
            ...
            await message.answer(
                ...
                reply_markup=types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text="Отмена", callback_data="admin_contests_referral"
                            )
                        ],
                    ]
                ),
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Отмена", callback_data=f"admin_contest_vp_{vp.contest_id}"
                    )
                ],
            ]
        ),
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_referral_contests_menu, F.data == "admin_contests_referral"
    )
    dp.callback_query.register(list_contests, F.data == "admin_contests_list")
    dp.callback_query.register(
        list_contests, F.data.startswith("admin_contests_list_page_")
    )
    dp.callback_query.register(
        show_contest_details, F.data.startswith("admin_contest_view_")
    )
    dp.callback_query.register(
        toggle_contest, F.data.startswith("admin_contest_toggle_")
    )
    dp.callback_query.register(
        prompt_edit_summary_times, F.data.startswith("admin_contest_edit_times_")
    )
    dp.callback_query.register(
        delete_contest, F.data.startswith("admin_contest_delete_")
    )
    dp.callback_query.register(
        show_leaderboard, F.data.startswith("admin_contest_leaderboard_")
    )
    dp.callback_query.register(
        show_detailed_stats_page,
        F.data.startswith("admin_contest_detailed_stats_page_"),
    )
    dp.callback_query.register(
        show_detailed_stats, F.data.startswith("admin_contest_detailed_stats_")
    )
    dp.callback_query.register(sync_contest, F.data.startswith("admin_contest_sync_"))
    dp.callback_query.register(
        debug_contest_transactions, F.data.startswith("admin_contest_debug_")
    )
    dp.callback_query.register(
        start_contest_creation, F.data == "admin_contests_create"
    )
    dp.callback_query.register(
        select_contest_mode,
    ...
    dp.message.register(
        process_description, AdminStates.creating_referral_contest_description
    )
    ...
    dp.message.register(
        finalize_contest_creation, AdminStates.creating_referral_contest_time
    )
    dp.message.register(
        process_edit_summary_times, AdminStates.editing_referral_contest_summary_times
    )

    dp.callback_query.register(
        start_add_virtual_participant, F.data.startswith("admin_contest_vp_add_")
    )
    dp.callback_query.register(
        delete_virtual_participant_handler, F.data.startswith("admin_contest_vp_del_")
    )
    dp.callback_query.register(
        start_edit_virtual_participant, F.data.startswith("admin_contest_vp_edit_")
    )
    dp.callback_query.register(
        start_mass_virtual_participants, F.data.startswith("admin_contest_vp_mass_")
    )
    dp.callback_query.register(
        show_virtual_participants, F.data.regexp(r"^admin_contest_vp_\d+$")
    )
    dp.message.register(
        process_virtual_participant_name, AdminStates.adding_virtual_participant_name
    )
    dp.message.register(
        process_virtual_participant_count, AdminStates.adding_virtual_participant_count
    )
    dp.message.register(
        process_edit_virtual_participant_count,
        AdminStates.editing_virtual_participant_count,
    )
    dp.message.register(
        process_mass_virtual_count, AdminStates.adding_mass_virtual_count
    )
    dp.message.register(
        process_mass_virtual_referrals, AdminStates.adding_mass_virtual_referrals
    )
