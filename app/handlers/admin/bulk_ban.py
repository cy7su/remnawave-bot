...
async def process_bulk_ban_list(
    ...
    try:
        successfully_banned, not_found, error_ids = (
            await bulk_ban_service.ban_users_by_telegram_ids(
                db=db,
        ...
        await message.answer(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="К пользователям", callback_data="admin_users"
                        )
                    ]
                ]
            ),
...
def register_bulk_ban_handlers(dp):
    ...
    dp.callback_query.register(
        start_bulk_ban_process, lambda c: c.data == "admin_bulk_ban_start"
    )
