...
async def show_daily_contests(
    ...
    if not templates:
        ...
        for tpl in templates:
            ...
            prize_info = (
                f"{tpl.prize_value} ({tpl.prize_type})"
                if tpl.prize_type
                else tpl.prize_value
            )
            lines.append(
                f"{status} <b>{tpl.name}</b> (slug: {tpl.slug}) — приз {prize_info}, макс {tpl.max_winners}"
            )
    ...
    keyboard_rows.append(
        [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_contests")]
    )
...
async def show_daily_contest(
    ...
    await callback.message.edit_text(
        "\n".join(lines),
        reply_markup=get_daily_contest_manage_keyboard(
            tpl.id, tpl.is_enabled, db_user.language
        ),
    )
    await callback.answer()
...
async def start_round_now(
    ...
    await callback.answer(
        texts.t("ADMIN_ROUND_STARTED", "Раунд запущен"), show_alert=True
    )
    await show_daily_contest(callback, db_user, db)
...
async def manual_start_round(
    ...
    if exists:
        await callback.answer(
            texts.t("ADMIN_ROUND_ALREADY_ACTIVE", "Раунд уже активен."), show_alert=True
        )
        await show_daily_contest(callback, db_user, db)
        return
    ...
    await callback.answer(
        texts.t("ADMIN_ROUND_STARTED", "Тестовый раунд запущен"), show_alert=True
    )
    await show_daily_contest(callback, db_user, db)
...
async def start_all_contests(
    ...
    if not templates:
        await callback.answer(
            texts.t("ADMIN_CONTESTS_EMPTY", "Нет активных конкурсов."), show_alert=True
        )
        return
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        show_daily_contest, F.data.startswith("admin_daily_contest_")
    )
    dp.callback_query.register(
        toggle_daily_contest, F.data.startswith("admin_daily_toggle_")
    )
    ...
    dp.callback_query.register(
        manual_start_round, F.data.startswith("admin_daily_manual_")
    )
    dp.callback_query.register(close_all_rounds, F.data == "admin_daily_close_all")
    dp.callback_query.register(
        reset_all_attempts, F.data == "admin_daily_reset_all_attempts"
    )
    dp.callback_query.register(
        reset_attempts, F.data.startswith("admin_daily_reset_attempts_")
    )
    dp.callback_query.register(close_round, F.data.startswith("admin_daily_close_"))
    dp.callback_query.register(
        prompt_edit_field, F.data.startswith("admin_daily_edit_")
    )
