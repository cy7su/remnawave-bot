...
async def _delete_message_later(
    bot, chat_id: int, message_id: int, delay: int = 10
) -> None:
    try:
        ...
        logger.debug(
            "Не удалось удалить сообщение опроса", message_id=message_id, error=error
        )
    ...
    *,
    ...
    try:
        ...
        if "message is not modified" in error_text:
            logger.debug(
                "Опросное сообщение уже актуально, пропускаем обновление", error=error
            )
            return True
...
def _build_options_keyboard(
    response_id: int, question: PollQuestion
...
async def handle_poll_start(
    ...
    if not response.poll or not response.poll.questions:
        await callback.answer(
            texts.t("POLL_EMPTY", "Опрос пока недоступен."), show_alert=True
        )
        return
    ...
    if not question:
        await callback.answer(
            texts.t("POLL_ERROR", "Не удалось загрузить вопросы."), show_alert=True
        )
        return
    ...
    if not await _update_poll_message(
        ...
        await callback.answer(
            texts.t("POLL_ERROR", "Не удалось показать вопрос."), show_alert=True
        )
        return
    await callback.answer()
...
async def handle_poll_answer(
    ...
    if not response.poll:
        await callback.answer(
            texts.t("POLL_ERROR", "Опрос недоступен."), show_alert=True
        )
        return
    ...
    if not question:
        await callback.answer(
            texts.t("POLL_ERROR", "Вопрос не найден."), show_alert=True
        )
        return
    ...
    if not option:
        await callback.answer(
            texts.t("POLL_ERROR", "Вариант ответа не найден."), show_alert=True
        )
        return
