...
async def _handle_wheel_spin_payment(
    ...
    try:
        ...
        if not subscription or not subscription.is_active:
            ...
            rubles_fallback = TelegramStarsService.calculate_rubles_from_stars(
                stars_amount
            )
            kopeks_fallback = int(
                (rubles_fallback * Decimal(100)).to_integral_value(
        ...
        prizes_with_probs = wheel_service.calculate_prize_probabilities(
            config, prizes, payment_value_kopeks
        )
        selected_prize = wheel_service._select_prize(prizes_with_probs)

        # Применяем приз
        generated_promocode = await wheel_service._apply_prize(
            db, user, selected_prize, config
        )
        ...
        prize_message = wheel_service._get_prize_message(
            selected_prize, generated_promocode
        )
...
async def _handle_trial_payment(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Ошибка создания пользователя RemnaWave для триала", rw_error=rw_error
            )
        ...
        try:
            ...
            logger.warning(
                "Ошибка отправки уведомления админам о триале", admin_error=admin_error
            )
...
async def _handle_guest_purchase_payment(
    ...
    try:
        ...
        if existing and existing.amount_kopeks:
            expected_stars = max(
                1, settings.rubles_to_stars(existing.amount_kopeks / 100)
            )
        ...
        logger.error(
            "Error handling guest purchase Stars payment", error=e, exc_info=True
        )
        await message.answer(
...
async def handle_pre_checkout_query(query: types.PreCheckoutQuery):
    ...
    try:
        ...
        if not query.invoice_payload or not query.invoice_payload.startswith(
            allowed_prefixes
        ):
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if not user:
                    logger.warning(
                        "Пользователь не найден в БД", from_user_id=query.from_user.id
                    )
                    await query.answer(
                        ok=False,
                        error_message=texts.t(
            ...
            logger.error(
                "Ошибка подключения к БД в pre_checkout_query", db_error=db_error
            )
            await query.answer(
                ok=False,
                error_message=texts.t(
        ...
        logger.info(
            "Pre-checkout одобрен для пользователя", from_user_id=query.from_user.id
        )
...
async def handle_successful_payment(
    message: types.Message, db: AsyncSession, state: FSMContext, **kwargs
):
    texts = get_texts(DEFAULT_LANGUAGE)
    ...
    try:
        ...
        if not user:
            logger.error(
                "Пользователь не найден при обработке Stars платежа", user_id=user_id
            )
            await message.answer(
                texts.t(
        ...
        if payment.invoice_payload and payment.invoice_payload.startswith(
            "wheel_spin_"
        ):
        ...
        for chat_id, message_id, label in [
            ...
            if message_id:
                try:
                    ...
                    Exception
                ) as delete_error:  # pragma: no cover - зависит от прав бота
                    logger.warning(
        ...
        if success:
            ...
            logger.error(
                "Ошибка обработки Stars платежа для пользователя", user_id=user.id
            )
            await message.answer(
                texts.t(
