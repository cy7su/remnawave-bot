...
async def handle_gift_activate(callback: types.CallbackQuery) -> None:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            ...
            .options(
                selectinload(GuestPurchase.user), selectinload(GuestPurchase.tariff)
            )
        ...
        tariff_name = (
            html_mod.escape(purchase.tariff.name)
...
def register_handlers(dp: Dispatcher) -> None:
    dp.callback_query.register(
        handle_gift_activate, F.data.startswith("gift_activate:")
    )
