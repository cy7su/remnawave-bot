...
from app.services.payment_service import (
    PaymentService,
    get_user_by_id as fetch_user_by_id,
)
...
async def start_wata_payment(
    ...
    keyboard = await get_topup_amount_keyboard(
        "wata", db_user.language, back_callback="back_to_menu"
    )
...
async def process_wata_payment_amount(
    ...
    try:
        ...
        logger.warning(
            "Не удалось удалить сообщение с суммой WATA", delete_error=delete_error
        )
    ...
    if prompt_message_id:
        try:
            ...
            logger.warning(
                "Не удалось удалить сообщение с запросом суммы WATA",
...
async def check_wata_payment_status(
    ...
    status_labels: dict[str, dict[str, str]] = {
        "Opened": {
            "emoji": "",
            "label": texts.t("WATA_STATUS_OPENED", "Ожидает оплаты"),
        },
        "Closed": {
            "emoji": "",
            "label": texts.t("WATA_STATUS_CLOSED", "Обрабатывается"),
        },
    ...
    label_info = status_labels.get(
        payment.status,
        {"emoji": "", "label": texts.t("WATA_STATUS_UNKNOWN", "Неизвестно")},
    )
