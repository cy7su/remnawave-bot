...
async def start_mulenpay_payment(
    ...
    keyboard = await get_topup_amount_keyboard(
        "mulenpay", db_user.language, back_callback="back_to_menu"
    )
...
async def process_mulenpay_payment_amount(
    ...
    try:
        ...
        logger.warning(
            "Не удалось удалить сообщение с суммой MulenPay", delete_error=delete_error
        )
    ...
    if prompt_message_id:
        try:
            ...
            logger.warning(
                "Не удалось удалить сообщение с запросом суммы MulenPay",
    ...
    try:
        ...
        payment_id_display = (
            mulen_payment_id if mulen_payment_id is not None else local_payment_id
        )
        ...
        try:
            ...
            logger.warning(
                "Не удалось сохранить данные сообщения MulenPay", error=error
            )
...
async def check_mulenpay_payment_status(
    ...
    try:
        ...
        if len(message_text) > 190:
            ...
            await callback.answer("Статус платежа отправлен в чат", show_alert=True)
        else:
