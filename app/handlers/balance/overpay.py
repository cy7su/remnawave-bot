...
async def process_overpay_payment_amount(
    ...
    if option == "int":
        ...
        if amount_eur < settings.OVERPAY_INT_MIN_EUR:
            min_rub = math.ceil(
                settings.OVERPAY_INT_MIN_EUR * settings.OVERPAY_RUB_PER_EUR
            )
            await message.answer(
                texts.t(
...
async def start_overpay_topup(
    ...
    keyboard_rows = [
        [InlineKeyboardButton(text=label, callback_data=f"topup_{method}")]
...
async def _start_overpay_option_topup_impl(
    ...
    if option == "int":
        min_int_kopeks = (
            math.ceil(settings.OVERPAY_INT_MIN_EUR * settings.OVERPAY_RUB_PER_EUR) * 100
        )
