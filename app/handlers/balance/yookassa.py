...
async def start_yookassa_payment(
    ...
    if not settings.is_yookassa_enabled():
        await callback.answer(
            "Оплата картой через YooKassa временно недоступна", show_alert=True
        )
        return
    ...
    keyboard = await get_topup_amount_keyboard(
        "yookassa", db_user.language, back_callback="back_to_menu"
    )
...
async def start_yookassa_sbp_payment(
    ...
    keyboard = await get_topup_amount_keyboard(
        "yookassa_sbp", db_user.language, back_callback="back_to_menu"
    )
...
async def process_yookassa_payment_amount(
    ...
    try:
            ...
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="Оплатить картой", url=confirmation_url
                    )
                ],
                [
                    types.InlineKeyboardButton(
        ...
        try:
            ...
            logger.warning(
                "Не удалось удалить сообщение с суммой YooKassa",
        ...
        if prompt_message_id and prompt_message_id != message.message_id:
            try:
                ...
                logger.warning(
                    "Не удалось удалить сообщение с запросом суммы YooKassa",
                    delete_error=delete_error,
                )
...
async def process_yookassa_sbp_payment_amount(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Не удалось удалить сообщение с суммой YooKassa (СБП)",
                delete_error=delete_error,
            )
        ...
        if prompt_message_id and prompt_message_id != message.message_id:
            try:
                ...
                logger.warning(
                    "Не удалось удалить сообщение с запросом суммы YooKassa (СБП)",
                    delete_error=delete_error,
                )
...
async def check_yookassa_payment_status(
    callback: types.CallbackQuery, db: AsyncSession
):
    try:
