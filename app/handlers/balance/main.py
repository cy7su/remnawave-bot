...
async def route_payment_by_method(
    ...
    if payment_method == "severpay":
        ...
        async with AsyncSessionLocal() as db:
            await process_severpay_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method == "paypear":
        ...
        async with AsyncSessionLocal() as db:
            await process_paypear_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method == "rollypay":
        ...
        async with AsyncSessionLocal() as db:
            await process_rollypay_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method in ("overpay", "overpay_fps", "overpay_card", "overpay_int"):
        ...
        async with AsyncSessionLocal() as db:
            await process_overpay_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method in ("aurapay", "aurapay_sbp", "aurapay_card"):
        ...
        async with AsyncSessionLocal() as db:
            await process_aurapay_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method in ("etoplatezhi", "etoplatezhi_sbp", "etoplatezhi_card"):
        ...
        async with AsyncSessionLocal() as db:
            await process_etoplatezhi_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method in (
        ...
        async with AsyncSessionLocal() as db:
            await process_antilopay_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method in ("jupiter", "jupiter_sbp"):
        ...
        async with AsyncSessionLocal() as db:
            await process_jupiter_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
    ...
    if payment_method in ("donut", "donut_card", "donut_sbp", "donut_sbp_qr"):
        ...
        async with AsyncSessionLocal() as db:
            await process_donut_payment_amount(
                message, db_user, db, amount_kopeks, state
            )
        return True
...
async def show_balance_menu(
    ...
    try:
        ...
        logger.warning(
            "Failed to edit balance message, sending a new one instead", error=error
        )
        await callback.message.answer(balance_text, reply_markup=reply_markup)
    await callback.answer()
...
async def show_balance_history(
    ...
    raw_transactions = await get_user_transactions(
        db, db_user.id, limit=TRANSACTIONS_PER_PAGE * 3, offset=offset
    )
    ...
    if not unique_transactions:
        await callback.message.edit_text(
            "История операций пуста", reply_markup=get_back_keyboard(db_user.language)
        )
        await callback.answer()
        return
...
async def show_payment_methods(
    ...
    if isinstance(callback.message, InaccessibleMessage):
        await callback.message.answer(
            full_text, reply_markup=keyboard, parse_mode="HTML"
        )
        await callback.answer()
        return
...
async def handle_successful_topup_with_cart(
    user_id: int, amount_kopeks: int, bot, db: AsyncSession
):
    ...
    try:
        ...
        if (
            current_state == "SubscriptionStates:cart_saved_for_topup"
            and state_data.get("saved_cart")
        ):
            ...
            if 0 < total_price <= user.balance_kopeks:
                ...
                balance_hint = (
                    f"Не хватает: {texts.format_price(missing, round_kopeks=False)}"
                )
...
"""
    ...
    await callback.message.edit_text(
        support_text, reply_markup=keyboard, parse_mode="HTML"
    )
    await callback.answer()
    ...
    try:
        if not message.text:
            ...
            await message.answer(
                texts.INVALID_AMOUNT, reply_markup=get_back_keyboard(db_user.language)
            )
            return
        ...
        if amount_rubles < 1:
            await message.answer(
                ...
                reply_markup=get_back_keyboard(
                    db_user.language, callback_data="balance_topup"
                ),
            )
            return
        ...
        if not await route_payment_by_method(
            message, db_user, amount_kopeks, state, payment_method
        ):
    ...
    try:
        ...
        if method.startswith("platega_m"):
            ...
            await state.update_data(
                payment_method="platega", platega_method=platega_method_code
            )
...
def register_balance_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        handle_balance_history_pagination, F.data.startswith("balance_history_page_")
    )
    ...
    dp.callback_query.register(
        start_yookassa_sbp_payment, F.data == "topup_yookassa_sbp"
    )
    ...
    dp.callback_query.register(
        check_yookassa_payment_status, F.data.startswith("check_yookassa_")
    )
    ...
    dp.callback_query.register(
        check_cryptobot_payment_status, F.data.startswith("check_cryptobot_")
    )
    ...
    dp.callback_query.register(
        check_heleket_payment_status, F.data.startswith("check_heleket_")
    )

    from .cloudpayments import start_cloudpayments_payment

    dp.callback_query.register(
        start_cloudpayments_payment, F.data == "topup_cloudpayments"
    )
    ...
    dp.callback_query.register(
        start_freekassa_sbp_topup, F.data == "topup_freekassa_sbp"
    )
    dp.callback_query.register(
        start_freekassa_card_topup, F.data == "topup_freekassa_card"
    )
    ...
    dp.callback_query.register(
        start_kassa_ai_card_topup, F.data == "topup_kassa_ai_card"
    )
    dp.callback_query.register(
        start_kassa_ai_sberpay_topup, F.data == "topup_kassa_ai_sberpay"
    )
    ...
    dp.callback_query.register(
        start_etoplatezhi_sbp_topup, F.data == "topup_etoplatezhi_sbp"
    )
    dp.callback_query.register(
        start_etoplatezhi_card_topup, F.data == "topup_etoplatezhi_card"
    )
    ...
    dp.callback_query.register(
        start_antilopay_sbp_topup, F.data == "topup_antilopay_sbp"
    )
    dp.callback_query.register(
        start_antilopay_card_topup, F.data == "topup_antilopay_card"
    )
    dp.callback_query.register(
        start_antilopay_sberpay_topup, F.data == "topup_antilopay_sberpay"
    )
    ...
    dp.callback_query.register(
        check_mulenpay_payment_status, F.data.startswith("check_mulenpay_")
    )

    from .wata import check_wata_payment_status

    dp.callback_query.register(
        check_wata_payment_status, F.data.startswith("check_wata_")
    )

    from .pal24 import check_pal24_payment_status

    dp.callback_query.register(
        check_pal24_payment_status, F.data.startswith("check_pal24_")
    )

    from .platega import check_platega_payment_status

    dp.callback_query.register(
        check_platega_payment_status, F.data.startswith("check_platega_")
    )

    dp.callback_query.register(
        handle_payment_methods_unavailable, F.data == "payment_methods_unavailable"
    )

    dp.callback_query.register(
        handle_topup_amount_callback, F.data.startswith("topup_amount|")
    )
    ...
    dp.callback_query.register(
        handle_confirm_unlink, F.data.startswith("confirm_unlink_")
    )
