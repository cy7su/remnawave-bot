...
def _locale_file_exists(language: str) -> bool:
    ...
    return any(
        (user_dir / f"{code}{extension}").exists()
...
def _determine_default_language() -> str:
    ...
    if _normalize_language_code(fallback_language) != _normalize_language_code(
        _FALLBACK_LANGUAGE
    ):
        _logger.warning(
...
def _directory_is_writable(directory: Path) -> bool:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            dir=directory, prefix=".locale_write_test_", delete=True
        ):
            pass
        return True
...
def ensure_locale_templates() -> None:
    ...
    try:
        ...
        _logger.warning(
            "Unable to create locales directory", destination=destination, error=error
        )
        return
