...
def _get_trusted_proxy_networks() -> tuple[IPNetwork, ...]:
    ...
    for part in raw_value.split(","):
        ...
        try:
            ...
            logger.warning(
                "Неверная сеть доверенного прокси YooKassa", candidate=candidate
            )
    ...
    *,
    ...
    return (
        ...
        if remote_ip is not None
            ...
            (
                ip
                for ip in (_parse_candidate_ip(value) for value in candidate_list)
                if ip is not None
            ),
            None,
        )
...
class YooKassaWebhookHandler:
    ...
    async def handle_webhook(self, request: web.Request) -> web.Response:
        try:
            logger.info(
                "Получен YooKassa webhook", method=request.method, path=request.path
            )
            ...
            if client_ip is None:
                logger.warning(
                    ...
                    header_ip_candidates=header_ip_candidates
                    + ([request.remote] if request.remote else []),
                )
            ...
            async with AsyncSessionLocal() as db:
                try:
                    ...
                    await db.rollback()
                    logger.error(
                        "Ошибка обработки webhook YooKassa", error=e, exc_info=True
                    )
                    return web.Response(status=500, text="Processing error")

        except Exception as e:
            logger.error(
                "Критическая ошибка обработки webhook YooKassa", error=e, exc_info=True
            )
