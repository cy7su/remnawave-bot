        ...
        if settings.is_maintenance_mode() and not self._status.is_active:
            asyncio.create_task(
                self.enable_maintenance(
                    reason="Включено из системных настроек", auto=False
                )
            )
...
"""
    ...
    async def _send_admin_notification(self, message: str, alert_type: str = "info"):
        ...
        try:
            ...
            logger.error(
                "Ошибка отправки уведомления через AdminNotificationService", error=e
            )
            return False
        ...
        for admin_id in admin_ids:
            try:
                await self._bot.send_message(
                    chat_id=admin_id, text=formatted_message, parse_mode="HTML"
                )
                success_count += 1
                ...
                logger.error(
                    "Ошибка отправки уведомления админу", admin_id=admin_id, error=e
                )

        if success_count > 0:
            logger.info(
                "Уведомление отправлено администраторам", success_count=success_count
            )
    ...
    async def enable_maintenance(
        self, reason: str | None = None, auto: bool = False
    ) -> bool:
        try:
            ...
            self._status.reason = reason or (
                "Автоматическое включение" if auto else "Включено администратором"
            )
    ...
    async def disable_maintenance(self) -> bool:
        try:
            ...
            notification_time = format_local_datetime(
                datetime.now(UTC), "%d.%m.%Y %H:%M:%S %Z"
            )
            ...
            async with api:
                for attempt in range(1, attempts + 1):
                    ...
                    if is_connected:
                        if attempt > 1:
                            logger.info(
                                "API Remnawave ответило с попытки", attempt=attempt
                            )
...
API снова отвечает на запросы.""",
                        ...
                        if self._status.is_active and self._status.auto_enabled:
                            ...
                            logger.info(
                                "API восстановился, режим техработ автоматически отключен"
                            )
...
Не удалось выполнить проверку доступности API.""",
    ...
    async def _save_status_to_cache(self):
        try:
            status_data = {
                ...
                "enabled_at": (
                    self._status.enabled_at.isoformat()
                    if self._status.enabled_at
                    else None
                ),
                ...
                "last_check": (
                    self._status.last_check.isoformat()
                    if self._status.last_check
                    else None
                ),
            }
    ...
    async def _load_status_from_cache(self):
        try:
            ...
            self._status.consecutive_failures = status_data.get(
    ...
    async def send_remnawave_status_notification(
        self, status: str, details: str = ""
    ) -> bool:
        try:
