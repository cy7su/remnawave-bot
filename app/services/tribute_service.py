...
class TributeService:
    ...
    @classmethod
    def remember_invoice_message(
        cls, user_id: int, chat_id: int, message_id: int
    ) -> None:
    ...
    async def _cleanup_invoice_message(self, user_id: int) -> None:
        ...
        try:
            ...
            logger.warning(
                "Не удалось удалить Tribute счёт", message_id=message_id, error=error
            )
    ...
    async def _handle_successful_payment(self, payment_data: dict[str, Any]):
        try:
            ...
            async for session in get_db():
                ...
                if not user:
                    logger.error(
                        "Пользователь не найден", user_telegram_id=user_telegram_id
                    )
                    return
                ...
                try:
                    ...
                    await process_referral_topup(
                        session, user.id, amount_kopeks, self.bot
                    )
                except Exception as e:
                    logger.error(
                        "Ошибка обработки реферального пополнения Tribute", error=e
                    )
                ...
                try:
                    ...
                    logger.error(
                        "Ошибка отправки уведомления о Tribute пополнении", error=e
                    )
            ...
            logger.error(
                "Ошибка обработки успешного Tribute платежа", error=e, exc_info=True
            )
            ...
            async for session in get_db():
                ...
                if transaction:
                    transaction.description = (
                        f"{transaction.description} (платеж отклонен)"
                    )
                    await session.commit()
        ...
        try:
            ...
            async for session in get_db():
                ...
                await send_cart_notification_after_topup(
                    user, amount_kopeks, session, self.bot
                )
                break
    ...
    async def _send_failure_notification(self, user_id: int):
        ...
        if not user_id:
            logger.debug(
                "Пропуск уведомления об ошибке Tribute для пользователя без telegram_id"
            )
            return
                ...
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="Попробовать снова", callback_data="menu_balance"
                        )
                    ],
                    [
                        InlineKeyboardButton(
    ...
    async def _send_refund_notification(self, user_id: int, amount_kopeks: int):
        ...
        if not user_id:
            logger.debug(
                "Пропуск уведомления о возврате Tribute для пользователя без telegram_id"
            )
            return
    ...
    async def force_process_payment(
        ...
        try:
            ...
            async for session in get_db():
                ...
                external_id = (
                    f"force_donation_{payment_id}_{int(datetime.now(UTC).timestamp())}"
                )
