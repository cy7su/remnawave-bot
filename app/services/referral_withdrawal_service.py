...
class ReferralWithdrawalService:
    ...
    async def get_first_referral_earning_date(
        ...
        """
        result = await db.execute(
            select(func.min(ReferralEarning.created_at)).where(
                ReferralEarning.user_id == user_id
            )
        )
    ...
    async def get_user_spending_after_first_earning(
        self, db: AsyncSession, user_id: int
    ) -> int:
        """
    ...
    async def get_referral_balance_stats(self, db: AsyncSession, user_id: int) -> dict:
        ...
        spending_after_earning = await self.get_user_spending_after_first_earning(
            db, user_id
        )
    ...
    async def can_request_withdrawal(
        ...
        if available < min_amount:
            return (
                False,
                f"Минимальная сумма вывода: {min_amount / 100:.0f}₽. Доступно: {available / 100:.0f}₽",
                stats,
            )
        ...
        if last_request:
            ...
            if not settings.REFERRAL_WITHDRAWAL_TEST_MODE:
                ...
                if datetime.now(UTC) < cooldown_end:
                    ...
                    return (
                        False,
                        f"Следующий запрос на вывод будет доступен через {days_left} дн.",
                        stats,
                    )
    ...
    async def analyze_for_money_laundering(
        ...
        if referral_ids:
            ...
            ref_deposits_result = await db.execute(
                select(
                    ...
                    func.coalesce(func.sum(Transaction.amount_kopeks), 0).label(
                        "total"
                    ),
                )
                .where(
            ...
            ref_deposit_map = {
                row.user_id: (row.count, row.total) for row in ref_deposits_result.all()
            }

            referrals_by_id = {r.id: r for r in referrals_list}
            max_deposits = (
                settings.REFERRAL_WITHDRAWAL_SUSPICIOUS_MAX_DEPOSITS_PER_MONTH
            )
            ...
            if suspicious_referrals:
                analysis["flags"].append(
                    f"Подозрительная активность у {len(suspicious_referrals)} реферала(ов)"
                )
        ...
        earnings_by_reason = {
            r.reason: {"count": r.count, "total": r.total} for r in earnings.fetchall()
        }
        ...
        recent_earnings = await db.execute(
            select(
                func.count(), func.coalesce(func.sum(ReferralEarning.amount_kopeks), 0)
            ).where(
                ReferralEarning.user_id == user_id,
        ...
        if recent_count > 20:
            analysis["risk_score"] += 15
            analysis["flags"].append(
                f"{recent_count} начислений за неделю ({recent_amount / 100:.0f}₽)"
            )
    ...
    async def create_withdrawal_request(
        ...
        if settings.REFERRAL_WITHDRAWAL_ONLY_REFERRAL_BALANCE:
            if amount_kopeks > stats["available_referral"]:
                return (
                    None,
                    f'Недостаточно реферального баланса. Доступно: {stats["available_referral"] / 100:.0f}₽',
                )
    ...
    async def approve_request(
        ...
        if user.balance_kopeks < request.amount_kopeks:
            return (
                False,
                f"Недостаточно средств на балансе. Баланс: {user.balance_kopeks / 100:.0f}₽",
            )
    ...
    def format_balance_stats_for_user(self, stats: dict, texts) -> str:
        ...
        text += (
            texts.t(
                "REFERRAL_WITHDRAWAL_STATS_WITHDRAWN", "Выведено: <b>{amount}</b>"
