...
class BotConfigurationService:
    ...
    def is_secret_key(cls, key: str) -> bool:
        ...
        return any(
            ...
            for keyword in ("TOKEN", "SECRET", "PASSWORD", "PASSPHRASE", "KEY")
        )
    ...
    CATEGORY_TITLES: dict[str, str] = {
        ...
        "PAYMENT_VERIFICATION": "Проверка платежей",
        ...
        "MODERATION": "Модерация и фильтры",
    ...
    CHOICES: dict[str, list[ChoiceOption]] = {
        ...
        "DEFAULT_TRAFFIC_RESET_STRATEGY": [
            ChoiceOption("NO_RESET", "Без сброса"),
        ...
        "LOG_LEVEL": [
            ...
            ChoiceOption("INFO", "Info"),
            ChoiceOption("WARNING", "Warning"),
    ...
    SETTING_HINTS: dict[str, dict[str, str]] = {
        ...
        "REMNAWAVE_AUTO_SYNC_TIMES": {
            "description": (
                "Список времени в формате HH:MM, когда запускается автосинхронизация в течение суток."
            ),
    ...
    def _build_simple_subscription_device_choices(cls) -> list[ChoiceOption]:
        ...
        max_limit = (
            getattr(settings, "MAX_DEVICES_LIMIT", default_limit) or default_limit
        )
        try:
        ...
        current_limit = (
            getattr(settings, "SIMPLE_SUBSCRIPTION_DEVICE_LIMIT", default_limit)
            or default_limit
        )
        try:
    ...
    def _build_simple_subscription_traffic_choices() -> list[ChoiceOption]:
        try:
            ...
            logger.warning(
                "Не удалось получить пакеты трафика", error=error, exc_info=True
            )
            packages = []
        ...
        current_limit = getattr(
            settings, "SIMPLE_SUBSCRIPTION_TRAFFIC_GB", default_limit
        )
        try:
            current_limit = int(current_limit)
        except (TypeError, ValueError):
        ...
        if choices:
            ...
            if python_type is str:
                ...
                if normalized is not None:
                    ...
                    readable = ", ".join(
                        f"{option.label} ({cls.format_value(option.value)})"
    ...
    async def reset_value(
        ...
        *,
        ...
        if cls._is_env_override(key):
            logger.info(
                "Настройка сброшена в БД, используется значение из окружения", key=key
            )
        else:
            original = cls.get_original_value(key)
            ...
            if key == "SALES_MODE":
                ...
                try:
                    ...
                    remnawave_sync_service.schedule_refresh(
                        run_immediately=(
                            key == "REMNAWAVE_AUTO_SYNC_ENABLED" and bool(value)
                        )
                    )
                except Exception as error:
                    logger.error(
                ...
                try:
                    ...
                    logger.error(
                        "Не удалось применить новые настройки бекапа", error=error
                    )
            elif key in {
                "REMNAWAVE_API_URL",
                "REMNAWAVE_API_KEY",
