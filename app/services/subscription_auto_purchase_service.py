...
async def _prepare_auto_extend_context(
    ...
    if settings.is_multi_tariff_enabled():
        ...
        if saved_subscription_id is not None:
            ...
            subscription = (
                await get_subscription_by_id_for_user(db, parsed_sub_id, user.id)
                if parsed_sub_id
                else None
            )
            ...
            if len(active_subs) == 1:
                ...
                autopay_subs = [
                    s for s in active_subs if getattr(s, "autopay_enabled", False)
                ]
    ...
    if tariff_id:
        ...
        if (
            _tariff
            and _tariff.period_prices
            and not getattr(_tariff, "is_daily", False)
        ):
    ...
    if tariff_id:
        ...
        description = (
            cart_data.get("description")
            or f"Продление тарифа {tariff_name} на {format_days_declension(period_days)}"
        )
    else:
        description = (
            cart_data.get("description")
            or f"Продление подписки на {format_days_declension(period_days)}"
        )
...
def _apply_extension_updates(context: AutoExtendContext) -> None:
    ...
    if subscription.is_trial:
        ...
        if context.device_limit is not None:
            ...
            subscription.device_limit = max(
                subscription.device_limit or 0, settings.DEFAULT_DEVICE_LIMIT
            )
        if context.squad_uuid and context.squad_uuid not in (
            subscription.connected_squads or []
        ):
            subscription.connected_squads = (subscription.connected_squads or []) + [
                context.squad_uuid
            ]
    else:
        ...
        if context.traffic_limit_gb not in (None, 0):
            subscription.traffic_limit_gb = context.traffic_limit_gb
        if context.device_limit is not None and context.device_limit > (
            subscription.device_limit or 0
        ):
...
async def _auto_extend_subscription(
    ...
    *,
    ...
    saved_promo_percent = (
        ...
        if prepared.consume_promo_offer
        else 0
    )
    saved_promo_source = (
    ...
    )  # Запоминаем старый тариф для определения смены

    _apply_extension_updates(prepared)

    # Определяем, произошла ли смена тарифа
    is_tariff_change = (
        prepared.tariff_id is not None and old_tariff_id != prepared.tariff_id
    )
    ...
    try:
        ...
        updated_subscription = await extend_subscription(
            ...
            traffic_limit_gb=(
                conversion_traffic_limit_gb if apply_traffic_limit else None
            ),
    ...
    try:
        ...
        if hasattr(updated_subscription, "id") and hasattr(
...
async def _auto_purchase_tariff(
    ...
    *,
    ...
    saved_promo_source = (
        getattr(user, "promo_offer_discount_source", None) if consume_promo else None
    )
    saved_promo_expires = (
        getattr(user, "promo_offer_discount_expires_at", None)
        if consume_promo
    ...
    try:
        description = (
            f"Покупка тарифа {tariff.name} на {format_days_declension(period_days)}"
        )
    ...
    *,
    ...
    try:
        if existing_subscription:
            ...
            was_trial_conversion = (
                existing_subscription.is_trial
            ...
            old_tariff = (
                await _get_old_tariff(db, existing_subscription.tariff_id)
    ...
    *,
    ...
    if settings.is_multi_tariff_enabled() and _cart_sub_id_devices:
        locked_result = await db.execute(
            select(Subscription)
            .where(
                Subscription.id == _cart_sub_id_devices, Subscription.user_id == user.id
            )
            .with_for_update()
    ...
    if bot:
        try:
            ...
            logger.warning(
                "Автопокупка устройств: не удалось уведомить админов", error=error
            )
...
async def _auto_add_traffic(
    ...
    *,
    ...
    if is_tariff_mode:
        ...
        price_kopeks, _ = calculate_prorated_price(
    ...
    if bot:
        try:
            ...
            logger.warning(
                "Автопокупка трафика: не удалось уведомить админов", error=error
            )
    ...
    *,
    ...
    if settings.is_multi_tariff_enabled():
        ...
        expired_subs = [
            s
            for s in all_subs
            if s.status == SubscriptionStatus.EXPIRED.value and s.is_trial is False
        ]
        if not expired_subs:
            subscription = None
        else:
    ...
    try:
        ...
        if subscription.updated_at and (
            datetime.now(UTC) - subscription.updated_at
    ...
    # Deduct balance
    description = (
        f"Автопродление истёкшей подписки на {format_days_declension(period_days)}"
    )
    try:
        deducted = await subtract_user_balance(
            db,
...
async def _is_subscription_disabled(
    ...
    if settings.is_multi_tariff_enabled():
        ...
        if len(_active_subs) == 1:
            ...
            _non_daily = [
                s for s in _active_subs if not getattr(s, "is_daily_tariff", False)
            ]
    ...
    return (
        _existing_sub is not None
        and _existing_sub.status == SubscriptionStatus.DISABLED.value
    )
    ...
    if cart_mode in ("extend", "tariff_purchase", "daily_tariff_purchase"):
        try:
            if cart_sub_id:
                ...
                target_sub = await get_subscription_by_id_for_user(
                    db, cart_sub_id, user.id
                )
                if (
                    target_sub
                    and target_sub.updated_at
                    and (datetime.now(UTC) - target_sub.updated_at)
                    < timedelta(seconds=60)
                ):
                    logger.info(
                        ...
                        subscription_id=cart_sub_id,
                        total_seconds=(
                            datetime.now(UTC) - target_sub.updated_at
                        ).total_seconds(),
                    )
                    return False
            else:
                ...
                if recent_transactions:
                    ...
                    if (
                        ...
                        and (datetime.now(UTC) - last_tx.created_at)
                        < timedelta(seconds=60)
                    ):
                        logger.info(
                            ...
                            format_user_id=_format_user_id(user),
                            total_seconds=(
                                datetime.now(UTC) - last_tx.created_at
                            ).total_seconds(),
                        )
                        return False
        except Exception as check_error:
        ...
        if user.telegram_id:
            try:
                ...
                if (
                    ...
                    and subscription
                    and getattr(subscription, "tariff_id", None)
                ):
                    try:
    ...
    try:
        if was_trial_conversion:
            ...
            await notify_user_subscription_activated(
                user_id=user.id,
                subscription_id=subscription.id if subscription else None,
                expires_at=format_email_datetime(
            ...
            await notify_user_subscription_renewed(
                user_id=user.id,
                subscription_id=subscription.id if subscription else None,
                new_expires_at=format_email_datetime(
                    subscription.end_date if subscription else None
                ),
