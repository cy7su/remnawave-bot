...
class BlacklistService:
    ...
    def __init__(self):
        self.blacklist_data = (
            []
        )  # Список в формате [(telegram_id, username, reason), ...]
    ...
    async def is_user_blacklisted(
        ...
        if not self.blacklist_data or (
            self.last_update
            and datetime.now(UTC) - self.last_update > required_interval
        ):
    ...
    async def get_user_by_username(self, username: str) -> tuple[int, str, str] | None:
        ...
        for bl_id, bl_username, bl_reason in self.blacklist_data:
            if (
                bl_username == username_with_at
                or bl_username.lstrip("@") == username_without_at
            ):
                return (bl_id, bl_username, bl_reason)
        return None
