...
class PlategaService:
        ...
        *,
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                logger.debug(
                    "Platega request cancelled", method=method, endpoint=endpoint
                )
                raise
    ...
    async def _deserialize_response(
        ...
        if "json" in content_type.lower() or not content_type:
            try:
                ...
                logger.error(
                    "Failed to decode Platega JSON response",
                    url=response.url,
                    error=error,
                )
    ...
    def _sanitize_description(description: str, max_bytes: int) -> str:
        ...
        logger.debug(
            "Platega description trimmed from to bytes",
    ...
    def parse_expires_at(expires_in: str | None) -> datetime | None:
        ...
        try:
            ...
            logger.warning(
                "Failed to parse Platega expiresIn value", expires_in=expires_in
            )
            return None
