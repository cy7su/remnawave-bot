...
class UserCartService:
    ...
    async def save_user_cart(
        self, user_id: int, cart_data: dict[str, Any], ttl: int | None = None
    ) -> bool:
        """
        ...
        **also** written to the per-subscription key
        ...
        if client is None:
            logger.warning(
                "Redis недоступен, корзина пользователя НЕ сохранена", user_id=user_id
            )
            return False
        ...
        try:
            ...
            logger.error(
                "Ошибка сохранения корзины пользователя", user_id=user_id, error=e
            )
            return False
    ...
    async def get_user_cart(self, user_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error(
                "Ошибка получения корзины пользователя", user_id=user_id, error=e
            )
            return None
    ...
    async def delete_user_cart(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                "Ошибка удаления корзины пользователя", user_id=user_id, error=e
            )
            return False
    ...
    async def has_user_cart(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                "Ошибка проверки наличия корзины пользователя", user_id=user_id, error=e
            )
            return False
    ...
    async def has_topup_intent(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                "Ошибка чтения метки намерения пополнения", user_id=user_id, error=e
            )
            return False
    ...
    async def clear_topup_intent(self, user_id: int) -> None:
        ...
        try:
            ...
            logger.error(
                "Ошибка удаления метки намерения пополнения", user_id=user_id, error=e
            )
    ...
    async def delete_subscription_cart(
        self, user_id: int, subscription_id: int
    ) -> bool:
        ...
        try:
            ...
            while True:
                cursor, keys = await client.scan(
                    cursor=cursor, match=f"{prefix}*", count=50
                )
                for raw_key in keys:
                    ...
                    if raw_data:
                        try:
                            ...
                            logger.warning(
                                "Невалидный JSON в корзине подписки", key=key
                            )
                if not cursor or cursor == 0 or cursor == b"0":
                    break
        except Exception as e:
            logger.error(
                "Ошибка сканирования корзин подписок", user_id=user_id, error=e
            )
