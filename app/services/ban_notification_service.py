...
class BanNotificationService:
    ...
    async def _find_user_by_identifier(
        ...
        try:
            telegram_id = await remnawave_service.get_telegram_id_by_email(
                user_identifier
            )
            if telegram_id:
    ...
    async def send_punishment_notification(
        ...
        if not user:
            logger.warning(
                "Пользователь не найден в базе данных", user_identifier=user_identifier
            )
        ...
        try:
            ...
            message_text = settings.BAN_MSG_PUNISHMENT.format(
                ip_count=ip_count, limit=limit, ban_minutes=ban_minutes
            )
        ...
        if not user.telegram_id:
            reason = (
                f"IP лимит превышен: {ip_count}/{limit}. Бан на {ban_minutes} минут."
            )
            if node_name:
            ...
            if success:
                logger.info(
                    "Email уведомление о бане отправлено пользователю", user_id=user.id
                )
    ...
    async def send_warning_notification(
        ...
        if not user.telegram_id:
            ...
            if success:
                logger.info(
                    "Email предупреждение отправлено пользователю", user_id=user.id
                )
    ...
    async def send_network_wifi_notification(
        ...
        logger.info(
            "WiFi notification", node_name=repr(node_name), node_info=repr(node_info)
        )
        ...
        try:
            ...
            logger.warning(
                "BAN_MSG_WIFI template missing placeholders, adding node_info to end"
            )
        ...
        if not user.telegram_id:
            ...
            if success:
                logger.info(
                    "Email WiFi уведомление отправлено пользователю", user_id=user.id
                )
                return True, "Email уведомление отправлено", None
            return False, "Не удалось отправить email уведомление", None
    ...
    async def send_network_mobile_notification(
        ...
        if not user.telegram_id:
            reason = (
                f"Использование мобильной сети запрещено. Бан на {ban_minutes} минут."
            )
            if network_type:
                reason += f" Сеть: {network_type}"
            if node_name:
            ...
            if success:
                logger.info(
                    "Email Mobile уведомление отправлено пользователю", user_id=user.id
                )
                return True, "Email уведомление отправлено", None
            return False, "Не удалось отправить email уведомление", None
