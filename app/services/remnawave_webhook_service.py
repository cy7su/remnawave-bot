...
_ADMIN_NODE_CONNECTION_EVENTS = frozenset(
...
class RemnaWaveWebhookService:
    ...
    def _prune_intentional_panel_deletions(cls) -> None:
        if (
            not cls._intentional_panel_deletions_by_uuid
            and not cls._intentional_panel_deletions_by_telegram_id
        ):
            return
        ...
        uuid_keys = [
            ...
            >= cls._INTENTIONAL_PANEL_DELETION_GUARD_SECONDS
        ]
    ...
    def mark_intentional_panel_deletion(
        ...
        *,
        ...
        total = len(cls._intentional_panel_deletions_by_uuid) + len(
            cls._intentional_panel_deletions_by_telegram_id
        )
        if total >= cls._MAX_INTENTIONAL_ENTRIES:
            logger.warning(
                "Intentional deletion guard at capacity, skipping", total=total
            )
            return
    ...
    def _is_intentional_panel_deletion_event(cls, data: dict[str, Any]) -> bool:
        ...
        return any(
            uid in cls._intentional_panel_deletions_by_uuid for uid in candidate_uuids
        ) or any(
            tid in cls._intentional_panel_deletions_by_telegram_id
            for tid in candidate_telegram_ids
        )

    async def process_event(
        self, db: AsyncSession | None, event_name: str, data: dict
    ) -> bool:
    ...
    async def _process_user_event(
        self, db: AsyncSession, event_name: str, data: dict, handler: Any
    ) -> bool:
    ...
    async def _process_admin_event(self, event_name: str, data: dict) -> bool:
        ...
        if event_name == "service.subpage_config_changed":
            try:
                ...
                logger.warning(
                    "Failed to invalidate app config cache on subpage_config_changed"
                )
        ...
        if not self._admin_service.is_enabled:
            logger.debug(
                "Admin notifications disabled, skipping event", event_name=event_name
            )
            return True
        ...
        api_token = (
            data.get("apiToken") if isinstance(data.get("apiToken"), dict) else {}
        )
        name = html.escape(
            data.get("name")
            or data.get("nodeName")
            or data.get("username")
            or api_token.get("name")
            or ""
        )
        if name:
        ...
        due_date = html.escape(
            data.get("dueDate")
            or data.get("paymentDate")
            or data.get("nextBillingAt")
            or ""
        )
        if due_date:
        ...
        if isinstance(subpage, dict):
            ...
            lines.append(
                f"Действие: {action_labels.get(action, html.escape(str(action)))}"
            )
        ...
        try:
            ...
            logger.exception(
                "Failed to send admin notification for event", event_name=event_name
            )
            return False
    ...
    async def _enqueue_node_event(self, event_name: str, data: dict) -> bool:
        ...
        async with self._node_event_lock:
            ...
            if len(bucket) >= self._NODE_EVENT_BUFFER_MAX:
                ...
                self._node_event_overflow[event_name] = (
                    self._node_event_overflow.get(event_name, 0) + 1
                )
            else:
                bucket.append(data)
            if (
                self._node_event_flush_task is None
                or self._node_event_flush_task.done()
            ):
    ...
    async def _send_coalesced_node_notification(
        ...
        *,
        ...
        if overflow_count > 0:
            node_lines.append(
                f"• <i>… ещё {overflow_count} событий отброшено (buffer overflow)</i>"
            )
    ...
    def _get_renew_keyboard(
        self, user: User, subscription_id: int | None = None
            ...
            inline_keyboard=[
                [
                    build_miniapp_or_callback_button(
                        text=button_text, callback_data=extend_callback
                    )
                ],
            ]
        )
    ...
    def _get_subscription_keyboard(self, user: User) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    build_miniapp_or_callback_button(
                        text=button_text, callback_data="menu_subscription"
                    )
                ],
            ]
        )
    ...
    def _get_traffic_keyboard(self, user: User) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    build_miniapp_or_callback_button(
                        text=buy_text, callback_data="buy_traffic"
                    )
                ],
                [
                    build_miniapp_or_callback_button(
                        text=sub_text, callback_data="menu_subscription"
                    )
                ],
            ]
        )
    ...
    async def _notify_user(
        ...
        *,
        ...
        if not notification_type:
            logger.warning(
                "No NotificationType mapping for text_key", text_key=text_key
            )
            return
    ...
    async def _handle_user_disabled(
        ...
        if (
            ...
            and is_recently_updated_by_webhook(subscription)
        ):
            logger.info(
    ...
    async def _handle_user_modified(
        ...
        used_traffic_bytes = (
            ...
            and user_traffic.get("usedTrafficBytes") is not None
        ...
        subscription_crypto_link = data.get("subscriptionCryptoLink") or (
            data.get("happ") or {}
        ).get("cryptoLink", "")
    ...
    async def _handle_user_deleted(
        ...
        if sub_id and sub_id in self._recent_recreations:
            elapsed = (
                datetime.now(UTC) - self._recent_recreations[sub_id]
            ).total_seconds()
        ...
        if subscription:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        "Webhook: user not found after rollback", user_id=user_id
                    )
                    return
                if not user:
                    logger.error(
        ...
        for other_sub in getattr(user, "subscriptions", None) or []:
            ...
            sibling_uuid = getattr(other_sub, "remnawave_uuid", None) or getattr(
                user, "remnawave_uuid", None
            )
            ...
            await db.execute(
                delete(SubscriptionServer).where(
                    SubscriptionServer.subscription_id == other_sub.id
                )
            )
            logger.info(
    ...
    async def _attempt_panel_recreation(
    ...
    async def _handle_user_revoked(
        ...
        new_crypto_link = data.get("subscriptionCryptoLink") or (
            data.get("happ") or {}
        ).get("cryptoLink", "")
        changed = False
        ...
        if (
            new_url
            and self._is_valid_url(new_url)
            and subscription.subscription_url != new_url
        ):
    ...
    def _extract_device_info(cls, data: dict) -> tuple[str, str]:
        ...
        platform_display = (
            f"{emoji} {html.escape(platform)}" if emoji else html.escape(platform)
        )
    ...
    async def _handle_device_added(
        ...
        device_name = (
            f"{platform_display} ({tag})"
            if platform_display and tag
            else platform_display or tag
        )
        logger.info(
