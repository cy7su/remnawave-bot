...
def validate_p12(p12_bytes: bytes, passphrase: str | None) -> dict[str, Any]:
    ...
    try:
        private_key, certificate, additional_certs = pkcs12.load_key_and_certificates(
            p12_bytes, passphrase_bytes
        )
    except Exception as error:
        raise ValueError(
            "Не удалось прочитать P12: неверный пароль или повреждённый файл"
        ) from error
    ...
    not_valid_after = (
        getattr(certificate, "not_valid_after_utc", None) or certificate.not_valid_after
    )
    return {
...
async def store_certificate(
    db: AsyncSession, p12_bytes: bytes, passphrase: str | None
) -> dict[str, Any]:
    ...
    await bot_configuration_service.set_value(
        db, "OVERPAY_P12_PASSPHRASE", passphrase or ""
    )
    await db.commit()
    ...
    metadata["warning"] = (
        ENV_LOCK_WARNING.format(path=path)
