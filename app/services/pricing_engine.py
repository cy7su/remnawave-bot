...
class TariffSwitchResult:
    ...
    offer_discount_pct: int
    new_period_days: int = (
        0  # 0 = keep current end date, >0 = set new subscription period
    )
...
class PricingEngine:
    ...
    def calculate_traffic_discount(
        ...
        pct = PricingEngine.get_addon_discount_percent(
            user, "traffic", period_days_hint
        )
        if pct <= 0:
            return base_price, 0, 0
    ...
    def calculate_tariff_switch_cost(
        ...
        *,
        ...
        if not current_is_daily and new_is_daily:
            return self._calculate_switch_to_daily(
                new_tariff, remaining_days, user=user
            )
        if current_is_daily and not new_is_daily:
            return self._calculate_switch_from_daily(
                new_tariff, remaining_days, user=user
            )
        ...
        if group_pct > 0 or offer_pct > 0:
            upgrade_cost, _, _ = self.apply_stacked_discounts(
                raw_cost, group_pct, offer_pct
            )
        else:
    ...
    def _calculate_switch_from_daily(
        ...
        *,
        ...
        if new_tariff.period_prices:
            ...
            min_period_price = (
                new_tariff.period_prices.get(str(min_period_days), 0) or 0
            )
        ...
        if group_pct > 0 or offer_pct > 0:
            upgrade_cost, _, _ = self.apply_stacked_discounts(
                min_period_price, group_pct, offer_pct
            )
        else:
    ...
    async def _calculate_servers_price(
        ...
        *,
        ...
        try:
            ...
            Exception
        ) as e:  # intentional broad catch: pricing must not crash on DB errors, servers_price=0 is safe (user pays less)
            logger.error(
            ...
            return 0, [
                {"uuid": uuid, "id": None, "price": 0, "status": "error"}
        ...
        for uuid in country_uuids:
            ...
            if server is None:
                ...
                details.append(
                    {"uuid": uuid, "id": None, "price": 0, "status": "not_found"}
                )
                continue
            ...
            total_price += price
            details.append(
                {"uuid": uuid, "id": server.id, "price": price, "status": status}
            )
    ...
    def _calculate_traffic_price(
        ...
        purchased_price = (
            settings.get_traffic_price(purchased_gb) if purchased_gb > 0 else 0
        )
    ...
    async def calculate_renewal_price(
        ...
        *,
        ...
        return await self._calculate_classic_mode(
            db, subscription, period_days, user=user
        )
    ...
    async def _calculate_tariff_core(
        ...
        *,
        ...
        traffic_price = 0
        if custom_traffic_gb is not None and hasattr(
            tariff, "get_price_for_custom_traffic"
        ):
        ...
        if promo_group is not None and not tariff.is_available_for_promo_group(
            promo_group.id
        ):
            promo_group = None
        if promo_group is not None:
        ...
        if traffic_price > 0 and user and promo_group is not None:
            discounted_traffic, _, _ = self.calculate_traffic_discount(
                traffic_price, user
            )
    ...
    async def calculate_tariff_purchase_price(
        ...
        *,
        ...
        effective_device_limit = (
            device_limit if device_limit is not None else (tariff.device_limit or 0)
        )
        return await self._calculate_tariff_core(
            tariff,
            period_days,
    ...
    async def _calculate_classic_core(
        ...
        *,
        ...
        discounted_servers_per_month = self.apply_discount(
            servers_price_per_month, servers_pct
        )
        ...
        traffic_price_per_month = self._calculate_traffic_price(
            traffic_limit_gb, purchased_traffic_gb
        )
        discounted_traffic_per_month = self.apply_discount(
            traffic_price_per_month, traffic_pct
        )
        ...
        discounted_devices_per_month = self.apply_discount(
            devices_price_per_month, devices_pct
        )
        ...
        servers_group_discount = (
            servers_price_per_month - discounted_servers_per_month
        ) * months
        traffic_group_discount = (
            traffic_price_per_month - discounted_traffic_per_month
        ) * months
        devices_group_discount = (
            devices_price_per_month - discounted_devices_per_month
        ) * months
        ...
        breakdown = dataclasses.asdict(
            ClassicBreakdown(
                ...
                servers_individual_prices=[
                    self.apply_discount(d["price"], servers_pct) * months
                    for d in valid_servers
                ],
    ...
    def classic_pricing_to_purchase_details(pricing: RenewalPricing) -> dict[str, Any]:
        ...
        traffic_discount_total = (
            ...
            - PricingEngine.apply_discount(traffic_price_per_month, traffic_pct)
        ) * months
        servers_discount_total = (
            servers_price_per_month
            - PricingEngine.apply_discount(servers_price_per_month, servers_pct)
        ) * months
        devices_discount_total = (
            devices_price_per_month
            - PricingEngine.apply_discount(devices_price_per_month, devices_pct)
        ) * months
