...
class AdminNotificationService:
    def __init__(self, bot: Bot):
        ...
        self.ticket_topic_id = getattr(
            settings, "ADMIN_NOTIFICATIONS_TICKET_TOPIC_ID", None
        )
        ...
        self.category_topics: dict[NotificationCategory, int | None] = {
            NotificationCategory.PURCHASES: getattr(
                settings, "ADMIN_NOTIFICATIONS_PURCHASES_TOPIC_ID", None
            ),
            NotificationCategory.RENEWALS: getattr(
                settings, "ADMIN_NOTIFICATIONS_RENEWALS_TOPIC_ID", None
            ),
            NotificationCategory.TRIALS: getattr(
                settings, "ADMIN_NOTIFICATIONS_TRIALS_TOPIC_ID", None
            ),
            NotificationCategory.BALANCE: getattr(
                settings, "ADMIN_NOTIFICATIONS_BALANCE_TOPIC_ID", None
            ),
            NotificationCategory.ADDONS: getattr(
                settings, "ADMIN_NOTIFICATIONS_ADDONS_TOPIC_ID", None
            ),
            NotificationCategory.INFRASTRUCTURE: getattr(
                settings, "ADMIN_NOTIFICATIONS_INFRASTRUCTURE_TOPIC_ID", None
            ),
            NotificationCategory.ERRORS: getattr(
                settings, "ADMIN_NOTIFICATIONS_ERRORS_TOPIC_ID", None
            ),
            NotificationCategory.PROMO: getattr(
                settings, "ADMIN_NOTIFICATIONS_PROMO_TOPIC_ID", None
            ),
            NotificationCategory.PARTNERS: getattr(
                settings, "ADMIN_NOTIFICATIONS_PARTNERS_TOPIC_ID", None
            ),
    ...
    async def _get_referrer_info(
        self, db: AsyncSession, referred_by_id: int | None
    ) -> str:
        ...
        try:
            ...
            logger.error(
                "Ошибка получения данных рефера", referred_by_id=referred_by_id, error=e
            )
            return f"ID {referred_by_id}"

    async def _get_user_promo_group(
    ...
    async def send_trial_activation_notification(
        ...
        *,
        ...
        try:
            ...
            user_status = (
                "Новый" if not user.has_had_paid_subscription else "Существующий"
            )
            ...
            if promo_group:
                message_lines.append(
                    f"<b>Промогруппа:</b> {html.escape(promo_group.name)}"
                )
            else:
    ...
    async def send_subscription_purchase_notification(
        ...
        purchase_type: (
            str | None
        ) = None,  # 'first_purchase', 'renewal', 'tariff_switch', None (auto-detect)
    ) -> bool:
        try:
            total_amount = (
                amount_kopeks
                if amount_kopeks is not None
                else (abs(transaction.amount_kopeks) if transaction else 0)
            )
            ...
            elif purchase_type == "renewal" or (
                purchase_type is None and user.has_had_paid_subscription
            ):
    ...
    async def send_version_update_notification(
        self, current_version: str, latest_version, total_updates: int
    ) -> bool:
        ...
        try:
            ...
            repo = getattr(
                settings, "VERSION_CHECK_REPO", "fr1ngg/remnawave-bedolaga-telegram-bot"
            )
            release_url = (
                f"https://github.com/{repo}/releases/tag/{latest_version.tag_name}"
            )
            ...
            raw_description = (
                getattr(latest_version, "full_description", "")
                or latest_version.short_description
            )
            ...
            return await self._send_message(
                message, category=NotificationCategory.INFRASTRUCTURE
            )
    ...
    async def send_version_check_error_notification(
        self, error_message: str, current_version: str
    ) -> bool:
        if not self._is_enabled():
            return False
    ...
    ⚙️ <i>Система автоматических обновлений • {format_local_datetime(datetime.now(UTC), '%d.%m.%Y %H:%M:%S')}</i>"""
            ...
            return await self._send_message(
                message, category=NotificationCategory.ERRORS
            )

        except Exception as e:
            logger.error(
                "Ошибка отправки уведомления об ошибке проверки версий", error=e
            )
            return False
    ...
    def _build_balance_topup_message(
        ...
        *,
        ...
        if transaction.created_at:
            detail_lines.append(
                f'Создана: {format_local_datetime(transaction.created_at, "%d.%m.%Y %H:%M:%S")}'
            )

        if transaction.completed_at:
            detail_lines.append(
                f'Завершена: {format_local_datetime(transaction.completed_at, "%d.%m.%Y %H:%M:%S")}'
            )
    ...
    async def send_balance_topup_notification(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                "Перехвачена ошибка при создании сообщения уведомления",
                __name__=type(error).__name__,
                error=error,
            )
            if not self._is_lazy_loading_error(error):
                logger.error(
                    "Ошибка подготовки уведомления о пополнении",
                    error=error,
                    exc_info=True,
                )
                return False
            ...
            logger.warning(
                "Повторная загрузка данных для уведомления о пополнении после ошибки ленивой загрузки",
                error=error,
            )
            ...
            try:
                ...
                (
                    ...
                    subscription,
                    promo_group,
                ) = await self._reload_topup_notification_entities(
                    db, user, transaction
                )
        ...
        try:
            return await self._send_message(
                message, category=NotificationCategory.BALANCE
            )
        except Exception as e:
            logger.error(
                "Ошибка отправки уведомления о пополнении", error=e, exc_info=True
            )
            return False
    ...
    async def send_subscription_extension_notification(
        ...
        *,
        ...
        try:
            ...
            current_balance = (
                balance_after if balance_after is not None else user.balance_kopeks
            )
...
⏰ <i>{format_local_datetime(datetime.now(UTC), '%d.%m.%Y %H:%M:%S')}</i>"""
            ...
            return await self._send_message(
                message, category=NotificationCategory.RENEWALS
            )
        ...
        try:
            ...
            if promo_type == PromoCodeType.DISCOUNT.value:
                ...
                if subscription_days:
                    message_lines.append(
                        f"Срок действия скидки: {subscription_days} ч."
                    )
                else:
            ...
            message_lines.extend(
                [
                    ...
                    (
                        ...
                        if balance_before_kopeks is not None
                        and balance_after_kopeks is not None
    ...
    async def send_user_promo_group_change_notification(
        ...
        *,
        ...
        try:
            title = (
                "АВТОМАТИЧЕСКАЯ СМЕНА ПРОМОГРУППЫ" if automatic else "СМЕНА ПРОМОГРУППЫ"
            )
            initiator_line = None
            if initiator:
                initiator_line = f"<b>Инициатор:</b> {html.escape(initiator.full_name)} (ID: {initiator.telegram_id})"
            elif automatic:
            ...
            message_lines = [
                ...
                self._format_promo_group_block(
                    new_group, title="Новая промогруппа", icon=""
                ),
            ]
            ...
            if old_group and old_group.id != new_group.id:
                message_lines.extend(
                    [
                        "",
                        self._format_promo_group_block(
                            old_group, title="Предыдущая промогруппа", icon=""
                        ),
                    ]
                )
    ...
    def _resolve_topic_id(
        self, category: NotificationCategory | None = None
    ) -> int | None:
    ...
    async def _send_message(
        ...
        *,
        ...
        if category and not self.category_enabled.get(category, True):
            logger.debug(
                "Уведомление подавлено (категория отключена)", category=category.value
            )
            return False
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                logger.warning(
                    "Бот не имеет прав для отправки в чат", chat_id=self.chat_id
                )
                return False
                ...
                logger.warning(
                    "Telegram flood control при отправке в админ-чат", **log_kwargs
                )
    ...
    async def send_admin_notification(
        ...
        *,
        ...
        if not self._is_enabled():
            return False
        return await self._send_message(
            text, reply_markup=reply_markup, category=category
        )
        ...
        try:
            ...
            if is_cabinet:
                ...
                if buyer:
                    buyer_name = (
                        f"@{buyer.username}"
                        if buyer.username
                        else buyer.email or f"id:{buyer.id}"
                    )
                    message_lines.append(
                        f"Покупатель: <code>{html.escape(buyer_name)}</code>"
                    )
                else:
                    message_lines.append(
            ...
            if purchase.is_gift:
                if purchase.gift_recipient_value:
                    recipient_icon = (
                        "📧" if purchase.gift_recipient_type == "email" else "📱"
                    )
                    recipient_value = html.escape(purchase.gift_recipient_value)
                    message_lines.append(
                        f"{recipient_icon} Получатель: <code>{recipient_value}</code>"
                    )
                else:
    ...
    def _get_subscription_status(self, subscription: Subscription | None) -> str:
        ...
        if subscription.is_active:
            return (
                f'Активна (до {format_local_datetime(subscription.end_date, "%d.%m")})'
            )
        return "Неактивна"
    ...
    async def send_maintenance_status_notification(
        ...
        try:
            ...
            if event_type == "enable":
                ...
                message_parts.append(
                    f'<b>API URL:</b> {details.get("api_url", "неизвестно")}'
                )

                if status == "online":
                    if details.get("response_time"):
                        message_parts.append(
                            f'<b>Время отклика:</b> {details["response_time"]} сек'
                        )

                    if details.get("consecutive_failures", 0) > 0:
                        message_parts.append(
                            f'<b>Неудачных попыток было:</b> {details["consecutive_failures"]}'
                        )
                    ...
                    if details.get("consecutive_failures"):
                        message_parts.append(
                            f'<b>Попытка №:</b> {details["consecutive_failures"]}'
                        )
                ...
                if status == "started":
                    if details.get("check_interval"):
                        message_parts.append(
                            f'<b>Интервал проверки:</b> {details["check_interval"]} сек'
                        )
                    ...
                    if details.get("max_failures"):
                        message_parts.append(
                            f'<b>Порог ошибок:</b> {details["max_failures"]}'
                        )
    ...
    async def send_remnawave_panel_status_notification(
        ...
        try:
            ...
            status_config = {
                "online": {
                    "icon": "",
                    "title": "ПАНЕЛЬ REMNAWAVE ДОСТУПНА",
                    "alert_type": "success",
                },
                "offline": {
                    "icon": "",
                    "title": "ПАНЕЛЬ REMNAWAVE НЕДОСТУПНА",
                    "alert_type": "error",
                },
                "degraded": {
                    "icon": "",
                    "title": "ПАНЕЛЬ REMNAWAVE РАБОТАЕТ СО СБОЯМИ",
                    "alert_type": "warning",
                },
                "maintenance": {
                    "icon": "",
                    "title": "ПАНЕЛЬ REMNAWAVE НА ОБСЛУЖИВАНИИ",
                    "alert_type": "info",
                },
            }
            ...
            if status == "online":
                ...
                if details.get("users_online"):
                    message_parts.append(
                        f'<b>Пользователей онлайн:</b> {details["users_online"]}'
                    )
                ...
                if details.get("consecutive_failures"):
                    message_parts.append(
                        f'<b>Неудачных попыток:</b> {details["consecutive_failures"]}'
                    )

                message_parts.append("")
                message_parts.append(
                    "Панель недоступна. Проверьте соединение и статус сервера."
                )
                ...
                if details.get("maintenance_reason"):
                    message_parts.append(
                        f'<b>Причина:</b> {html.escape(details["maintenance_reason"])}'
                    )

                if details.get("estimated_duration"):
                    message_parts.append(
                        f'<b>Ожидаемая длительность:</b> {details["estimated_duration"]}'
                    )
            ...
            logger.error(
                "Ошибка отправки уведомления о статусе панели Remnawave", error=e
            )
            return False
    ...
    async def _format_servers_detailed(self, server_uuids: list[str]) -> str:
        ...
        try:
            ...
            logger.warning(
                "Ошибка получения названий серверов для уведомления", error=e
            )
            return f"{len(server_uuids)} серверов"
    ...
    async def send_partner_application_notification(
        ...
        try:
            ...
            if application_data.get("company_name"):
                message_lines.append(
                    f'Компания: {html.escape(str(application_data["company_name"]))}'
                )
            if application_data.get("telegram_channel"):
                message_lines.append(
                    f'Канал: {html.escape(str(application_data["telegram_channel"]))}'
                )
            if application_data.get("website_url"):
                message_lines.append(
                    f'Сайт: {html.escape(str(application_data["website_url"]))}'
                )
            ...
            if application_data.get("expected_monthly_referrals"):
                message_lines.append(
                    f'Ожидаемых рефералов: {application_data["expected_monthly_referrals"]}/мес'
                )
            if application_data.get("desired_commission_percent"):
                message_lines.append(
                    f'Желаемая комиссия: {application_data["desired_commission_percent"]}%'
                )
    ...
    async def send_bulk_ban_notification(
        ...
        try:
            ...
            message = "\n".join(message_lines)
            return await self._send_message(
                message, category=NotificationCategory.PARTNERS
            )
    ...
    async def send_ticket_event_notification(
        ...
        *,
        ...
        if media_file_id and media_type == "photo":
            return await self._send_ticket_photo_notification(
                text, media_file_id, keyboard
            )

        return await self._send_message(
            text, reply_markup=keyboard, category=NotificationCategory.TICKETS
        )
    ...
    async def send_suspicious_traffic_notification(
        self, message: str, bot: Bot, topic_id: int | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                "Ошибка отправки уведомления о подозрительной активности", error=e
            )
            return False
        except Exception as e:
            logger.error(
                "Неожиданная ошибка при отправке уведомления о подозрительной активности",
                error=e,
            )
            return False
