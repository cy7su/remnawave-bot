...
DATABASE_ERROR_KEYWORDS: Final[tuple[str, ...]] = (
    "database",
    "postgres",
    "connection refused",
)
...
class StartupNotificationService:
    ...
    async def send_startup_notification(self) -> bool:
        ...
        try:
            ...
            remnawave_connected, remnawave_status = (
                await self._check_remnawave_connection()
            )
    ...
    async def prewarm_logo(self) -> bool:
        ...
        try:
            ...
            msg = await asyncio.wait_for(
                self.bot.send_photo(**send_kwargs), timeout=timeout
            )
            ...
            logger.info(
                "Логотип прогрет на старте: file_id закеширован", chat_id=target
            )
            return True
        except Exception as e:
...
def _get_error_recommendations(error_message: str) -> str | None:
    ...
    if any(keyword in error_lower for keyword in WEBHOOK_ERROR_KEYWORDS):
        ...
        return (
            "<blockquote expandable><b>Рекомендации:</b>\n"
            + "\n".join(tips)
            + "</blockquote>"
        )
...
async def send_crash_notification(
    bot: Bot, error: Exception, traceback_str: str
) -> bool:
    """
    ...
    try:
        ...
        file_name = (
            f"crash_report_{datetime.now(UTC).strftime(DATETIME_FORMAT_FILENAME)}.txt"
        )
