...
class AppleIAPReconciliationService:
    ...
    async def reconcile_recent_transactions(
        self, db: AsyncSession, limit: int = 100
    ) -> AppleReconciliationResult:
        ...
        for apple_txn in transactions:
            if apple_txn.environment == "Sandbox":
                continue
            fetched = await self.apple_service.verify_transaction(
                apple_txn.transaction_id, apple_txn.environment
            )
            if not fetched:
                drift_count += 1
            ...
            if (
                fetched.get("appAccountToken")
                and fetched.get("appAccountToken") != apple_txn.app_account_token
            ):
