...
class YooKassaPaymentMixin:
    ...
    async def create_yookassa_sbp_payment(
        ...
        try:
            ...
            confirmation_token = (yookassa_response.get("confirmation", {}) or {}).get(
                "confirmation_token"
            )
            ...
            return {
                ...
                "qr_confirmation_data": yookassa_response.get(
        ...
        if payment.status == "succeeded" and getattr(payment, "is_paid", False):
            if not transaction_id:
                try:
                    ...
                    Exception
                ) as refresh_error:  # pragma: no cover - defensive logging
                    logger.warning(
            ...
            if transaction_id:
                ...
                try:
                    ...
                    Exception
                ) as process_error:  # pragma: no cover - defensive logging
                    logger.error(
    ...
    async def _process_successful_yookassa_payment(
        ...
        try:
            ...
            locked_result = await db.execute(
                select(YKPayment).where(YKPayment.id == payment.id).with_for_update()
            )
            ...
            if getattr(payment, "test_mode", False) and not getattr(
                settings, "YOOKASSA_TEST_MODE", False
            ):
                logger.warning(
            ...
            if get_transaction_by_external_id:
                try:
                    ...
                    logger.debug(
                        "get_transaction_by_external_id недоступен, пропускаем проверку дубликатов"
                    )
            ...
            try:
                ...
                logger.error(
                    "Ошибка парсинга метаданных платежа", parse_error=parse_error
                )
            ...
            if existing_transaction_id:
                try:
                    ...
                    transaction = await get_transaction_by_id(
                        db, existing_transaction_id
                    )
                except (
                    Exception
                ) as fetch_error:  # pragma: no cover - диагностический лог
                    logger.warning(
                ...
                if existing_transaction:
                    ...
                    if not getattr(payment, "transaction_id", None):
                        try:
                            ...
                            Exception
                        ) as link_error:  # pragma: no cover - защитный лог
                            logger.warning(
                                "Не удалось привязать платеж YooKassa к существующей транзакции",
                                yookassa_payment_id=payment.yookassa_payment_id,
                                existing_transaction_id=existing_transaction.id,
            ...
            if not getattr(payment, "transaction_id", None):
                ...
                if linked_payment:
                    payment.transaction_id = getattr(
                        linked_payment, "transaction_id", transaction.id
                    )
                    if hasattr(linked_payment, "transaction"):
                        payment.transaction = linked_payment.transaction
                ...
                if is_trial_payment:
                    ...
                    try:
                        ...
                        if subscription_id:
                                ...
                                if getattr(self, "bot", None):
                                    try:
                                        admin_notification_service = (
                                            AdminNotificationService(self.bot)
                                        )
                            ...
                            logger.error(
                                "Отсутствует subscription_id в metadata триального платежа YooKassa"
                            )
                    ...
                    full_subs = (
                        getattr(full_user, "subscriptions", []) if full_user else []
                    )
                    ...
                    if (
                        was_first_topup
                        and not getattr(user, "has_made_first_topup", False)
                    ...
                    if getattr(self, "bot", None):
                        try:
                            ...
                            logger.info(
                                "Уведомление админам о пополнении отправлено успешно"
                            )
                        except Exception as error:
                            logger.error(
                    ...
                    if not is_recurrent_topup:
                        ...
                        if getattr(self, "bot", None) and user.telegram_id:
                            try:
                                ...
                                logger.info(
                                    "Уведомление пользователю о платеже отправлено успешно"
                                )
                            except Exception as error:
                                logger.error(
                ...
                if is_simple_subscription:
                    ...
                    try:
                        ...
                        subscription_period = int(
                            payment_metadata.get("subscription_period", 30)
                        )
                            ...
                            if getattr(self, "bot", None) and user.telegram_id:
                                ...
                                if settings.is_multi_tariff_enabled() and getattr(
                                    ...
                                    try:
                                        ...
                                        _t = await get_tariff_by_id(
                                            db, subscription.tariff_id
                                        )
                                        if _t:
                                            tariff_line = f"\nТариф: «{_t.name}»"
                                    except Exception:
    ...
    async def _save_payment_method_if_available(
        ...
        try:
            ...
            existing = await get_payment_method_by_yookassa_id(
                db, pm_id, include_inactive=True
            )
            if existing:
                logger.debug(
    ...
    async def process_yookassa_webhook(
        ...
        if remote_data:
            ...
            event_object = self._merge_remote_yookassa_payload(
                event_object, remote_data
            )
        ...
        payment.refundable = event_object.get(
            "refundable", getattr(payment, "refundable", False)
        )

        current_paid = bool(
            getattr(payment, "is_paid", getattr(payment, "paid", False))
        )
        ...
        if payment.status == "succeeded" and payment.is_paid:
            return await self._process_successful_yookassa_payment(
                db, payment, event_object=event_object
            )
    ...
    async def _restore_missing_yookassa_payment(
        ...
        try:
            ...
            if user_id <= _INT32_MAX:
                ...
                if user:
                    meta_tg = metadata.get("user_telegram_id") or metadata.get(
                        "userTelegramId"
                    )
    ...
    def _normalise_yookassa_metadata(metadata: Any) -> dict[str, Any]:
        ...
        if isinstance(metadata, str):
            try:
                ...
                logger.debug(
                    "Не удалось распарсить metadata webhook YooKassa", metadata=metadata
                )
    ...
    def _extract_confirmation_url(event_object: dict[str, Any]) -> str | None:
        ...
        if isinstance(confirmation, dict):
            return confirmation.get("confirmation_url") or confirmation.get(
                "return_url"
            )
