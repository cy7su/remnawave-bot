...
class Pal24PaymentMixin:
    ...
    async def process_pal24_callback(
        ...
        try:
            ...
            if not payment and order_id:
                payment = await payment_module.get_pal24_payment_by_order_id(
                    db, order_id
                )

            if not payment:
                logger.error(
                    "Pal24 платеж не найден: /", bill_id=bill_id, order_id=order_id
                )
                return False
            ...
            locked = await pal24_lock_crud.get_pal24_payment_by_id_for_update(
                db, payment.id
            )
            if not locked:
                logger.error(
                    "Pal24: не удалось заблокировать платёж", payment_id=payment.id
                )
                return False
            payment = locked
            ...
            if status in {"PAID", "SUCCESS", "OVERPAID"}:
                ...
                callback_amount_str = (
                    callback.get("OutSum")
                    or callback.get("out_sum")
                    or callback.get("Amount")
                )
                ...
                balance_amount = callback.get("BalanceAmount") or callback.get(
                    "balance_amount"
                )
                if balance_amount is not None:
                    payment.balance_amount = balance_amount
                balance_currency = callback.get("BalanceCurrency") or callback.get(
                    "balance_currency"
                )
                if balance_currency is not None:
                    payment.balance_currency = balance_currency
                payer_account = (
                    callback.get("AccountNumber")
    ...
    async def _finalize_pal24_payment(
        ...
        *,
        ...
        await payment_module.link_pal24_payment_to_transaction(
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Pal24", error=error
                )
        ...
        try:
            ...
            if service and payment.bill_id:
                ...
                try:
                    ...
                    logger.error(
                        "Ошибка Pal24 API при получении статуса счёта", error=error
                    )
                else:
                    if response:
                        remote_payloads["bill_status"] = response
                        status_value = response.get("status") or (
                            response.get("bill") or {}
                        ).get("status")
                        if status_value:
                ...
                if payment.payment_id:
                    ...
                    try:
                        payment_response = await service.get_payment_status(
                            payment_id_str
                        )
                        ...
                        if payment_response:
                            ...
                            extracted = self._extract_remote_payment_info(
                                payment_response
                            )
                            if extracted:
                                payment_info_candidates.append(extracted)
                ...
                try:
                    ...
                    if payments_response:
                        ...
                        for candidate in self._collect_payment_candidates(
                            payments_response
                        ):
            ...
            payment_info = self._select_best_payment_info(
                payment, payment_info_candidates
            )
            if payment_info:
                remote_payloads.setdefault("selected_payment", payment_info)

            bill_success = (
                getattr(service, "BILL_SUCCESS_STATES", {"SUCCESS"})
                if service
                else {"SUCCESS"}
            )
            bill_failed = (
                getattr(service, "BILL_FAILED_STATES", {"FAIL"})
                if service
                else {"FAIL"}
            )
            bill_pending = (
                getattr(service, "BILL_PENDING_STATES", {"NEW", "PROCESS"})
                if service
                else {"NEW", "PROCESS"}
            )
            ...
            if remote_status:
                ...
                if remote_status in bill_success:
                    is_paid_update = True
                elif remote_status in bill_failed or (
                    remote_status in bill_pending and is_paid_update is None
                ):
            ...
            if payment_info:
                ...
                if payment_status_code:
                    existing_status = (
                        getattr(payment, "payment_status", "") or ""
                    ).upper()
                ...
                if balance_currency and balance_currency != (
                    payment.balance_currency or ""
                ):
                ...
                if payment_status_code:
                    ...
                    if payment_status_code in success_states:
                        ...
                        payment_status_code in failed_states
                        and is_paid_update is not True
                    ) or (
            ...
            secondary_url = (
                links_map.get("page") or links_map.get("card") or links_map.get("sbp")
            )
    ...
    def _extract_remote_payment_info(remote_data: Any) -> dict[str, str | None]:
        ...
        def _normalize(candidate: dict[str, Any]) -> dict[str, str | None]:
            ...
            return {
                ...
                "method": _stringify(
                    candidate.get("method") or candidate.get("payment_method")
                ),
                ...
                "balance_currency": _stringify(
                    candidate.get("balance_currency")
                    or candidate.get("BalanceCurrency")
                ),
                "account": _stringify(
    ...
    def _collect_payment_candidates(remote_data: Any) -> list[dict[str, Any]]:
        ...
        def _visit(value: Any) -> None:
            if isinstance(value, dict):
                ...
                has_status = any(
                    key in lower_keys for key in ("status", "payment_status")
                )
                has_identifier = any(
                    key in lower_keys
                    for key in (
