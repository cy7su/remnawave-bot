...
class OverpayPaymentMixin:
    ...
    async def create_overpay_payment(
        ...
        *,
        ...
        if option == "fps":
            ...
            payment_methods = (
                [m.strip() for m in payment_methods_str.split(",") if m.strip()]
        ...
        use_direct_qr = (
            option == "fps"
            and settings.is_overpay_sbp_direct_qr_enabled()
            and bool(effective_return_url)
        )
        ...
        try:
            ...
            if use_direct_qr:
                ...
                if init_result is not None:
                    ...
                    payment_url = await overpay_service.wait_for_redirect_link(
                        overpay_payment_id
                    )
                    if not payment_url:
                        logger.error(
            ...
            if not payment_url:
                ...
                overpay_payment_id = (
                    str(result.get("id", "")) if result.get("id") else None
                )
            ...
            expires_at = datetime.now(UTC) + timedelta(
                minutes=settings.OVERPAY_LIFETIME_MINUTES
            )
    ...
    async def process_overpay_webhook(
        ...
        try:
            ...
            if not overpay_payment_id or not overpay_status:
                logger.warning(
                    "Overpay webhook: отсутствуют обязательные поля", payload=payload
                )
                return False
            ...
            if merchant_transaction_id:
                payment = await overpay_crud.get_overpay_payment_by_order_id(
                    db, merchant_transaction_id
                )
            if not payment and overpay_payment_id:
                payment = await overpay_crud.get_overpay_payment_by_overpay_id(
                    db, overpay_payment_id
                )
            ...
            locked = await overpay_crud.get_overpay_payment_by_id_for_update(
                db, payment.id
            )
            if not locked:
                logger.error(
                    "Overpay: не удалось заблокировать платёж", payment_id=payment.id
                )
                return False
            payment = locked
            ...
            if payment.is_paid:
                logger.info(
                    "Overpay webhook: платеж уже обработан", order_id=payment.order_id
                )
                return True
            ...
            if is_paid:
                ...
                try:
                    ...
                    if remote_status is not None:
                        _, remote_paid = OVERPAY_STATUS_MAP.get(
                            remote_status, ("pending", False)
                        )
                        if not remote_paid:
                            logger.warning(
                ...
                payment.overpay_payment_id = (
                    overpay_payment_id or payment.overpay_payment_id
                )
    ...
    async def _finalize_overpay_payment(
        ...
        *,
        ...
        await overpay_crud.link_overpay_payment_to_transaction(
        ...
        if not should_credit_balance:
            logger.info(
                "Overpay платеж уже зачислил баланс ранее", order_id=payment.order_id
            )
            return True
        ...
        try:
            ...
            logger.error(
                "Ошибка обработки реферального пополнения Overpay", error=error
            )
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Overpay", error=error
                )
    ...
    async def check_overpay_payment_status(
        ...
        try:
            ...
            if payment.overpay_payment_id:
                try:
                    order_data = await overpay_service.get_payment(
                        payment.overpay_payment_id
                    )
                    ...
                    if overpay_status:
                        status_info = OVERPAY_STATUS_MAP.get(
                            overpay_status, ("pending", False)
                        )
                        internal_status, is_paid = status_info
                        ...
                        if is_paid:
                            ...
                            locked = (
                                await overpay_crud.get_overpay_payment_by_id_for_update(
                                    db, payment.id
                                )
                            )
                            if not locked:
                    ...
                    logger.error(
                        "Error checking Overpay payment status via API", error=e
                    )
