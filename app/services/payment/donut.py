...
class DonutPaymentMixin:
    ...
    async def process_donut_callback(
        ...
        try:
            ...
            if not our_order_id or not donut_status:
                logger.warning(
                    "Donut callback: отсутствуют обязательные поля", payload=payload
                )
                return False
            ...
            if not payment:
                logger.warning(
                    "Donut callback: платеж не найден", order_id=our_order_id
                )
                return False
            ...
            if not locked:
                logger.error(
                    "Donut: не удалось заблокировать платёж", payment_id=payment.id
                )
                return False
            payment = locked
            ...
            if payment.is_paid:
                logger.info(
                    "Donut callback: платеж уже обработан", order_id=payment.order_id
                )
                return True
            ...
            internal_status, is_paid = DONUT_STATUS_MAP.get(
                donut_status, ("pending", False)
            )
            ...
            if is_paid:
                ...
                payment.donut_transaction_id = (
                    str(donut_transaction_id)
                    if donut_transaction_id
                    else payment.donut_transaction_id
                )
                payment.callback_payload = callback_payload
                payment.updated_at = datetime.now(UTC)
                await db.flush()
                return await self._finalize_donut_payment(
    ...
    async def _finalize_donut_payment(
        ...
        *,
        ...
        if not should_credit_balance:
            logger.info(
                "Donut платеж уже зачислил баланс ранее", order_id=payment.order_id
            )
            return True
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Donut", error=error
                )
    ...
    async def check_donut_payment_status(
        ...
        try:
            ...
            logger.error(
                "Donut: ошибка проверки статуса", transaction_id=transaction_id, error=e
            )
            return None
