...
class FreekassaPaymentMixin:
        ...
        *,
        ...
        try:
            ...
            if not freekassa_service.verify_webhook_ip(client_ip):
                logger.warning(
                    "Freekassa webhook: недоверенный IP", client_ip=client_ip
                )
                return False
            ...
            if not payment:
                logger.warning(
                    "Freekassa webhook: платеж не найден order_id", order_id=order_id
                )
                return False

            # Lock payment row immediately to prevent concurrent webhook processing (TOCTOU race)
            locked = await freekassa_crud.get_freekassa_payment_by_id_for_update(
                db, payment.id
            )
            if not locked:
                logger.error(
    ...
    async def _finalize_freekassa_payment(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "Ошибка обработки реферального пополнения Freekassa", error=error
            )
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Freekassa", error=error
                )
    ...
    async def get_freekassa_payment_status(
        ...
        if not payment:
            logger.warning(
                "Freekassa payment not found: id", local_payment_id=local_payment_id
            )
            return None
        ...
        try:
            ...
            if target_order:
                ...
                if fk_status == 1:
                    logger.info(
                        "Freekassa payment confirmed via API", order_id=payment.order_id
                    )

                    # Lock payment row before finalization to prevent concurrent double-processing
                    locked = (
                        await freekassa_crud.get_freekassa_payment_by_id_for_update(
                            db, payment.id
                        )
                    )
                    if not locked:
