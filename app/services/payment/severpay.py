...
class SeverPayPaymentMixin:
    ...
    async def process_severpay_webhook(
        ...
        try:
            ...
            if webhook_type != "payin":
                logger.info(
                    "SeverPay webhook: пропускаем тип", webhook_type=webhook_type
                )
                return True
            ...
            if not severpay_id or not severpay_status:
                logger.warning(
                    "SeverPay webhook: отсутствуют обязательные поля", payload=payload
                )
                return False
            ...
            payment = None
            if order_id:
                payment = await severpay_crud.get_severpay_payment_by_order_id(
                    db, order_id
                )
            if not payment and severpay_id:
                payment = await severpay_crud.get_severpay_payment_by_severpay_id(
                    db, severpay_id
                )
            ...
            locked = await severpay_crud.get_severpay_payment_by_id_for_update(
                db, payment.id
            )
            if not locked:
                logger.error(
                    "SeverPay: не удалось заблокировать платёж", payment_id=payment.id
                )
                return False
            payment = locked
            ...
            if payment.is_paid:
                logger.info(
                    "SeverPay webhook: платеж уже обработан", order_id=payment.order_id
                )
                return True
            ...
            if is_paid:
                ...
                await db.flush()
                return await self._finalize_severpay_payment(
                    db, payment, severpay_id=severpay_id, trigger="webhook"
                )
    ...
    async def _finalize_severpay_payment(
        ...
        *,
        ...
        await severpay_crud.link_severpay_payment_to_transaction(
        ...
        if not should_credit_balance:
            logger.info(
                "SeverPay платеж уже зачислил баланс ранее", order_id=payment.order_id
            )
            return True
        ...
        try:
            ...
            logger.error(
                "Ошибка обработки реферального пополнения SeverPay", error=error
            )
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю SeverPay", error=error
                )
        ...
        try:
            ...
            if payment.severpay_id:
                try:
                    ...
                    if severpay_status:
                        status_info = SEVERPAY_STATUS_MAP.get(
                            severpay_status, ("pending", False)
                        )
                        internal_status, is_paid = status_info
                    ...
                    logger.error(
                        "Error checking SeverPay payment status via API", error=e
                    )
