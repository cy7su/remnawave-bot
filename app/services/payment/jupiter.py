    ...
    async def process_jupiter_callback(
        ...
        try:
            ...
            if not our_order_id or not jupiter_status:
                logger.warning(
                    "Jupiter callback: отсутствуют обязательные поля", payload=payload
                )
                return False

            jupiter_crud = import_module("app.database.crud.jupiter")
            payment = await jupiter_crud.get_jupiter_payment_by_order_id(
                db, our_order_id
            )
            if not payment:
                logger.warning(
                    "Jupiter callback: платеж не найден", order_id=our_order_id
                )
                return False

            locked = await jupiter_crud.get_jupiter_payment_by_id_for_update(
                db, payment.id
            )
            if not locked:
                logger.error(
                    "Jupiter: не удалось заблокировать платёж", payment_id=payment.id
                )
                return False
            payment = locked
            ...
            if payment.is_paid:
                logger.info(
                    "Jupiter callback: платеж уже обработан", order_id=payment.order_id
                )
                return True
            ...
            internal_status, is_paid = JUPITER_STATUS_MAP.get(
                jupiter_status, ("pending", False)
            )
            ...
            if is_paid:
                ...
                if received_value is not None:
                    ...
                    if (
                        received_kopeks is not None
                        and abs(received_kopeks - payment.amount_kopeks) > 1
                    ):
                        logger.error(
            ...
            if is_paid:
                ...
                payment.jupiter_transaction_id = (
                    str(jupiter_transaction_id)
                    if jupiter_transaction_id
                    else payment.jupiter_transaction_id
                )
                payment.callback_payload = callback_payload
                payment.updated_at = datetime.now(UTC)
                await db.flush()
                return await self._finalize_jupiter_payment(
    ...
    async def _finalize_jupiter_payment(
        ...
        *,
        ...
        if not should_credit_balance:
            logger.info(
                "Jupiter платеж уже зачислил баланс ранее", order_id=payment.order_id
            )
            return True
        ...
        try:
            ...
            logger.error(
                "Ошибка обработки реферального пополнения Jupiter", error=error
            )
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Jupiter", error=error
                )
