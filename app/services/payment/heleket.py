...
class HeleketPaymentMixin:
    ...
    async def create_heleket_payment(
        ...
        *,
        ...
        if amount_kopeks <= 0:
            logger.error(
                "Сумма Heleket должна быть положительной", amount_kopeks=amount_kopeks
            )
            return None
        ...
        order_id = (
            f'heleket_{user_id or "guest"}_{int(time.time())}_{secrets.token_hex(3)}'
        )
        ...
        if markup_percent:
            try:
                ...
                logger.warning(
                    "Некорректная наценка Heleket", markup_percent=markup_percent
                )
        ...
        status = (
            ...
            or payment_result.get("payment_status")
            or "check"
        )
        ...
        if exchange_rate_value is None and payer_amount:
            try:
                exchange_rate_value = (
                    float(payer_amount) / amount_rubles if amount_rubles else None
                )
        ...
        logger.info(
            "Создан Heleket платеж", uuid=uuid, amount_str=amount_str, user_id=user_id
        )
    ...
    async def _process_heleket_payload(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            logger.error(
                "Heleket webhook payload не является словарём", payload=payload
            )
            return None
        ...
        if exchange_rate is None and payer_amount:
            try:
                exchange_rate = (
                    float(payer_amount) / payment.amount_float
                    if payment.amount_float
                    else None
                )
            except (TypeError, ValueError, ZeroDivisionError):
                exchange_rate = None
        ...
        if getattr(self, "bot", None) and invoice_message and is_final:
            ...
            if chat_id and message_id:
                try:
                    ...
                    logger.warning(
                        "Не удалось удалить счёт Heleket",
        ...
        if invoice_message_removed:
            try:
                ...
                logger.warning(
                    "Не удалось обновить метаданные Heleket после удаления счёта",
                    error=error,
                )

        heleket_lock_crud = import_module("app.database.crud.heleket")
        locked = await heleket_lock_crud.get_heleket_payment_by_id_for_update(
            db, updated_payment.id
        )
        if not locked:
            logger.error(
                "Heleket: не удалось заблокировать платёж",
        ...
        if status_normalized not in {"paid", "paid_over"}:
            logger.info(
                "Heleket платеж в статусе , зачисление не требуется",
                uuid=updated_payment.uuid,
                status=status,
            )
            return updated_payment
        ...
        if amount_kopeks <= 0:
            logger.error(
                "Heleket платеж имеет некорректную сумму",
                uuid=updated_payment.uuid,
        ...
        transaction = await payment_module.create_transaction(
            ...
            description=(
                ...
                if not updated_payment.payer_currency
                    ...
                    f"Пополнение через Heleket ({updated_payment.payer_amount} {updated_payment.payer_currency})"
                )
            ),
        ...
        if not user:
            logger.error(
                "Пользователь не найден для Heleket платежа",
                user_id=updated_payment.user_id,
            )
            return None
        ...
        if getattr(self, "bot", None):
            ...
            if user.telegram_id:
                try:
                    ...
                    logger.error(
                        "Ошибка отправки уведомления пользователю Heleket", error=error
                    )
            else:
                logger.info(
                    "Пропуск Telegram-уведомления Heleket для email-пользователя",
                    user_id=user.id,
                )
        ...
        try:
            ...
            logger.error(
                "Ошибка при работе с автоактивацией для пользователя",
                user_id=user.id,
                error=error,
                exc_info=True,
            )
    ...
    async def sync_heleket_payment_status(
        ...
        *,
        ...
        if not payment:
            logger.error(
                "Heleket платеж с id= не найден", local_payment_id=local_payment_id
            )
            return None
        ...
        try:
            ...
            logger.exception(
                "Ошибка получения статуса Heleket платежа",
                uuid=payment.uuid,
                error=error,
            )
        else:
            if response:
                ...
                if isinstance(result, dict):
                    ...
                    logger.error(
                        "Некорректный ответ Heleket API при проверке платежа",
                        uuid=payment.uuid,
                        response=response,
                    )
        ...
        if payload is None:
            ...
            if not fallback:
                logger.warning(
                    "Heleket API не вернул информацию по платежу", uuid=payment.uuid
                )
                return payment
