...
class EtoplatezhiPaymentMixin:
    ...
    async def process_etoplatezhi_callback(
        ...
        try:
            ...
            our_payment_id = (
                str(etoplatezhi_payment_id) if etoplatezhi_payment_id else None
            )
            ...
            etoplatezhi_crud = import_module("app.database.crud.etoplatezhi")
            payment = await etoplatezhi_crud.get_etoplatezhi_payment_by_order_id(
                db, our_payment_id
            )
            ...
            locked = await etoplatezhi_crud.get_etoplatezhi_payment_by_id_for_update(
                db, payment.id
            )
            if not locked:
                logger.error(
            ...
            # Маппинг статуса
            status_info = ETOPLATEZHI_STATUS_MAP.get(
                etoplatezhi_status, ("pending", False)
            )
            ...
            if is_paid:
                ...
                payment.etoplatezhi_payment_id = (
                    str(etoplatezhi_payment_id) if etoplatezhi_payment_id else None
                )
    ...
    async def _finalize_etoplatezhi_payment(
        ...
        *,
        ...
        if not user:
            logger.error(
                "Пользователь не найден для Etoplatezhi", user_id=payment.user_id
            )
            return False
        ...
        try:
            ...
            logger.error(
                "Ошибка обработки реферального пополнения Etoplatezhi", error=error
            )
        ...
        if getattr(self, "bot", None):
            try:
                ...
                logger.error(
                    "Ошибка отправки админ уведомления Etoplatezhi", error=error
                )
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Etoplatezhi", error=error
                )
