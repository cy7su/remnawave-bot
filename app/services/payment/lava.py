...
class LavaPaymentMixin:
    ...
    async def create_lava_payment(
        ...
        *,
        ...
        try:
            ...
            data = (
                (api_result.get("data") or api_result)
    ...
    async def process_lava_callback(
        ...
        try:
            ...
            if not payment:
                ...
                if lava_invoice_id:
                    payment = await lava_crud.get_lava_payment_by_invoice_id(
                        db, str(lava_invoice_id)
                    )
                ...
                if not payment:
                    logger.warning(
                        "Lava webhook: платеж не найден", order_id=our_order_id
                    )
                    return False
            ...
            if not locked:
                logger.error(
                    "Lava: не удалось заблокировать платёж", payment_id=payment.id
                )
                return False
            payment = locked
            ...
            if payment.is_paid:
                logger.info(
                    "Lava webhook: платеж уже обработан", order_id=payment.order_id
                )
                return True
            ...
            internal_status, is_paid = LAVA_STATUS_MAP.get(
                lava_status, ("pending", False)
            )
    ...
    async def _finalize_lava_payment(
        ...
        *,
        ...
        if not should_credit_balance:
            logger.info(
                "Lava платеж уже зачислил баланс ранее", order_id=payment.order_id
            )
            return True
        ...
        if getattr(self, "bot", None) and user.telegram_id:
            try:
                ...
                logger.error(
                    "Ошибка отправки уведомления пользователю Lava", error=error
                )
    ...
    async def check_lava_payment_status(
        ...
        try:
            return await lava_service.get_invoice_status(
