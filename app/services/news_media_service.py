...
def detect_file_type(data: bytes) -> tuple[MediaType, str]:
    ...
    if data[4:8] == b"ftyp":
        ...
        if brand in _MP4_VIDEO_BRANDS:
            return "video", ".mp4"
        logger.warning(
            "Unknown ftyp brand rejected", brand=brand.decode("ascii", errors="replace")
        )
...
def _process_and_save_image(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Failed to generate thumbnail", filename=filename, exc_info=True
            )
            thumbnail_filename = None

        relative_path = f"{_IMAGES_DIR}/{filename}"
        thumbnail_path = (
            f"{_THUMBNAILS_DIR}/{thumbnail_filename}" if thumbnail_filename else None
        )
...
async def save_image(
    ...
    return await asyncio.to_thread(
        _process_and_save_image, data, upload_path, max_dim, quality
    )
