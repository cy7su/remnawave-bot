...
class FreekassaService:
    ...
    def _generate_api_signature_hmac(self, params: dict[str, Any]) -> str:
        ...
        # HMAC-SHA256
        return hmac.new(
            self.api_key.encode("utf-8"), msg.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    def generate_form_signature(
        ...
        sign_string = (
            f"{self.shop_id}:{final_amount}:{self.secret1}:{currency}:{order_id}"
        )
    ...
    def build_payment_url(
        ...
        if ps_id == 44:
            try:
                ...
                logger.info(
                    "Freekassa synchronous build_payment_url for 44", params=params
                )
