...
def _transaction_fields(txn_info: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "environment": str(
            txn_info.get("environment") or settings.get_apple_iap_environment()
        ),
...
class AppleIAPFulfillmentService:
    ...
    async def fulfill_verified_transaction(
        ...
        *,
        ...
        validation_error = self.apple_service.validate_transaction_info(
            txn_info, product_id
        )
        if validation_error:
            await create_apple_abuse_event(
        ...
        if web_order_line_item_id:
            existing = await get_apple_transaction_by_web_order_line_item_id(
                db, web_order_line_item_id
            )
            if existing:
                if existing.user_id == user_id and existing.status in {
                    "verified",
        ...
        try:
            ...
            if web_order_line_item_id:
                ...
                if existing:
                    if existing.user_id == user_id:
                        await db.commit()
                        return AppleFulfillmentResult(
                            True, "already_processed", existing
                        )
                    await create_apple_abuse_event(
                        db,
                        "web_order_line_item_owner_mismatch",
    ...
    async def _record_sandbox_on_production(
        ...
        *,
        ...
        try:
            ...
            apple_txn = await get_apple_transaction_by_transaction_id(
                db, fields["transaction_id"]
            )
...
class AppleIAPNotificationService:
    def __init__(
        ...
        self.apple_service = apple_service or AppleIAPService()
        self.fulfillment_service = fulfillment_service or AppleIAPFulfillmentService(
            self.apple_service, bot=bot
        )

    async def process_signed_payload(
        self, signed_payload: str, raw_body: bytes
    ) -> tuple[bool, str]:
        try:
        ...
        if not self._environment_allowed(environment):
            logger.warning(
                "Apple notification environment ignored", environment=environment
            )
            return True, "environment_ignored"

        signed_txn = data.get("signedTransactionInfo")
        txn_info = (
            self.apple_service.verify_signed_transaction_info(signed_txn, environment)
            if signed_txn
            else None
        )
        ...
        async with AsyncSessionLocal() as db:
            ...
            if apple_notification is None:
                existing_payload = await get_apple_notification_by_payload_hash(
                    db, payload_hash
                )
                if existing_payload:
                    logger.warning(
                ...
                try:
                    async with db.begin_nested():
                        apple_notification = await create_apple_notification(
                            ...
                            transaction_id=str(
                                (txn_info or {}).get("transactionId") or ""
                            )
                            or None,
                            original_transaction_id=str(
                                (txn_info or {}).get("originalTransactionId") or ""
                            )
                            or None,
                            ...
                            metadata_json={
                                ...
                                "subtype": subtype,
                                "signedPayloadHash": notification.get(
                                    "signedPayloadHash"
                                ),
                            },
                        )
                except IntegrityError:
                    apple_notification = await get_apple_notification_by_uuid(
                        db, notification_uuid
                    )
            ...
            try:
                reason = await self._dispatch(
                    db, notification_type, txn_info, apple_notification
                )
            except Exception as error:
                logger.error(
                    "Apple notification processing failed", error=error, exc_info=True
                )
                await mark_apple_notification_processed(
                    db, apple_notification, status="failed", error=str(error)[:500]
                )
                await db.commit()
            ...
            if reason in _RETRYABLE_NOTIFICATION_REASONS:
                await mark_apple_notification_processed(
                    db, apple_notification, status="failed", error=reason
                )
                await db.commit()
                return False, reason

            await mark_apple_notification_processed(
                db, apple_notification, status="processed"
            )
            await db.commit()
            return True, reason
    ...
    async def _validate_stored_transaction_matches_notification(
        ...
        *,
        ...
        signed_original_transaction_id = str(
            txn_info.get("originalTransactionId") or ""
        )
        stored_transaction_ids = {
            value
            for value in (apple_txn.transaction_id, apple_txn.original_transaction_id)
            if value
        }
        signed_transaction_ids = {
            value
            for value in (signed_transaction_id, signed_original_transaction_id)
            if value
        }
        if not stored_transaction_ids.intersection(signed_transaction_ids):
            await create_apple_abuse_event(
                ...
                details_json={
                    ...
                    "signed_original_transaction_id": signed_original_transaction_id
                    or None,
                },
            )
        ...
        if (
            not apple_txn.app_account_token
            or signed_app_account_token != apple_txn.app_account_token
        ):
            await create_apple_abuse_event(
                db,
    ...
    async def _handle_one_time_charge(
        self, db: AsyncSession, txn_info: dict[str, Any] | None
    ) -> str:
        if not txn_info:
    ...
    async def _handle_refund(
        ...
        apple_txn = await get_apple_transaction_by_transaction_id_for_update(
            db, lookup_id
        )
        ...
        if (
            apple_txn.environment == "Sandbox"
            and settings.get_apple_iap_environment() == "Production"
        ):
