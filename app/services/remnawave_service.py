...
class RemnaWaveService:
    ...
    def _ensure_user_remnawave_uuid(
        ...
        if conflicting_user and conflicting_user is not user:
            logger.warning(
                "Обнаружен конфликт UUID между пользователями и . Сбрасываем у старой записи.",
    ...
    def _parse_remnawave_date(self, date_str: str) -> datetime:
        ...
        try:
            ...
            logger.warning(
                "Не удалось распарсить дату . Используем дефолтную дату.",
                date_str=date_str,
                error=e,
            )
            return self._now_utc() + timedelta(days=30)
    ...
    def _safe_expire_at_for_panel(self, expire_at: datetime | None) -> datetime:
        ...
        if not expire_at:
            ...
            if normalized_expire < minimum_expire:
                logger.debug(
                    "Коррекция даты истечения до минимально допустимой для панели",
    ...
    def _is_preferred_panel_user(
        ...
        *,
        ...
        if (
            candidate_status in active_statuses
    ...
    def _extract_user_data_from_description(
    ...
    async def _get_or_create_bot_user_from_panel(
        ...
        first_name_from_desc, last_name_from_desc, username_from_desc = (
            self._extract_user_data_from_description(description)
        )
        ...
        try:
            ...
            logger.info(
                "Пользователь с telegram_id уже существует. Используем существующую запись.",
                telegram_id=telegram_id,
            )
            ...
            try:
                ...
                if existing_user is None:
                    logger.error(
                        "Не удалось найти существующего пользователя с telegram_id",
                        telegram_id=telegram_id,
                    )
                    return None, False
                ...
                logger.error(
                    "Ошибка загрузки существующего пользователя",
                    telegram_id=telegram_id,
            ...
            logger.error(
                "Общая ошибка создания/загрузки пользователя",
                telegram_id=telegram_id,
        ...
        try:
            async with self.get_api_client() as api:
                ...
                try:
                    ...
                    nodes_online = sum(
                        1 for n in all_nodes if n.is_connected and not n.is_disabled
                    )
                    total_nodes = len(all_nodes)
                except Exception as e:
                    logger.warning(
                        "Не удалось получить список нод для подсчёта онлайн", error=e
                    )
                    nodes_online = 0
                    total_nodes = 0

                total_download = sum(
                    node.get("downloadBytes", 0) for node in realtime_usage
                )
                total_upload = sum(
                    node.get("uploadBytes", 0) for node in realtime_usage
                )
                total_realtime_traffic = total_download + total_upload

                total_user_traffic = int(
                    system_stats.get("users", {}).get("totalTrafficBytes", "0")
                )
                ...
                if nodes_stats.get("lastSevenDays"):
                    ...
                    for day_data in nodes_stats["lastSevenDays"]:
                        ...
                        nodes_by_name[node_name]["days_data"].append(
                            {"date": day_data["date"], "bytes": daily_bytes}
                        )
                ...
                try:
                    uptime_seconds = (
                        int(float(uptime_value)) if uptime_value is not None else 0
                    )
                except (TypeError, ValueError):
                    logger.warning(
                        "Не удалось преобразовать uptime в число, используем 0",
                        uptime_value=uptime_value,
                    )

                result = {
                    "system": {
                        "users_online": system_stats.get("onlineStats", {}).get(
                            "onlineNow", 0
                        ),
                        "total_users": system_stats.get("users", {}).get(
                            "totalUsers", 0
                        ),
                        "active_connections": system_stats.get("onlineStats", {}).get(
                            "onlineNow", 0
                        ),
                        "nodes_online": nodes_online,
                        "total_nodes": total_nodes,
                        "users_last_day": system_stats.get("onlineStats", {}).get(
                            "lastDay", 0
                        ),
                        "users_last_week": system_stats.get("onlineStats", {}).get(
                            "lastWeek", 0
                        ),
                        "users_never_online": system_stats.get("onlineStats", {}).get(
                            "neverOnline", 0
                        ),
                        "total_user_traffic": total_user_traffic,
                    },
                    "users_by_status": system_stats.get("users", {}).get(
                        "statusCounts", {}
                    ),
                    "server_info": {
                    ...
                    "traffic_periods": {
                        "last_2_days": {
                            "current": self._parse_bandwidth_string(
                                bandwidth_stats.get("bandwidthLastTwoDays", {}).get(
                                    "current", "0 B"
                                )
                            ),
                            ...
                            "difference": bandwidth_stats.get(
                                "bandwidthLastTwoDays", {}
                            ).get("difference", "0 B"),
                        },
                        "last_7_days": {
                            "current": self._parse_bandwidth_string(
                                bandwidth_stats.get("bandwidthLastSevenDays", {}).get(
                                    "current", "0 B"
                                )
                            ),
                            "previous": self._parse_bandwidth_string(
                        ...
                        "last_30_days": {
                            "current": self._parse_bandwidth_string(
                                bandwidth_stats.get("bandwidthLast30Days", {}).get(
                                    "current", "0 B"
                                )
                            ),
                            "previous": self._parse_bandwidth_string(
                        ...
                        "current_month": {
                            "current": self._parse_bandwidth_string(
                                bandwidth_stats.get("bandwidthCalendarMonth", {}).get(
                                    "current", "0 B"
                                )
                            ),
                            "previous": self._parse_bandwidth_string(
                        ...
                        "current_year": {
                            "current": self._parse_bandwidth_string(
                                bandwidth_stats.get("bandwidthCurrentYear", {}).get(
                                    "current", "0 B"
                                )
                            ),
                            "previous": self._parse_bandwidth_string(
        ...
        try:
            ...
            return {
                ...
                "total": {
                    ...
                    "traffic_bytes": self._parse_bandwidth_string(
                        total.get("traffic", "0 B")
                    ),
                    "nodes_ram_bytes": self._parse_bandwidth_string(
                        total.get("nodesRam", "0 B")
                    ),
        ...
        try:
            ...
            if not by_app_raw:
                ...
                for p in by_platform:
                    ...
                    if not isinstance(
                        nested, list
                    ):  # malformed/absent — skip, don't blow up the whole payload
                        continue
                    for a in nested:
                        name = a.get("app") or "Unknown"
                        app_counts[name] = app_counts.get(name, 0) + (
                            a.get("count") or 0
                        )
                by_app_raw = [
                    {"app": name, "count": count} for name, count in app_counts.items()
                ]
            ...
            return {
                "top_users": [
                    {
                        ...
                        "devices_count": u.get("devicesCount", 0),
                    }
                ...
                "by_app": [
                    {"app": a.get("app") or "Unknown", "count": a.get("count") or 0}
                    for a in by_app_raw
                ],
            ...
            async with self.get_api_client() as api:
                ...
                for node in nodes:
                    try:
                        ...
                        logger.warning(
                            "Не удалось получить топ юзеров ноды",
                            node=node.uuid,
                            error=node_err,
                        )
                        continue
                    for u in data.get("topUsers", []):
                        ...
                        if not username:
                            continue
                        totals[username] = totals.get(username, 0) + int(
                            u.get("total", 0) or 0
                        )
    ...
    async def get_health_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            latest = (
                max(metrics, key=lambda m: m.get("timestamp", 0)) if metrics else {}
            )
            return {
    ...
    def _parse_bandwidth_string(self, bandwidth_str: str | float) -> int:
        try:
            ...
            logger.warning(
                "Не удалось распарсить строку трафика", bandwidth_str=bandwidth_str
            )
            return 0

        except Exception as e:
            logger.error(
                "Ошибка парсинга строки трафика", bandwidth_str=bandwidth_str, error=e
            )
            return 0
    ...
    async def get_node_details(self, node_uuid: str) -> dict[str, Any] | None:
        try:
            ...
            logger.error(
                "Ошибка получения информации о ноде", node_uuid=node_uuid, error=e
            )
            return None
    ...
    async def manage_node(self, node_uuid: str, action: str) -> bool:
        try:
            async with self.get_api_client() as api:
                ...
                logger.info(
                    "Действие выполнено для ноды", action=action, node_uuid=node_uuid
                )
                return True
    ...
    async def update_squad_inbounds(
    ...
    async def update_squad(
        self, uuid: str, name: str = None, inbounds: list[str] = None
    ) -> bool:
        try:
            async with self.get_api_client() as api:
    ...
    async def migrate_squad_users(
        ...
        if not subscriptions:
            logger.info(
                "Переезд сквада → : подходящих подписок не найдено",
        ...
        try:
            ...
            for subscription in subscriptions:
                ...
                _uuid = (
                    ...
                    if settings.is_multi_tariff_enabled()
                        ...
                        subscription.user.remnawave_uuid if subscription.user else None
                    )
                )
                if _uuid:
                    if api is None:
            ...
            if updated_subscriptions:
                ...
                counter_updates = {}
                if source_decrement:
                    counter_updates[source_server.id] = func.greatest(
                        ServerSquad.current_users - source_decrement, 0
                    )
                if target_increment:
                    counter_updates[target_server.id] = (
                        ServerSquad.current_users + target_increment
                    )
                for sid in sorted(counter_updates):
                    await db.execute(
                        update(ServerSquad)
                        .where(ServerSquad.id == sid)
                        .values(current_users=counter_updates[sid])
                    )
    ...
    async def sync_users_from_panel(
        self, db: AsyncSession, sync_type: str = "all"
    ) -> dict[str, int]:
        ...
        try:
            ...
            async with self.get_api_client() as api:
                ...
                logger.info(
                    "Всего загружено пользователей из панели",
            ...
            if email_users_count > 0:
                logger.info(
                    "Email-only пользователей (без telegram_id)",
            ...
            logger.info(
                "Пользователей в панели с Telegram ID",
                panel_users_with_tg_count=len(panel_users_with_tg),
            )

            unique_panel_users_map = self._deduplicate_panel_users_by_telegram_id(
                panel_users_with_tg
            )
            ...
            if duplicates_count:
                logger.info(
                    "Обнаружено дубликатов пользователей по Telegram ID. Используем самые свежие записи.",
            ...
            for i, panel_user in enumerate(unique_panel_users):
                ...
                try:
                    ...
                    if not db_user:
                        if sync_type in ["new_only", "all"]:
                            logger.info(
                                "Создание пользователя для telegram_id",
                                telegram_id=telegram_id,
                            )

                            db_user, is_created = (
                                await self._get_or_create_bot_user_from_panel(
                                    db, panel_user
                                )
                            )
                            ...
                            if is_created:
                                await self._create_subscription_from_panel_data(
                                    db, db_user, panel_user
                                )
                                stats["created"] += 1
                                logger.info(
                                    "Создан пользователь с подпиской",
                                    telegram_id=telegram_id,
                                )
                            else:
                                ...
                                stats["updated"] += 1
                                logger.info(
                                    "Обновлена подписка существующего пользователя",
                                    telegram_id=telegram_id,
                                )
                        ...
                        if settings.is_multi_tariff_enabled():
                            from app.database.crud.subscription import (
                                get_active_subscriptions_by_user_id as _get_subs,
                            )
                            ...
                            existing_sub = next(
                                (
                                    s
                                    for s in _subs
                                    if s.remnawave_uuid == panel_user.get("uuid")
                                ),
                                None,
                            )
                            ...
                            from app.database.crud.subscription import (
                                get_subscription_by_user_id as _get_sub,
                            )
                    ...
                    logger.warning(
                        "Сессия повреждена после rollback, прерываем обработку",
                        i=i + 1,
                        unique_panel_users_count=len(unique_panel_users),
                    )
                    break
                ...
                if (i + 1) % batch_size == 0:
                    try:
                        await db.commit()
                        logger.debug(
                            "Коммит изменений после обработки пользователей", i=i + 1
                        )
                        pending_uuid_mutations.clear()
                    except Exception as commit_error:
                        logger.error(
                            "Ошибка коммита после обработки пользователей",
                            i=i + 1,
            ...
            try:
                ...
                logger.error(
                    "Ошибка финального коммита", final_commit_error=final_commit_error
                )
                await db.rollback()
                for mutation in reversed(pending_uuid_mutations):
                    mutation.rollback()
            ...
            if panel_users_email_only and sync_type in ["new_only", "all"]:
                        ...
                        if db_user:
                            ...
                            if settings.is_multi_tariff_enabled():
                                ...
                                if not existing_sub and _subs_e:
                                    ...
                                    existing_sub = max(
                                        _pool_e, key=lambda s: s.days_left
                                    )
                            else:
                                from app.database.crud.subscription import (
                                    get_subscription_by_user_id as _get_sub_email,
                                )
                            ...
                            stats["updated"] += 1
                            logger.info(
                                "Обновлен email-пользователь", panel_email=panel_email
                            )
                        else:
                            # Email-only пользователи не создаются автоматически при синхронизации,
                            # они должны сначала зарегистрироваться через cabinet
                            logger.debug(
                                "Email-пользователь не найден в боте, пропускаем",
                                panel_email=panel_email,
                            )

                    except Exception as email_user_error:
                        logger.error(
                            "Ошибка обработки email-пользователя",
                            email_user_error=email_user_error,
                        )
                        stats["errors"] += 1
                ...
                try:
                    ...
                    logger.error(
                        "Ошибка коммита email-пользователей",
                        email_commit_error=email_commit_error,
                    )
                    await db.rollback()
            ...
            if sync_type == "all":
                ...
                users_to_deactivate = [
                    ...
                    and any(
                        True for _ in (getattr(db_user, "subscriptions", None) or [])
                    )
                ...
                if users_to_deactivate:
                    logger.info(
                        "Найдено пользователей для деактивации",
                        users_to_deactivate_count=len(users_to_deactivate),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "Не удалось создать API клиент для сброса HWID",
                ...
                try:
                    for telegram_id, db_user in users_to_deactivate:
                        ...
                        try:
                            user_subscriptions = (
                                getattr(db_user, "subscriptions", None) or []
                            )
                            ...
                            all_recently_updated = all(
                                is_recently_updated_by_webhook(subscription)
                            ...
                            logger.info(
                                "Деактивация подписок пользователя (нет в панели)",
                                telegram_id=telegram_id,
                            )
                            ...
                            for subscription in user_subscriptions:
                                ...
                                if was_paid:
                                    ...
                                    logger.warning(
                                        "ПЛАТНАЯ подписка пользователя отключена (нет в панели), но is_trial= и end_date= СОХРАНЕНЫ",
                                        telegram_id=telegram_id,
                                        subscription_id=subscription.id,
                            ...
                            cleanup_mutation.set_user_updated_at(
                                db_user, datetime.now(UTC)
                            )
                    ...
                    try:
                        ...
                        logger.error(
                            "Ошибка финального коммита при деактивации",
                            final_commit_error=final_commit_error,
                        )
                        await db.rollback()
                        for mutation in reversed(cleanup_uuid_mutations):
                            mutation.rollback()
    ...
    async def _sync_users_from_panel_multi(
        self, db: AsyncSession, sync_type: str
    ) -> dict[str, int]:
        ...
        try:
            logger.info(
                "[multi-tariff] Начинаем синхронизацию типа", sync_type=sync_type
            )
            ...
            logger.info(
                "[multi-tariff] Загружено из панели", panel_users_count=len(panel_users)
            )
            ...
            subs_result = await db.execute(
                select(Subscription)
                .options(
                    selectinload(Subscription.user), selectinload(Subscription.tariff)
                )
            ...
            bot_users_by_email = {
                u.email.lower(): u for u in _all_users if u.email and u.email_verified
            }
            ...
            for panel_user in panel_users:
                ...
                if not subscription:
                    ...
                    if legacy_user:
                        ...
                        active = [
                            s for s in user_subs if s.status in ("active", "trial")
                        ]
                        if active:
                            non_daily = [
                                s
                ...
                if not subscription:
                    ...
                    if not _bot_user:
                        logger.debug(
                            "[multi-tariff] Panel user has no matching bot user",
                            panel_uuid=panel_uuid,
                    ...
                    _active_count = sum(
                        1 for s in _user_subs if s.status in ("active", "trial")
                    )
                    if _active_count >= settings.get_max_active_subscriptions():
                        logger.debug(
                            "[multi-tariff] User at max subscriptions, skipping",
                    ...
                    try:
                        ...
                        _expire_at = self._parse_remnawave_date(
                        ...
                        _traffic_limit_bytes = int(
                            panel_user.get("trafficLimitBytes") or 0
                        )  # 2.8.0: number→int
                        ...
                        if _squad_uuids:
                            try:
                                from app.database.crud.tariff import (
                                    get_all_active_tariffs,
                                )

                                _all_tariffs = await get_all_active_tariffs(db)
                                for _t in _all_tariffs:
                                    if _t.allowed_squads and set(_squad_uuids).issubset(
                                        set(_t.allowed_squads)
                                    ):
                        ...
                        new_sub = Subscription(
                            ...
                            end_date=_expire_at,
                            traffic_limit_gb=(
                                _traffic_limit_bytes // (1024**3)
                                if _traffic_limit_bytes > 0
                                else 0
                            ),
                            traffic_used_gb=_used_bytes / (1024**3),
                            device_limit=coerce_panel_device_limit(
                                panel_user.get("hwidDeviceLimit")
                            ),
                            ...
                            subscription_crypto_link=panel_user.get(
                        ...
                        if hasattr(_bot_user, "subscriptions") and isinstance(
                            _bot_user.subscriptions, list
                        ):
                ...
                try:
                    ...
                    if (
                        crypto_link
                        and subscription.subscription_crypto_link != crypto_link
                    ):
                    ...
                    if isinstance(_panel_squads, list):
                        for _sq in _panel_squads:
                            if isinstance(_sq, dict) and "uuid" in _sq:
                                ...
                                _squad_uuids.append(_sq)
                    if _squad_uuids and set(_squad_uuids) != set(
    ...
    async def _create_subscription_from_panel_data(
        self, db: AsyncSession, user, panel_user
    ):
        try:
            ...
            traffic_limit_gb = (
                traffic_limit_bytes // (1024**3) if traffic_limit_bytes > 0 else 0
            )
            ...
            logger.info(
                "Подготовлена подписка для пользователя",
            ...
            logger.error(
                "Ошибка создания подписки для пользователя",
                telegram_id=user.telegram_id,
                error=e,
            )
            try:
                ...
                logger.info(
                    "Подготовлена базовая подписка для пользователя",
                    telegram_id=user.telegram_id,
                )
            except Exception as basic_error:
                logger.error(
                    "Ошибка создания базовой подписки", basic_error=basic_error
                )
    ...
    async def _update_subscription_from_panel_data(
        ...
        try:
            ...
            if settings.is_multi_tariff_enabled():
                from app.database.crud.subscription import (
                    get_active_subscriptions_by_user_id as _get_subs_upd,
                )
            ...
            if is_recently_updated_by_webhook(subscription):
                logger.debug(
                    "Пропуск синхронизации подписки : обновлена вебхуком недавно",
                    subscription_id=subscription.id,
                )
                return
            ...
            if expire_at_str:
                ...
                if panel_status == "ACTIVE":
                    ...
                    if time_diff > 60:
                        ...
                        new_end_date_local = expire_at.replace(
                        ...
                        logger.debug(
                            "Sync: пропускаем обновление end_date — разница слишком мала (< 60с)",
                            value=getattr(user, "telegram_id", "?"),
                            time_diff=round(time_diff, 0),
                        )
                else:
                    logger.debug(
                        "Sync: пропускаем обновление end_date — статус в панели не ACTIVE",
                        value=getattr(user, "telegram_id", "?"),
            ...
            if panel_status == "ACTIVE" and end_date_utc > current_time:
                ...
                if subscription.status == SubscriptionStatus.ACTIVE.value:
                    logger.warning(
                        "Sync: пропускаем деактивацию подписки (статус в панели ACTIVE). Деактивация будет выполнена через middleware с буфером.",
                        value=getattr(user, "telegram_id", "?"),
                        end_date=subscription.end_date,
            ...
            if abs(subscription.traffic_used_gb - traffic_used_gb) > 0.01:
                subscription.traffic_used_gb = traffic_used_gb
                logger.debug(
                    "Обновлен использованный трафик", traffic_used_gb=traffic_used_gb
                )
            ...
            if panel_squad_uuids and set(panel_squad_uuids) != set(
                subscription.connected_squads or []
            ):
            ...
            panel_crypto_link = panel_user.get("subscriptionCryptoLink") or (
                panel_user.get("happ") or {}
            ).get("cryptoLink", "")
            if (
                panel_crypto_link
                and subscription.subscription_crypto_link != panel_crypto_link
            ):
            ...
            logger.debug(
                "Обновлена подписка для пользователя", telegram_id=user.telegram_id
            )

        except Exception as e:
            logger.error(
                "Ошибка обновления подписки для пользователя",
                telegram_id=user.telegram_id,
                error=e,
            )
            ...
            async with self.get_api_client() as api:
                ...
                while True:
                    ...
                    subscriptions = await get_subscriptions_batch(
                        db, offset=offset, limit=batch_size
                    )
                            ...
                            try:
                                ...
                                username_suffix = (
                                    ...
                                    if (
                                        ...
                                        and sub.remnawave_short_id
                                    )
                                    else ""
                                )
                                username = (
                                    settings.build_remnawave_subscription_username(
                                ...
                                if sub.tariff and sub.tariff.external_squad_uuid:
                                    create_kwargs["external_squad_uuid"] = (
                                        sub.tariff.external_squad_uuid
                                    )
                                ...
                                if not panel_uuid and user.telegram_id:
                                    existing_users = await api.get_user_by_telegram_id(
                                        user.telegram_id
                                    )
                                    if existing_users:
                                        if settings.is_multi_tariff_enabled():
                                ...
                                if not panel_uuid and user.email:
                                    existing_users = await api.get_user_by_email(
                                        user.email
                                    )
                                    if existing_users:
                                        if settings.is_multi_tariff_enabled():
                                ...
                                if panel_uuid:
                                    update_kwargs = dict(
                                        ...
                                        status=status,
                                        expire_at=expire_at,
                                        traffic_limit_bytes=create_kwargs[
                                            "traffic_limit_bytes"
                                        ],
                                        traffic_limit_strategy=get_traffic_reset_strategy(
                                            sub.tariff
                                    ...
                                    # Не отправляем null — RemnaWave API не принимает null для externalSquadUuid (A039)
                                    if sub.tariff and sub.tariff.external_squad_uuid:
                                        update_kwargs["external_squad_uuid"] = (
                                            sub.tariff.external_squad_uuid
                                        )
                                    ...
                                    try:
                                        ...
                                        error_code = (
                                            api_error.response_data or {}
                                        ).get("errorCode", "")
                                        if (
                                            api_error.status_code == 404
                                            or error_code in ("A018", "A063")
                                        ):
                    ...
                    try:
                        ...
                        logger.error(
                            "Ошибка фиксации транзакции при синхронизации в панель",
                            commit_error=commit_error,
                        )
                        await db.rollback()
        ...
        try:
            async with self.get_api_client() as api:
                ...
                return {
                    ...
                    "lifetime_used_traffic_gb": user.lifetime_used_traffic_bytes
                    / (1024**3),
            ...
            logger.error(
                "Ошибка получения статистики трафика для пользователя",
                telegram_id=telegram_id,
                error=e,
            )
            return None
    ...
    async def get_user_traffic_stats_by_uuid(
        ...
        try:
            ...
            logger.error(
                "Ошибка получения статистики трафика по UUID",
    ...
    async def get_telegram_id_by_email(self, user_identifier: str) -> int | None:
        ...
        try:
            async with self.get_api_client() as api:
                ...
                try:
                    ...
                    while True:
                        ...
                        page_response = await api.get_all_users_page_stream(
                            cursor=cursor, size=page_size
                        )
                        ...
                        for panel_user in users_list:
                            panel_email = (
                                panel_user.email
                                if hasattr(panel_user, "email")
                                else None
                            )
                            if (
                                panel_email
                                and panel_email.lower() == user_identifier.lower()
                            ):
                                panel_telegram_id = (
                                    panel_user.telegram_id
                                    if hasattr(panel_user, "telegram_id")
                                    else None
                                )
                        ...
                        if not page_response.get("hasMore") or not page_response.get(
                            "nextCursor"
                        ):
                            break
                ...
                logger.warning(
                    "Пользователь с идентификатором не найден в панели",
                    user_identifier=user_identifier,
                )
                return None
            ...
            logger.error(
                "Ошибка получения telegram_id по идентификатору",
                user_identifier=user_identifier,
                error=e,
            )
            return None
        ...
        try:
            async with self.get_api_client() as api:
                response = await api._make_request(
                    "POST", f"/api/internal-squads/{squad_uuid}/bulk-actions/add-users"
                )
    ...
    async def get_node_user_usage_by_range(
        self, node_uuid: str, start_date, end_date
    ) -> list[dict[str, Any]]:
        try:
            async with self.get_api_client() as api:
            ...
            logger.error(
                "Ошибка получения статистики использования ноды",
                node_uuid=node_uuid,
                error=e,
            )
            return []
    ...
    async def get_node_statistics(self, node_uuid: str) -> dict[str, Any] | None:
        try:
            ...
            logger.error(
                "Ошибка получения статистики ноды", node_uuid=node_uuid, error=e
            )
    ...
    async def validate_user_data_before_sync(self, panel_user) -> bool:
        try:
            ...
            if not panel_user.uuid:
                logger.debug(
                    "Нет UUID для пользователя", telegram_id=panel_user.telegram_id
                )
                return False

            if panel_user.telegram_id <= 0:
                logger.debug(
                    "Некорректный telegram_id", telegram_id=panel_user.telegram_id
                )
                return False
    ...
    async def force_cleanup_user_data(self, db: AsyncSession, user: User) -> bool:
        ...
        try:
            ...
            if was_paid:
                logger.warning(
                    "ВНИМАНИЕ: force_cleanup_user_data вызвана для ПЛАТНОГО пользователя",
                    user_id_display=user_id_display,
                    has_had_paid_subscription=user.has_had_paid_subscription,
                    balance_kopeks=user.balance_kopeks,
                    is_trial=(
                        [sub.is_trial for sub in user_subscriptions]
            ...
            logger.info(
                "ПРИНУДИТЕЛЬНАЯ полная очистка данных пользователя",
                user_id_display=user_id_display,
            )
            ...
            for _uuid in _uuids_to_reset:
                try:
                    ...
                    logger.warning(
                        "Failed to reset devices for UUID", uuid=_uuid, error=e
                    )
            ...
            try:
                ...
                if user_subscriptions:
                    logger.info(
                        "Удалены серверы подписок пользователя",
                        user_id_display=user_id_display,
                    )
                ...
                logger.info(
                    "Удалены транзакции пользователя", user_id_display=user_id_display
                )
                ...
                logger.info(
                    "Удалены реферальные доходы пользователя",
                    user_id_display=user_id_display,
                )
                ...
                logger.error(
                    "Ошибка удаления связанных записей", records_error=records_error
                )

            try:
                if user.balance_kopeks > 0:
                    logger.warning(
                        "force_cleanup: СОХРАНЯЕМ баланс пользователя (оплаченные средства)",
                        user_id_display=user_id_display,
                        balance_kopeks=user.balance_kopeks,
                    )
                ...
                logger.info(
                    "ПРИНУДИТЕЛЬНО очищены ВСЕ данные пользователя",
                    user_id_display=user_id_display,
                )
                return True
                ...
                logger.error(
                    "Ошибка финальной очистки пользователя", cleanup_error=cleanup_error
                )
                await db.rollback()
                return False
            ...
            logger.error(
                "Критическая ошибка принудительной очистки пользователя",
                telegram_id=user.telegram_id,
                error=e,
            )
            await db.rollback()
    ...
    async def cleanup_orphaned_subscriptions(self, db: AsyncSession) -> dict[str, int]:
        try:
            ...
            while True:
                subscriptions, total_count = await get_all_subscriptions(
                    db, page, limit
                )
                ...
                for subscription in subscriptions:
                    try:
                        ...
                        if user.telegram_id not in panel_telegram_ids:
                            logger.info(
                                "ПОЛНАЯ деактивация подписки пользователя (отсутствует в панели)",
                                telegram_id=user.telegram_id,
                            )

                            cleanup_success = await self.force_cleanup_user_data(
                                db, user
                            )
            ...
            while True:
                ...
                for subscription in subscriptions:
                    try:
                        ...
                        if not user.telegram_id:
                            logger.debug(
                                "Пропускаем email-пользователя при синхронизации с панелью",
                                user_id=user.id,
                            )
                            continue
                        ...
                        if panel_user:
                            ...
                            if is_recently_updated_by_webhook(subscription):
                                ...
                                logger.info(
                                    "Деактивируем подписку пользователя (нет в панели)",
                                    telegram_id=user.telegram_id,
                                )
                ...
                for subscription in subscriptions:
                    try:
                            ...
                            try:
                                async with self.get_api_client() as api:
                                    ...
                                    if rw_user:
                                        ...
                                        subscription.subscription_url = (
                                            rw_user.subscription_url
                                        )
                                        subscription.subscription_crypto_link = (
                                            rw_user.happ_crypto_link
                                        )
                                        logger.info(
                                ...
                                logger.warning(
                                    "Не удалось получить данные Remnawave для пользователя",
                                    telegram_id=user.telegram_id,
                                    rw_error=rw_error,
                                )
                        ...
                        if subscription.traffic_limit_gb < 0:
                            subscription.traffic_limit_gb = 0
                            logger.info(
                                "Исправлен некорректный лимит трафика для пользователя",
                                telegram_id=user.telegram_id,
                            )
                            issues_fixed += 1
                        ...
                        if device_limit_needs_heal(subscription.device_limit):
                            subscription.device_limit = 1
                            logger.info(
                                "Исправлен лимит устройств для пользователя",
                                telegram_id=user.telegram_id,
                            )
                            issues_fixed += 1
                        ...
                        if subscription.connected_squads is None:
                            subscription.connected_squads = []
                            logger.info(
                                "Инициализирован список сквадов для пользователя",
                                telegram_id=user.telegram_id,
                            )
                            issues_fixed += 1
    ...
    async def get_sync_recommendations(self, db: AsyncSession) -> dict[str, Any]:
        try:
            ...
            users_without_uuid = sum(
                ...
                and any(True for _ in (getattr(user, "subscriptions", None) or []))
            )
            ...
            active_expired = sum(
                1 for sub in expired_subscriptions if sub.status == "active"
            )
            ...
            if users_without_uuid > 10:
                ...
                recommendations["reasons"].append(
                    f"Найдено {users_without_uuid} пользователей без связи с Remnawave"
                )
            ...
            if active_expired > 5:
                ...
                recommendations["reasons"].append(
                    f"Найдено {active_expired} активных подписок с истекшим сроком"
                )

            if not recommendations["should_sync"]:
                recommendations["sync_type"] = "update_only"
                recommendations["reasons"].append(
                    "Рекомендуется регулярная синхронизация данных"
                )
        ...
        try:
            ...
            if current_status != previous_status and previous_status != "unknown":
                await self._send_status_change_notification(
                    bot, previous_status, current_status, status_result
                )
        ...
        try:
            ...
            if new_status == "offline":
                ...
                if status_data.get("response_time", 0) > 10:
                    issues.append(
                        f'Медленный отклик API ({status_data.get("response_time")}с)'
                    )
            ...
            logger.info(
                "Отправлено уведомление об изменении статуса панели",
                old_status=old_status,
    ...
    async def send_manual_status_notification(
        self, bot, status: str, message: str = ""
    ):
        try:
            from app.services.admin_notification_service import AdminNotificationService
        ...
        try:
            ...
            if status == "offline":
                summary["recommendation"] = (
                    "Проверьте подключение к серверу и работоспособность панели"
                )
            elif status == "degraded":
                summary["recommendation"] = (
                    "Рекомендуется проверить состояние нод и производительность сервера"
                )
            else:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                async with self.get_api_client() as api:
                    ...
                    try:
                        ...
                        nodes_online = sum(
                            1
                            for node in nodes
                            if node.is_connected and node.is_node_online
                        )
                ...
                if result["api_available"]:
                    if attempt > 1:
                        logger.info(
                            "Панель Remnawave ответила с попытки", attempt=attempt
                        )
                    return result
                ...
                if attempt < attempts:
                    logger.warning(
                        "Ошибка проверки здоровья панели (попытка /)",
                        attempt=attempt,
                        attempts=attempts,
                        error=error,
                    )
