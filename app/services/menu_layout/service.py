...
class MenuLayoutService:
    ...
    async def get_available_callbacks(cls, db: AsyncSession) -> list[dict[str, Any]]:
        ...
        for cb in AVAILABLE_CALLBACKS:
            is_in_menu = (
                cb["callback_data"] in buttons_in_menu
                or cb["callback_data"] in builtin_callbacks
            )
            result.append(
                {
                    **cb,
    ...
    def validate_config(cls, config: dict[str, Any]) -> dict[str, Any]:
        ...
        disabled_count = sum(
            1 for btn in buttons.values() if not btn.get("enabled", True)
        )
        if disabled_count > 0:
            warnings.append(
                {
    ...
    async def update_button(
        ...
        if (
            "connect" in actual_button_id.lower()
            or button.get("builtin_id") == "connect"
        ):
            logger.info(
    ...
    def _evaluate_conditions(
        ...
        if conditions.get("show_trial") is True:
            ...
            if (
                ...
                or settings.TRIAL_DISABLED_FOR == "all"
            ):
                return False
    ...
    def _build_button(
        ...
        if button_type == "url":
            return make_button(
                text=text, url=action, icon_custom_emoji_id=custom_emoji_id
            )
        ...
        if button_type == "callback":
            ...
            return make_button(
                text=text, callback_data=action, icon_custom_emoji_id=custom_emoji_id
            )
        ...
        if open_mode == "direct":
            ...
            if is_connect_button and (
                not url or not (url.startswith("http://") or url.startswith("https://"))
            ):
                if context.subscription:
                    ...
                    subscription_url = get_display_subscription_link(
                        context.subscription
                    )
                    if subscription_url:
                        url = subscription_url
                        logger.info(
                            "Кнопка connect: получен URL из подписки: ...", url=url[:50]
                        )
    ...
    async def build_keyboard(
        ...
        for row_config in rows_config:
            ...
            for button_id in row_config.get("buttons", []):
                ...
                button = cls._build_button(
                    button_cfg, context, texts, button_id=button_id
                )
                if button:
    ...
    async def get_button_clicks_by_day(
        ...
        return await MenuLayoutStatsService.get_button_clicks_by_day(
            db, button_id, days
        )
