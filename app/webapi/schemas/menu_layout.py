...
class ButtonConditions(BaseModel):
    ...
    traffic_topup_enabled: bool | None = Field(
        default=None, description="Докупка трафика включена"
    )
...
class MenuRowConfig(BaseModel):
    ...
    id: str = Field(
        ..., min_length=1, max_length=50, description="Уникальный ID строки"
    )
...
class ButtonUpdateRequest(BaseModel):
    ...
    icon: str | None = Field(
        default=None, max_length=100, description="Эмодзи/иконка кнопки"
    )
...
class AvailableCallback(BaseModel):
    ...
    category: str = Field(
        description="Категория: menu, subscription, balance, referral, support, etc."
    )
    default_text: dict[str, str] | None = Field(
        default=None, description="Текст по умолчанию"
    )
    default_icon: str | None = Field(default=None, description="Иконка по умолчанию")
    requires_subscription: bool = Field(
        default=False, description="Требует активную подписку"
    )
