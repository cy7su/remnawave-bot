...
async def _get_overview(db: AsyncSession) -> dict[str, object]:
    ...
    total_balance_kopeks = (
        await db.scalar(select(func.coalesce(func.sum(User.balance_kopeks), 0))) or 0
    )
...
async def stats_full(
    ...
    transactions_totals = {
        **transactions_totals,
        ...
        "expenses_rubles": _kopeks_to_rubles(
            transactions_totals.get("expenses_kopeks")
        ),
        "profit_rubles": _kopeks_to_rubles(transactions_totals.get("profit_kopeks")),
        "subscription_income_kopeks": abs(
            transactions_totals.get("subscription_income_kopeks", 0)
        ),
        "subscription_income_rubles": _kopeks_to_rubles(
            abs(transactions_totals.get("subscription_income_kopeks", 0))
        ),
    }
    ...
    referral_stats = {
        **referral_stats,
        ...
        "week_earnings_rubles": _kopeks_to_rubles(
            referral_stats.get("week_earnings_kopeks")
        ),
        "month_earnings_rubles": _kopeks_to_rubles(
            referral_stats.get("month_earnings_kopeks")
        ),
    }
