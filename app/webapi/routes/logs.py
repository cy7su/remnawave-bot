...
async def get_system_log_preview(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Не удалось прочитать лог-файл"
        ) from error
...
async def download_system_log(
    ...
    try:
        ...
        Exception
    ) as error:  # pragma: no cover - защита от неожиданных ошибок отдачи файла
        logger.error("Ошибка отправки лог-файла", log_path=log_path, error=error)
        raise HTTPException(
            status_code=500, detail="Не удалось отправить лог-файл"
        ) from error
...
async def list_monitoring_logs(
    ...
    total = await monitoring_service.get_monitoring_logs_count(
