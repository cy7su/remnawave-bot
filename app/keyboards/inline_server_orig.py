    ...
    *,
    ...
    if is_moderator and not is_admin:
        keyboard_rows.append(
            [make_button(text="🧑‍⚖️ Модерация", callback_data="moderator_panel")]
        )
...
def get_main_menu_keyboard(
    ...
    *,
    ...
    keyboard.append(
        [make_button(text=balance_button_text, callback_data="menu_balance")]
    )
    ...
    if show_trial:
        subscription_buttons.append(
            make_button(text=texts.MENU_TRIAL, callback_data="menu_trial")
        )

    if show_buy:
        subscription_buttons.append(
            make_button(text=texts.MENU_BUY_SUBSCRIPTION, callback_data="menu_buy")
        )
    ...
    if support_enabled:
        paired_buttons.append(
            make_button(text=texts.MENU_SUPPORT, callback_data="menu_support")
        )
...
def get_payment_methods_keyboard_with_cart(
    ...
    keyboard.inline_keyboard.append(
        [
            make_button(
                text="🗑️ Очистить корзину и вернуться", callback_data="clear_saved_cart"
            )
        ]
    )
...
def get_traffic_packages_keyboard(
    ...
    logger.info(
        "🔍 RAW CONFIG", TRAFFIC_PACKAGES_CONFIG=settings.TRAFFIC_PACKAGES_CONFIG
    )
    ...
    if settings.is_riopay_enabled():
        ...
        keyboard.append(
            [
                make_button(
                    text=texts.t(
                        "PAYMENT_RIOPAY", f"💳 Банковская карта ({riopay_name})"
                    ),
...
def get_saved_cards_keyboard(
    ...
    for card in cards:
        ...
        keyboard.append(
            [make_button(text=card_label, callback_data=f"unlink_card_{card.id}")]
        )
    keyboard.append(
        [make_button(text=texts.BACK, style="danger", callback_data="menu_balance")]
    )
...
def get_autopay_period_keyboard(
    ...
    default_label = texts.t(
        "AUTOPAY_PERIOD_DEFAULT_BUTTON", "⚙️ По умолчанию (самый дешёвый)"
    )
