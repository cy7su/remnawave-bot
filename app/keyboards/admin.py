...
def get_maintenance_keyboard(
    ...
    keyboard.append(
        [
            ...
            make_button(
                text=_t(texts, 'ADMIN_MAINTENANCE_PANEL_STATUS', 'Статус панели') + ('' if panel_has_issues else ''),
...
def get_welcome_text_keyboard(language: str = 'ru', is_enabled: bool = True) -> InlineKeyboardMarkup:
    texts = get_texts(language)
    toggle_text = (
        _t(texts, 'ADMIN_WELCOME_DISABLE', 'Отключить') if is_enabled else _t(texts, 'ADMIN_WELCOME_ENABLE', 'Включить')
    )
