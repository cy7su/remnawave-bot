...
def format_number(number: float, separator: str = " ") -> str:
    if isinstance(number, float):
        ...
        if decimal_part > 0:
            return (
                f"{formatted_integer}.{decimal_part:.2f}".split(".")[0]
                + f'.{str(decimal_part).split(".")[1][:2]}'
            )
...
def format_username(
    username: str | None, user_id: int, full_name: str | None = None
) -> str:
    if full_name:
        return full_name
    if username:
...
def format_subscription_status(
    is_active: bool, is_trial: bool, end_date: datetime | str, language: str = "ru"
) -> str:
