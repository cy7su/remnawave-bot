...
def calculate_prorated_price(
    monthly_price: int, end_date: datetime, min_charge_days: int = 1
) -> tuple[int, int]:
...
async def compute_simple_subscription_price(
    ...
    *,
    ...
    try:
        traffic_limit_gb = (
            int(traffic_limit_raw) if traffic_limit_raw is not None else 0
        )
    ...
    breakdown = _build_simple_subscription_breakdown(
        pricing, resolved_uuids, promo_group
    )
...
def _build_simple_subscription_breakdown(
    ...
    total_discount = (
        base_discount + traffic_discount + servers_discount + devices_discount
    )
    ...
    for srv in bd.get("servers", []):
        ...
        final_price = (
            PricingEngine.apply_discount(original_price, servers_discount_percent)
            if is_available
            else 0
        )
    ...
    return {
        ...
        "applied_promo_group_id": (
            getattr(promo_group, "id", None) if promo_group else None
        ),
...
def validate_pricing_calculation(
    base_price: int, monthly_additions: int, months: int, total_calculated: int
) -> bool:
