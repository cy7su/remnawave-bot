...
def _validate_row(row_id: str, data: dict) -> dict | None:
    ...
    clean_buttons = [
        b
        for b in buttons
        if isinstance(b, str) and (b in BUILTIN_SECTIONS or b.startswith("custom_"))
    ]
...
def _validate_custom_button(btn_id: str, data: dict) -> dict | None:
    ...
    if not isinstance(labels, dict):
        labels = {}
    clean_labels = {
        k: v for k, v in labels.items() if isinstance(k, str) and isinstance(v, str)
    }
...
async def load_menu_layout_cache() -> dict[str, object]:
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(SystemSetting).where(SystemSetting.key == MENU_LAYOUT_KEY)
            )
    ...
    logger.info(
        "Menu layout cache loaded",
        rows=len([k for k in merged if k.startswith("row_")]),
    )
    return merged
