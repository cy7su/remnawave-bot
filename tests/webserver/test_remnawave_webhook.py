...
def reset_remnawave_webhook_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        settings, "REMNAWAVE_WEBHOOK_PATH", "/remnawave-webhook", raising=False
    )
    monkeypatch.setattr(
        settings,
...
async def test_remnawave_webhook_accepts_event_without_scope(
...
def test_intentional_panel_deletion_guard_respects_hard_cap(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
