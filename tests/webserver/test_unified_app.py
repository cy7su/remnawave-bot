...
async def test_unified_app_health_reports_features(
    ...
    monkeypatch.setattr(
        settings, "WEBHOOK_URL", "https://hooks.example.com", raising=False
    )
    ...
    monkeypatch.setattr(
        settings, "MINIAPP_STATIC_PATH", str(miniapp_static_dir), raising=False
    )
    ...
    health_route = next(
        ...
        and getattr(route.endpoint, "__name__", "") == "unified_health"
    )
...
@pytest.mark.anyio
async def test_unified_app_health_path_without_admin(
...
@pytest.mark.anyio
async def test_unified_app_docs_enabled_with_alias(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
