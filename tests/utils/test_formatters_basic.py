...
def test_format_datetime_handles_iso_strings(fixed_datetime: datetime) -> None:
    ...
    assert formatters.format_datetime(iso_value) == fixed_datetime.strftime(
        "%d.%m.%Y %H:%M"
    )
...
def test_format_date_uses_custom_format(fixed_datetime: datetime) -> None:
    ...
    assert formatters.format_date(
        iso_value, format_str="%Y/%m/%d"
    ) == fixed_datetime.strftime("%Y/%m/%d")
...
def test_format_traffic_usage_supports_unlimited() -> None:
    ...
    assert (
        formatters.format_traffic_usage(10.0, 100, language="ru")
        == "10.0 ГБ / 100 ГБ (10.0%)"
    )
