...
def test_maps_tag_aliases_to_telegram_tags():
    assert (
        html_to_telegram("<strong>x</strong> <em>y</em> <del>z</del>")
        == "<b>x</b> <i>y</i> <s>z</s>"
    )
...
def test_link_kept_only_with_http_href():
    assert (
        html_to_telegram('<a href="https://example.com">x</a>')
...
def test_oversized_href_drops_anchor_but_keeps_text():
    ...
    body = "word " * 1500
    rendered = html_to_telegram(
        f'<p>{body}</p><p>see <a href="{href}">{body}</a> end</p>'
    )
...
def test_blockquote_and_code_preserved():
    assert (
        html_to_telegram("<blockquote>q</blockquote>") == "<blockquote>q</blockquote>"
    )
