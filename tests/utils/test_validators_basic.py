...
def test_validate_phone_strips_formatting_and_checks_pattern(
    phone: str, is_valid: bool
) -> None:
...
def test_validate_telegram_username_enforces_length(
    username: str, is_valid: bool
) -> None:
...
def test_validate_amount_returns_float_within_bounds() -> None:
    ...
    assert validators.validate_amount(
        "10.5", min_amount=5, max_amount=20
    ) == pytest.approx(10.5)
...
def test_validate_traffic_amount_supports_units(
...
def test_validate_rules_content_accepts_supported_markup() -> None:
    ...
    is_valid, message, suggestion = validators.validate_rules_content(
