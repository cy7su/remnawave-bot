...
def _enable_apple_iap(
    ...
    monkeypatch.setattr(
        settings, "APPLE_IAP_ISSUER_ID", "test-issuer-id", raising=False
    )
    monkeypatch.setattr(
        settings, "APPLE_IAP_BUNDLE_ID", "com.bitnet.vpnclient", raising=False
    )
    ...
    monkeypatch.setattr(
        settings, "APPLE_IAP_ROOT_CERTS_PATHS", str(cert_path), raising=False
    )
    monkeypatch.setattr(
        settings,
        "APPLE_IAP_PRODUCTS",
...
class TestSettings:
    ...
    def test_blank_key_metadata_disables(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        _enable_apple_iap(monkeypatch, tmp_path)
    ...
    def test_blank_private_key_disables(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        _enable_apple_iap(monkeypatch, tmp_path)
    ...
    def test_private_key_file_read_error_is_logged(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        ...
        monkeypatch.setattr(settings, "APPLE_IAP_PRIVATE_KEY", "", raising=False)
        monkeypatch.setattr(
            settings, "APPLE_IAP_PRIVATE_KEY_PATH", str(missing_key_path), raising=False
        )
        ...
        assert (
            error_log.call_args.args[0] == "Failed to load Apple IAP private key file"
        )
        assert error_log.call_args.kwargs["path"] == str(missing_key_path)
        assert error_log.call_args.kwargs["exc_info"] is True

    def test_product_mapping_normalizes_positive_ints(
...
class TestTransactionValidation:
    def test_valid_transaction(
        ...
        assert (
            service.validate_transaction_info(
                txn_info, "com.bitnet.vpnclient.topup.100"
            )
            is None
        )
    ...
    def test_rejects_revoked_transaction(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        _enable_apple_iap(monkeypatch, tmp_path)
        result = AppleIAPService().validate_transaction_info(
    ...
    async def test_verify_transaction_uses_client_and_verifier(
        ...
        class FakeVerifier:
            def verify_and_decode_signed_transaction(
                self, signed_transaction_info: str
            ):
        ...
        monkeypatch.setattr(
            service, "_verifier", lambda environment=None: FakeVerifier()
        )
...
class TestSchemas:
    ...
    def test_rejects_non_numeric_transaction_id(self) -> None:
        with pytest.raises(Exception, match="digits"):
            ApplePurchaseRequest(
                product_id="com.bitnet.vpnclient.topup.100", transaction_id="abc"
            )

    def test_rejects_empty_transaction_id(self) -> None:
        with pytest.raises(ValidationError):
            ApplePurchaseRequest(
                product_id="com.bitnet.vpnclient.topup.100", transaction_id=""
            )

    def test_account_token_response(self) -> None:
        response = AppleAccountTokenResponse(
...
class TestCabinetAppleIAPRoutes:
    @pytest.mark.anyio("asyncio")
    async def test_purchase_rate_limit_decodes_redis_bytes_counter(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        class FakeRedis:
            ...
            async def incr(
                self, key: str
            ) -> int:  # pragma: no cover - failure counter blocks before incr
                raise AssertionError(f"incr should not be called for {key}")

        monkeypatch.setattr(
            settings, "APPLE_IAP_PURCHASE_FAILURE_LIMIT_PER_HOUR", 3, raising=False
        )

        assert (
            await apple_iap_routes._check_purchase_rate_limit(
                FakeRedis(), user_id=1, ip_address=None
            )
            is False
        )
    ...
    async def test_purchase_rate_limit_blocks_invalid_redis_counter_by_default(
        ...
        class FakeRedis:
            ...
            async def incr(
                self, key: str
            ) -> int:  # pragma: no cover - invalid failure counter fails open first
                raise AssertionError(f"incr should not be called for {key}")

        monkeypatch.setattr(
            settings, "APPLE_IAP_RATE_LIMIT_FAIL_OPEN", False, raising=False
        )
    ...
    @pytest.mark.anyio("asyncio")
    async def test_purchase_rate_limit_can_fail_open_by_setting(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from app.cabinet import apple_iap as apple_iap_routes
        ...
        monkeypatch.setattr(
            settings, "APPLE_IAP_RATE_LIMIT_FAIL_OPEN", True, raising=False
        )
    ...
    async def test_purchase_rate_limit_sets_ttl_atomically_for_new_counter(
        ...
        class FakeRedis:
            ...
            async def incr(
                self, key: str
            ) -> int:  # pragma: no cover - SET NX creates first counter
                raise AssertionError(f"incr should not be called for new counter {key}")

            async def expire(
                self, key: str, ttl: int
            ) -> None:  # pragma: no cover - TTL must be set with SET
                raise AssertionError(
                    f"expire should not be called for {key} with ttl={ttl}"
                )

        client = FakeRedis()
        monkeypatch.setattr(
            settings, "APPLE_IAP_PURCHASE_RATE_LIMIT_PER_MINUTE", 10, raising=False
        )

        assert (
            await apple_iap_routes._check_purchase_rate_limit(
                client, user_id=1, ip_address=None
            )
            is True
        )
    ...
    async def test_purchase_rate_limit_increments_existing_counter_without_expire(
        ...
        class FakeRedis:
            ...
            async def expire(
                self, key: str, ttl: int
            ) -> None:  # pragma: no cover - existing key already has TTL
                raise AssertionError(
                    f"expire should not be called for {key} with ttl={ttl}"
                )
    ...
    @pytest.mark.anyio("asyncio")
    async def test_apple_iap_redis_client_uses_lifespan_state(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        async with apple_iap_routes.apple_iap_lifespan(app):
            ...
            assert (
                getattr(app.state, apple_iap_routes.APPLE_IAP_REDIS_STATE_KEY)
                is clients[0]
            )
    ...
    @pytest.mark.anyio("asyncio")
    async def test_apple_iap_router_lifespan_initializes_app_state(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from fastapi import FastAPI
        ...
        async with app.router.lifespan_context(app):
            assert (
                getattr(app.state, apple_iap_routes.APPLE_IAP_REDIS_STATE_KEY) is client
            )

    @pytest.mark.anyio("asyncio")
    async def test_account_token_requires_full_apple_iap_configuration(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await apple_iap_account_token(
                ...
                fulfillment_service=SimpleNamespace(
                    get_account_token=get_account_token
                ),
            )
...
class TestFulfillmentService:
    ...
    async def test_rejects_sandbox_transaction_when_production_disallows_sandbox(
        ...
        monkeypatch.setattr(
            settings, "APPLE_IAP_ALLOW_SANDBOX_ON_PRODUCTION", False, raising=False
        )
    ...
    async def test_records_sandbox_transaction_when_production_allows_sandbox(
        ...
        monkeypatch.setattr(
            settings, "APPLE_IAP_ALLOW_SANDBOX_ON_PRODUCTION", True, raising=False
        )
        service = AppleIAPFulfillmentService()
        record_sandbox = AsyncMock(
            return_value=AppleFulfillmentResult(True, "sandbox_recorded")
        )
    ...
    async def test_purchase_verification_respects_sandbox_fallback_setting(
        ...
        result = await AppleIAPFulfillmentService(
            apple_service
        ).verify_and_fulfill_purchase(
            _FakeDB(),
    ...
    async def test_successful_credit_locks_user_before_financial_rows(
        ...
        monkeypatch.setattr(
            apple_iap_module, "create_apple_transaction", create_apple_txn
        )
        monkeypatch.setattr(
            apple_iap_module, "create_transaction", create_financial_txn
        )
    ...
    @pytest.mark.anyio("asyncio")
    async def test_credit_side_effects_use_injected_bot(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        async def send_cart_notification_after_topup(
            _user, _amount_kopeks, _db, cart_bot
        ):
            cart_notifications.append(cart_bot)

        monkeypatch.setattr(
            apple_iap_module, "emit_transaction_side_effects", AsyncMock()
        )
        monkeypatch.setattr(
            "app.services.referral_service.process_referral_topup", AsyncMock()
        )
        monkeypatch.setattr(
...
class TestAdapterFallback:
    ...
    async def test_verify_transaction_can_disable_environment_fallback(
        ...
        await service.verify_transaction(
            "2000000123456789", "Production", allow_environment_fallback=False
        )
...
class TestNotificationService:
    ...
    async def test_signed_transaction_verification_failure_marks_notification_failed(
        ...
        _enable_apple_iap(monkeypatch, tmp_path)
        db = _FakeDB()
        notification_row = SimpleNamespace(
            status="received", notification_uuid="notification-uuid"
        )
        ...
        class FakeAppleService:
            ...
            def verify_signed_transaction_info(
                self, signed_transaction_info: str, environment: str
            ):
                return None

        monkeypatch.setattr(
            apple_iap_module, "AsyncSessionLocal", lambda: _AsyncContext(db)
        )
        monkeypatch.setattr(
            apple_iap_module,
        ...
        monkeypatch.setattr(
            apple_iap_module, "mark_apple_notification_processed", mark_processed
        )

        ok, reason = await AppleIAPNotificationService(
            FakeAppleService()
    ...
    async def test_refund_uses_outer_notification_environment_for_owner_check(
        ...
        monkeypatch.setattr(
            apple_iap_module, "mark_apple_notification_processed", AsyncMock()
        )
        monkeypatch.setattr(
            apple_iap_module,
        ...
        monkeypatch.setattr(
            apple_iap_module, "create_apple_abuse_event", create_abuse_event
        )
    ...
    async def test_duplicate_notification_insert_race_is_treated_as_duplicate(
        ...
        _enable_apple_iap(monkeypatch, tmp_path)
        db = _FakeDB()
        processed_row = SimpleNamespace(
            status="processed", notification_uuid="notification-uuid"
        )
        ...
        monkeypatch.setattr(
            apple_iap_module, "get_apple_notification_by_uuid", get_by_uuid
        )
        monkeypatch.setattr(
            apple_iap_module,
            "get_apple_notification_by_payload_hash",
        ...
        monkeypatch.setattr(
            apple_iap_module, "create_apple_notification", create_notification
        )
    ...
    @pytest.mark.anyio("asyncio")
    async def test_refund_rejects_app_account_token_mismatch(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        apple_txn = SimpleNamespace(
            transaction_id="2000000123456789",
        ...
        create_abuse_event.assert_awaited_once()
        assert (
            create_abuse_event.await_args.args[1]
            == "notification_app_account_token_mismatch"
        )
        lock_user.assert_not_awaited()

    @pytest.mark.anyio("asyncio")
    async def test_refund_rejects_environment_mismatch(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        apple_txn = SimpleNamespace(
            transaction_id="2000000123456789",
        ...
        create_abuse_event.assert_awaited_once()
        assert (
            create_abuse_event.await_args.args[1] == "notification_environment_mismatch"
        )
        lock_user.assert_not_awaited()

    @pytest.mark.anyio("asyncio")
    async def test_refund_reversed_credits_with_outer_transaction(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(settings, "APPLE_IAP_ENVIRONMENT", "Sandbox", raising=False)
        apple_txn = SimpleNamespace(
...
class TestAppleIAPRouting:
    ...
    async def test_legacy_aiohttp_webhook_server_does_not_mount_apple_iap(
        ...
        _enable_apple_iap(monkeypatch, tmp_path)
        monkeypatch.setattr(
            settings, "APPLE_IAP_WEBHOOK_PATH", "/apple-iap-webhook", raising=False
        )
