...
def configure_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        settings, "YOOKASSA_WEBHOOK_PATH", "/yookassa-webhook", raising=False
    )
...
def test_resolve_yookassa_ip_skips_trusted_proxy_hops(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        settings, "YOOKASSA_TRUSTED_PROXY_NETWORKS", "203.0.113.0/24", raising=False
    )
...
def test_resolve_yookassa_ip_trusted_public_proxy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        settings, "YOOKASSA_TRUSTED_PROXY_NETWORKS", "198.51.100.0/24", raising=False
    )
...
async def _post_webhook(
    client: TestClient, payload: dict, **headers: str
) -> web.Response:
...
@pytest.mark.asyncio
async def test_handle_webhook_trusts_cf_connecting_ip(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_get_db(monkeypatch)
...
@pytest.mark.asyncio
async def test_handle_webhook_with_optional_signature(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_get_db(monkeypatch)
...
@pytest.mark.asyncio
async def test_handle_webhook_accepts_canceled_event(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_get_db(monkeypatch)
