...
async def test_cannot_grant_permissions_not_held() -> None:
    ...
    with patch(
        ...
        await _ensure_can_grant(
            AsyncMock(), MagicMock(id=1), 100, ["roles:create", "roles:assign"]
        )
...
async def test_superadmin_exempt_and_does_not_query() -> None:
    fetch = AsyncMock()
    with patch(
        "app.cabinet.routes.admin_roles.UserRoleCRUD.get_user_permissions", fetch
    ):
        await _ensure_can_grant(
            AsyncMock(), MagicMock(id=1), SUPERADMIN_LEVEL + 1, ["*:*"]
        )
