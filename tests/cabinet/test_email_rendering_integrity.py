...
def _assert_single_document(body: str, label: str) -> None:
    lowered = body.lower()
    assert (
        lowered.count("<!doctype") == 1
    ), f"{label}: документ должен содержать ровно один DOCTYPE"
    assert (
        lowered.count("<html") == 1
    ), f"{label}: документ должен содержать ровно один <html>"
    assert (
        lowered.count("</html>") == 1
    ), f"{label}: документ должен содержать ровно один </html>"
...
def test_default_template_renders_clean_for_every_language(type_key, lang):
    ...
    ), f"{type_key}/{lang}: перенос строки в теме"
    ...
    leftover = [
        var for var in sample if f"{{{var}}}" in body or f"{{{var}}}" in subject
    ]
...
def test_wrap_full_document_is_not_double_wrapped():
    ...
    full_doc = _get_default_template(
        "email_verification", "ru", SAMPLE_CONTEXTS["email_verification"]
    )["body_html"]
...
def test_editor_default_roundtrips_through_override_render(type_key):
    ...
    templates = EmailNotificationTemplates()
    editor_payload = _get_default_template(
        type_key, "ru", _placeholder_context(type_key)
    )
    sample = SAMPLE_CONTEXTS[type_key]
