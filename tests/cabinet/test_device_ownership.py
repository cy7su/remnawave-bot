...
from app.cabinet.utils.device_ownership import (
...
async def test_verify_finds_hwid_on_first_panel() -> None:
    ...
    with patch(
        ...
        return_value=_patched_remnawave(devices),
    ):
...
async def test_verify_short_circuits_after_first_hit() -> None:
    ...
    with patch(
        "app.services.remnawave_service.RemnaWaveService", return_value=service_mock
    ):
        assert await verify_hwid_belongs_to_user(user, "TARGET") is True
