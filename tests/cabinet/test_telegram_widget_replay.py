...
async def test_widget_login_is_one_time_and_24h() -> None:
    req = TelegramWidgetAuthRequest(
        id=123, first_name="A", auth_date=1700000000, hash=H
    )
    ...
    with ExitStack() as s:
        s.enter_context(
            patch(
                "app.cabinet.routes.auth.get_client_ip",
        ...
        s.enter_context(
            patch("app.cabinet.routes.auth.validate_telegram_login_widget", validate)
        )
        s.enter_context(
            patch("app.cabinet.routes.auth.TokenReplayCache.is_token_replayed", replay)
        )
        with pytest.raises(HTTPException) as exc:
            await auth_telegram_widget(
                request=req, raw_request=MagicMock(), db=AsyncMock()
            )
    assert exc.value.status_code == status.HTTP_401_UNAUTHORIZED
    replay.assert_awaited_once()
    assert (
        validate.call_args.kwargs.get("max_age_seconds") == 86400
    )  # tightened from 30 days
...
async def test_widget_link_is_one_time_and_24h() -> None:
    ...
    with ExitStack() as s:
        ...
        s.enter_context(
            patch(
                "app.cabinet.routes.account_linking.validate_telegram_login_widget",
                validate,
            )
        )
        s.enter_context(
            patch(
                "app.cabinet.routes.account_linking.TokenReplayCache.is_token_replayed",
                replay,
            )
        )
        with pytest.raises(HTTPException) as exc:
