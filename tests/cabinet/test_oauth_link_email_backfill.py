...
def _provider(
    email: str | None, email_verified: bool, provider_id: str = "G1"
) -> MagicMock:
    prov = MagicMock()
    prov.exchange_code = AsyncMock(return_value={"access_token": "x"})
    prov.get_user_info = AsyncMock(
        return_value=SimpleNamespace(
            provider_id=provider_id, email=email, email_verified=email_verified
        )
    )
    return prov
...
async def _run_link(user, *, email, email_verified, email_owner=None):
    ...
    with ExitStack() as s:
        ...
        s.enter_context(
            patch(
                "app.cabinet.routes.account_linking.set_user_oauth_provider_id", set_id
            )
        )
...
async def test_backfills_verified_email_when_user_has_none() -> None:
    ...
    result, set_id, db = await _run_link(
        user, email="Foo.Bar@Gmail.com", email_verified=True
    )
...
async def test_does_not_overwrite_existing_email() -> None:
    user = SimpleNamespace(
        id=1, google_id=None, email="mine@example.com", email_verified=True
    )

    result, set_id, _ = await _run_link(
        user, email="other@gmail.com", email_verified=True
    )
...
@pytest.mark.asyncio
async def test_skips_backfill_when_email_owned_by_another_account_but_still_links() -> (
    None
):
    user = SimpleNamespace(id=1, google_id=None, email=None, email_verified=False)
...
async def test_does_not_backfill_unverified_provider_email() -> None:
    ...
    result, _set_id, _ = await _run_link(
        user, email="unverified@gmail.com", email_verified=False
    )
