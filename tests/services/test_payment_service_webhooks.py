...
class FakeSession:
    ...
    def add(
        self, obj: Any
    ) -> None:  # pragma: no cover - используется при создании транзакций
...
async def test_process_mulenpay_callback_success(
    ...
    monkeypatch.setattr(
        payment_service_module, "get_mulenpay_payment_by_uuid", fake_get_by_uuid
    )
    monkeypatch.setattr(
        payment_service_module, "get_mulenpay_payment_by_mulen_id", fake_get_by_id
    )
    ...
    monkeypatch.setattr(
        payment_service_module, "create_transaction", fake_create_transaction
    )
    ...
    monkeypatch.setattr(
        payment_service_module, "update_mulenpay_payment_status", fake_update_status
    )
    ...
    monkeypatch.setattr(
        payment_service_module, "link_mulenpay_payment_to_transaction", fake_link
    )
    ...
    monkeypatch.setattr(
        type(settings),
        "format_price",
        lambda self, amount: f"{amount / 100:.2f}₽",
        raising=False,
    )
...
@pytest.mark.anyio("asyncio")
async def test_process_cryptobot_webhook_success(
    ...
    monkeypatch.setitem(
        sys.modules, "app.database.crud.cryptobot", fake_cryptobot_module
    )
    ...
    monkeypatch.setitem(
        sys.modules, "app.database.crud.transaction", fake_transaction_module
    )
    monkeypatch.setattr(
        payment_service_module, "create_transaction", fake_create_transaction
    )
    ...
    monkeypatch.setattr(
        payment_service_module.currency_converter,
        "usd_to_rub",
...
async def test_process_heleket_webhook_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(payment_service_module, "get_user_by_id", fake_get_user)
    monkeypatch.setattr(
        "app.services.payment.heleket.format_referrer_info", lambda u: ""
    )
...
@pytest.mark.anyio("asyncio")
async def test_process_yookassa_webhook_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = DummyBot()
    service = _make_service(bot)
    ...
    async def fake_update(
        db, payment_id, status, is_paid, is_captured, captured_at, payment_method_type
    ):
        payment.status = status
...
@pytest.mark.anyio("asyncio")
async def test_process_yookassa_webhook_uses_remote_status(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = DummyBot()
    service = _make_service(bot)
...
@pytest.mark.anyio("asyncio")
async def test_process_yookassa_webhook_handles_cancellation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = DummyBot()
    service = _make_service(bot)
...
async def test_process_yookassa_webhook_restores_missing_payment(
    ...
    monkeypatch.setattr(
        payment_service_module, "get_yookassa_payment_by_id", fake_get_payment
    )
    monkeypatch.setattr(
        payment_service_module, "create_yookassa_payment", fake_create_payment
    )
    monkeypatch.setattr(
        payment_service_module, "update_yookassa_payment_status", fake_update_status
    )
    monkeypatch.setattr(
        payment_service_module, "link_yookassa_payment_to_transaction", fake_link
    )
...
@pytest.mark.anyio("asyncio")
async def test_process_yookassa_webhook_missing_metadata(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        payment_service_module, "update_yookassa_payment_status", update_mock
    )
...
@pytest.mark.anyio("asyncio")
async def test_process_yookassa_webhook_missing_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = DummyBot()
    service = _make_service(bot)
...
async def test_process_pal24_callback_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        payment_service_module, "get_pal24_payment_by_order_id", fake_get_by_order
    )
    monkeypatch.setattr(
        payment_service_module, "get_pal24_payment_by_bill_id", fake_get_by_bill
    )
    monkeypatch.setattr(
        payment_service_module, "update_pal24_payment_status", fake_update
    )
    monkeypatch.setattr(
        payment_service_module, "link_pal24_payment_to_transaction", fake_link
    )
    ...
    monkeypatch.setitem(
        ...
        SimpleNamespace(
            get_texts=lambda language: SimpleNamespace(
                t=lambda key, default=None: default
            )
        ),
    )
    ...
    assert (
        reply_markup.inline_keyboard[0][0].kwargs["callback_data"]
...
@pytest.mark.anyio("asyncio")
async def test_get_pal24_payment_status_auto_finalize(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = DummyBot()
    service = _make_service(bot)
    ...
    monkeypatch.setattr(
        payment_service_module, "get_pal24_payment_by_id", fake_get_payment_by_id
    )
    monkeypatch.setattr(
        payment_service_module, "update_pal24_payment_status", fake_update_payment
    )
    monkeypatch.setattr(
        payment_service_module, "link_pal24_payment_to_transaction", fake_link_payment
    )
...
@pytest.mark.anyio("asyncio")
async def test_process_pal24_callback_payment_not_found(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = DummyBot()
    service = _make_service(bot)
