...
async def test_lookup_delegates_to_support_query(
    ...
    monkeypatch.setattr(
        reconciliation_module, "find_apple_transactions_for_support", lookup
    )

    result = await AppleIAPReconciliationService().lookup(
        db, "2000000123456789", limit=5
    )
...
@pytest.mark.anyio("asyncio")
async def test_reconcile_recent_transactions_flags_drift_and_counts_backlog(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    db = _FakeDB()
    ...
    notifications = [
        SimpleNamespace(notification_uuid="n1"),
        SimpleNamespace(notification_uuid="n2"),
    ]
    ...
    result = await AppleIAPReconciliationService(
        apple_service
    ).reconcile_recent_transactions(db, limit=10)
