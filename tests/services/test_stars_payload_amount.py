...
def test_parser_extracts_amount_kopeks_from_known_payload_shapes(
    payload: str, expected: int
) -> None:
    assert (
        parse(payload) == expected
    ), f"Parser must recover {expected} kopeks from {payload!r}; got {parse(payload)!r}"
...
def test_parser_returns_none_for_unrecognised_shapes(payload: object) -> None:
    assert (
        parse(payload) is None
    ), f"Parser must return None for {payload!r} so caller falls back to stars×rate"
...
def test_negative_control_old_rate_was_lossy(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        reconstructed_rubles == 149.5
    ), "sanity check: 115 × 1.3 = 149.50 ₽ (the loss)"
    ...
    assert (
        plausible(
            payload_kopeks=payload_kopeks, reconstructed_kopeks=reconstructed_kopeks
        )
        is True
    )
...
def test_negative_control_at_new_rate_is_lossless_for_integer_rubles(
    ...
    for rubles in (50, 100, 150, 500, 1000):
        ...
        assert (
            plausible(payload_kopeks=kopeks, reconstructed_kopeks=reconstructed_kopeks)
            is True
        )
