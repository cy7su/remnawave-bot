...
def _enable_service(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        type(settings), "is_mulenpay_enabled", lambda self: True, raising=False
    )
    ...
    monkeypatch.setattr(
        settings, "MULENPAY_BASE_URL", "https://mulenpay.test", raising=False
    )
...
@pytest.mark.anyio("asyncio")
async def test_request_returns_none_after_timeouts(
