...
def test_build_miniapp_or_callback_button_falls_through_for_back_to_menu(
    ...
    monkeypatch.setattr(
        settings, "MINIAPP_CUSTOM_URL", "https://cabinet.example.com", raising=False
    )
...
def test_home_button_key_is_not_in_cabinet_miniapp_button_keys() -> None:
    ...
    assert (
        cabinet_keys is not None
    ), "CABINET_MINIAPP_BUTTON_KEYS expected in admin.messages"
...
async def test_topup_success_keyboard_renders_callback_main_menu_button_in_cabinet_mode(
    ...
    assert (
        len(last_row) == 1
    ), f"Main Menu row should contain exactly one button, got {last_row}"
