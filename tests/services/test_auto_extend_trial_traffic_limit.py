...
async def test_trial_conversion_adopts_unlimited_tariff_limit(monkeypatch):
    ...
    prepared = SimpleNamespace(
        traffic_limit_gb=None, tariff_id=5
    )  # renewal cart omits traffic
    subscription = SimpleNamespace(
        tariff_id=5
    )  # trial on the same tariff -> is_tariff_change False

    result = await svc._resolve_extend_traffic_limit_gb(
        AsyncMock(), prepared, subscription, was_trial=True
    )
...
async def test_non_trial_renewal_is_untouched(monkeypatch):
    ...
    result = await svc._resolve_extend_traffic_limit_gb(
        AsyncMock(), prepared, subscription, was_trial=False
    )
