...
async def test_commission_accrues_before_minimum_first_topup(monkeypatch):
    ...
    monkeypatch.setattr(
        referral_service, "create_referral_earning", create_referral_earning_mock
    )
    monkeypatch.setattr(
        referral_service, "get_user_campaign_id", AsyncMock(return_value=None)
    )

    monkeypatch.setattr(
        referral_service.settings, "REFERRAL_MINIMUM_TOPUP_KOPEKS", 20000
    )
    monkeypatch.setattr(
        referral_service.settings, "REFERRAL_FIRST_TOPUP_BONUS_KOPEKS", 5000
    )
    monkeypatch.setattr(
        referral_service.settings, "REFERRAL_INVITER_BONUS_KOPEKS", 10000
    )
...
async def test_first_topup_inviter_gets_fixed_plus_commission(monkeypatch):
    ...
    monkeypatch.setattr(
        referral_service, "get_commission_payment_count", AsyncMock(return_value=0)
    )
    monkeypatch.setattr(
        referral_service, "get_user_campaign_id", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
        referral_service, "get_effective_referral_commission_percent", lambda u: 15
    )

    monkeypatch.setattr(
        referral_service.settings, "REFERRAL_MINIMUM_TOPUP_KOPEKS", 10000
    )
    monkeypatch.setattr(
        referral_service.settings, "REFERRAL_FIRST_TOPUP_BONUS_KOPEKS", 5000
    )
    monkeypatch.setattr(
        referral_service.settings, "REFERRAL_INVITER_BONUS_KOPEKS", 5000
    )  # 50 rub
