...
def overpay_settings(monkeypatch: pytest.MonkeyPatch) -> pytest.MonkeyPatch:
    ...
    monkeypatch.setattr(
        settings, "OVERPAY_PROJECT_ID", "default-project", raising=False
    )
    monkeypatch.setattr(
        settings, "OVERPAY_SBP_TERMINAL_ID", "sbp-terminal", raising=False
    )
    monkeypatch.setattr(
        settings, "OVERPAY_CARD_TERMINAL_ID", "card-terminal", raising=False
    )
    monkeypatch.setattr(
        settings, "OVERPAY_INT_TERMINAL_ID", "int-terminal", raising=False
    )
    ...
    monkeypatch.setattr(
        settings, "OVERPAY_RETURN_URL", "https://t.me/testbot", raising=False
    )
...
def _make_service(
    stub: StubOverpayService, monkeypatch: pytest.MonkeyPatch
) -> tuple[PaymentService, dict[str, Any]]:
    ...
    monkeypatch.setattr(
        payment_service_module, "get_user_by_id", fake_get_user_by_id, raising=False
    )
    monkeypatch.setattr(
        overpay_crud_module,
        "create_overpay_payment",
        fake_create_overpay_payment,
        raising=False,
    )
...
async def test_card_option_routes_to_card_terminal(
    ...
    stub = StubOverpayService(
        create_response={"id": "op-1", "resultUrl": "https://pay.overpay.io/form"}
    )
...
async def test_int_option_converts_to_eur(
    ...
    stub = StubOverpayService(
        create_response={"id": "op-2", "resultUrl": "https://pay.overpay.io/form"}
    )
    service, captured = _make_service(stub, monkeypatch)
...
async def test_int_option_below_min_eur_rejected(
    ...
    stub = StubOverpayService(
        create_response={"id": "op-3", "resultUrl": "https://pay"}
    )
...
async def test_int_option_rejected_when_disabled(
    ...
    stub = StubOverpayService(
        create_response={"id": "op-4", "resultUrl": "https://pay"}
    )
    service, _ = _make_service(stub, monkeypatch)
...
async def test_fps_direct_qr_without_link_returns_none(
    ...
    overpay_settings.setattr(settings, "OVERPAY_SBP_DIRECT_QR", True, raising=False)
    stub = StubOverpayService(
        s2s_response={"id": "order-10", "status": "pending"}, redirect_link=None
    )
    service, _ = _make_service(stub, monkeypatch)
...
async def test_fps_without_direct_qr_uses_form(
    ...
    stub = StubOverpayService(
        create_response={"id": "op-5", "resultUrl": "https://pay.overpay.io/form"}
    )
    service, _ = _make_service(stub, monkeypatch)
...
async def test_legacy_call_without_option_keeps_old_behavior(
    ...
    stub = StubOverpayService(
        create_response={"id": "op-6", "resultUrl": "https://pay.overpay.io/form"}
    )
    service, captured = _make_service(stub, monkeypatch)
...
async def test_guest_payment_forwards_int_option(
    ...
    monkeypatch.setattr(
        service, "create_overpay_payment", fake_create_overpay_payment, raising=False
    )
    monkeypatch.setattr(
        overpay_crud_module, "get_overpay_payment_by_id", fake_getter, raising=False
    )
