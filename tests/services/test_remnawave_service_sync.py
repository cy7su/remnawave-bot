...
def test_deduplicate_prefers_latest_expire_date():
    ...
    telegram_id = 100
    older = _make_panel_user(
        telegram_id, datetime(2025, 1, 1, 0, 0, 0, tzinfo=UTC).isoformat()
    )
    newer = _make_panel_user(
        telegram_id, datetime(2025, 2, 1, 0, 0, 0, tzinfo=UTC).isoformat()
    )
...
def test_deduplicate_ignores_records_without_expire_date():
    ...
    valid = _make_panel_user(
        telegram_id, datetime(2025, 3, 1, 0, 0, 0, tzinfo=UTC).isoformat()
    )

    deduplicated = service._deduplicate_panel_users_by_telegram_id(
        [missing_expire, valid]
    )
...
async def test_get_or_create_user_handles_unique_violation(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.remnawave_service.create_user_no_commit", create_user_mock
    )
    monkeypatch.setattr(
