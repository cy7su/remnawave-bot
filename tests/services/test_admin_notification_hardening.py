...
async def test_send_message_retries_on_flood_control(
    ...
    monkeypatch.setattr(
        "app.services.admin_notification_service.asyncio.sleep", fake_sleep
    )

    flood_error = TelegramRetryAfter(
        method=SimpleNamespace(), message="flood", retry_after=3
    )
    admin_service.bot.send_message = AsyncMock(side_effect=[flood_error, None])

    result = await admin_service._send_message(
        "hello", category=NotificationCategory.INFRASTRUCTURE
    )
...
async def test_send_message_gives_up_after_three_flood_errors(
    ...
    result = await admin_service._send_message(
        "hi", category=NotificationCategory.INFRASTRUCTURE
    )
...
async def test_send_message_caps_retry_after_at_30s(
    ...
    flood = TelegramRetryAfter(
        method=SimpleNamespace(), message="flood", retry_after=120
    )
...
async def test_coalesced_summary_truncates_and_reports_overflow(
    ...
    webhook_service._admin_service.send_webhook_notification = AsyncMock(
...
async def test_coalesced_summary_dedupes_by_name_and_address(
    ...
    await webhook_service._send_coalesced_node_notification(
        "node.connection_lost", payloads
    )

    bullet_lines = [
        line for line in sent_text["value"].split("\n") if line.startswith("•")
    ]
...
async def test_send_message_logs_clamped_retry_after(
    ...
    structlog_mod = __import__(
        "app.services.admin_notification_service", fromlist=["logger"]
    )
    ...
    await admin_service._send_message(
        "hi", category=NotificationCategory.INFRASTRUCTURE
    )
...
async def test_telegram_notifier_processor_redacts_token_in_traceback(
    ...
    bot = MagicMock()
    leaked = "failed to POST https://api.telegram.org/bot8123456789:AAH-Sample_TokenString_With-Chars-aiogram01/sendMessage"
    event_dict = {
        "event": leaked,
