...
class DummySession:
    async def commit(
        self,
    ) -> None:  # pragma: no cover - метод вызывается, но без логики
        return None
...
async def test_create_mulenpay_payment_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        settings, "MULENPAY_MAX_AMOUNT_KOPEKS", 1_000_000, raising=False
    )
    ...
    monkeypatch.setattr(
        settings, "MULENPAY_PAYMENT_MODE", "full_payment", raising=False
    )
...
@pytest.mark.anyio("asyncio")
async def test_create_mulenpay_payment_respects_amount_limits(
...
async def test_process_mulenpay_callback_avoids_duplicate_transactions(
    ...
    async def fake_update_mulenpay_payment_status(
        ...
        payment.mulen_payment_id = kwargs.get(
    ...
    async def fake_auto_purchase_saved_cart_after_topup(
    ...
    monkeypatch.setitem(
        sys.modules, "app.services.subscription_auto_purchase_service", auto_module
    )
