...
class FakeClient:
    ...
    async def post(
        self, url: str, json: dict[str, Any] | None = None, headers: dict | None = None
    ) -> FakeResponse:
...
def service(monkeypatch: pytest.MonkeyPatch) -> tuple[OverpayService, FakeClient]:
    monkeypatch.setattr(
        settings, "OVERPAY_API_URL", "https://api-pay.overpay.io", raising=False
    )
...
async def test_create_payment_uses_explicit_project_id(
    ...
    svc, fake = service
    fake.post_responses = [
        FakeResponse(201, {"id": "op-1", "resultUrl": "https://pay.overpay.io/form"})
    ]
...
async def test_create_payment_defaults_to_settings_project_id(
    ...
    svc, fake = service
    fake.post_responses = [
        FakeResponse(201, {"id": "op-2", "resultUrl": "https://pay"})
    ]
...
async def test_wait_for_redirect_link_stops_on_declined(
    ...
    svc, fake = service
    fake.get_responses = [
        FakeResponse(200, {"orders": [{"status": "declined", "interaction": {}}]})
    ]
...
async def test_wait_for_redirect_link_gives_up_after_attempts(
    ...
    svc, fake = service
    fake.get_responses = [
        FakeResponse(200, {"orders": [{"status": "processing"}]}) for _ in range(4)
    ]
