...
async def test_fulfill_verified_transaction_happy_path_credits_balance_after_user_lock(
    ...
    monkeypatch.setattr(
        apple_iap_module, "create_apple_transaction", create_apple_transaction
    )
...
async def test_fulfill_verified_transaction_insert_race_returns_existing_without_double_credit(
    ...
    existing = SimpleNamespace(
        user_id=1, status="credited", transaction_id="2000000123456789"
    )
    ...
    monkeypatch.setattr(
        apple_iap_module, "lock_user_for_update", AsyncMock(return_value=user)
    )
    monkeypatch.setattr(
        apple_iap_module,
...
async def test_one_time_charge_dispatch_fulfills_account_owner(
    ...
    fulfillment = SimpleNamespace(
        fulfill_verified_transaction=AsyncMock(
            return_value=AppleFulfillmentResult(True, "credited")
        )
    )
    monkeypatch.setattr(
        apple_iap_module,
    ...
    reason = await AppleIAPNotificationService(
        fulfillment_service=fulfillment
    )._handle_one_time_charge(db, _txn_info())
...
async def test_refund_success_debits_balance_and_marks_transaction_refunded(
    ...
    monkeypatch.setattr(
        apple_iap_module, "lock_user_for_pricing", AsyncMock(return_value=user)
    )
    monkeypatch.setattr(
        "app.database.crud.user.subtract_user_balance", subtract_balance
    )
    monkeypatch.setattr(
        apple_iap_module, "mark_apple_transaction_refunded", mark_refunded
    )
...
async def test_consumption_request_requires_recorded_user_consent() -> None:
    reason = await AppleIAPNotificationService()._handle_consumption_request(
        _txn_info()
    )
...
async def test_notification_payload_hash_insert_race_is_treated_as_replay(
    ...
    existing_payload = SimpleNamespace(
        notification_uuid="existing-notification-uuid", status="processed"
    )
    ...
    monkeypatch.setattr(
        apple_iap_module, "AsyncSessionLocal", lambda: _AsyncContextWithValue(db)
    )
    monkeypatch.setattr(
        apple_iap_module, "get_apple_notification_by_uuid", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
        apple_iap_module,
