...
async def test_reads_receiver_meta_key_not_raw_meta():
    ...
    svc = _service()
    await svc._handle_user_expiration(
        None, _user(), _sub(), {"meta": {"expiration": -24}}
    )
...
async def test_user_modified_syncs_used_traffic_from_nested_user_traffic():
    ...
    await svc._handle_user_modified(
        AsyncMock(), _user(), sub, {"userTraffic": {"usedTrafficBytes": 5 * 1024**3}}
    )
...
async def test_user_modified_used_traffic_falls_back_to_flat_key():
    ...
    sub.traffic_used_gb = 0.0
    await svc._handle_user_modified(
        AsyncMock(), _user(), sub, {"usedTrafficBytes": 2 * 1024**3}
    )
