...
def _patches(send_mock: MagicMock):
    ...
    notif_module = SimpleNamespace(
        NotificationType=SimpleNamespace(GUEST_GIFT_RECEIVED="guest_gift_received")
    )
    return (
        patch.dict(
            "sys.modules",
        ...
        patch(
            "app.services.guest_purchase_service.settings.CABINET_URL",
            "https://cab.example",
        ),
    )
...
async def test_email_recipient_and_buyer_both_get_claim_link() -> None:
    ...
    buyer_call = next(
        c for c in send.call_args_list if c.kwargs["to_email"] == "buyer@example.com"
    )
...
async def test_non_gift_purchase_is_a_noop() -> None:
    send = MagicMock(return_value=True)
    purchase = _gift(
        is_gift=False, contact_type="email", contact_value="buyer@example.com"
    )
    mods, cab = _patches(send)
    with mods, cab:
