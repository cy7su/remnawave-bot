...
async def test_submit_subscription_renewal_uses_balance_when_sufficient(monkeypatch):
    ...
    async def fake_finalize(
        ...
        pricing,
        *,
        charge_balance_amount=None,
        description=None,
        payment_method=None,
    ):
        charge = (
            charge_balance_amount
            if charge_balance_amount is not None
    ...
    monkeypatch.setattr(
        miniapp, "_validate_subscription_id", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        miniapp, "_calculate_subscription_renewal_pricing", fake_calculate
    )
    ...
    response = await miniapp.submit_subscription_renewal_endpoint(
        payload, db=types.SimpleNamespace()
    )
...
async def test_submit_subscription_renewal_returns_cryptobot_invoice(monkeypatch):
    ...
    monkeypatch.setattr(
        miniapp, "PaymentService", lambda *args, **kwargs: DummyPaymentService()
    )
    ...
    assert response.payment_payload and response.payment_payload.startswith(
...
async def test_cryptobot_renewal_uses_pricing_snapshot(monkeypatch):
    ...
    async def fail_calculate(*args, **kwargs):
        raise AssertionError(
            "PricingEngine.calculate_renewal_price should not be called when snapshot is present"
        )
    ...
    cryptobot_crud = types.SimpleNamespace(
...
async def test_cryptobot_renewal_accepts_changed_pricing_without_snapshot(monkeypatch):
    ...
    cryptobot_crud = types.SimpleNamespace(
        link_cryptobot_payment_to_transaction=noop_link
    )
...
async def test_cryptobot_webhook_uses_inline_payload_when_db_missing(monkeypatch):
    ...
    async def fail_calculate(*args, **kwargs):
        raise AssertionError(
            "PricingEngine.calculate_renewal_price should not be called"
        )
    ...
    result = await mixin.process_cryptobot_webhook(
        types.SimpleNamespace(), webhook_payload
    )
...
async def test_resolve_yookassa_status_includes_identifiers(monkeypatch):
    ...
    result = await miniapp._resolve_yookassa_payment_status(
        db=None, user=user, query=query
    )
...
async def test_resolve_wata_payment_status_uses_payment_link_lookup(monkeypatch):
    ...
    monkeypatch.setattr(
        miniapp, "get_wata_payment_by_link_id", fake_get_wata_payment_by_link_id
    )
...
async def test_get_payment_methods_exposes_stars_min_amount(monkeypatch):
    ...
    stars_method = next(
        (method for method in response.methods if method.id == "stars"), None
    )
...
async def test_get_payment_methods_includes_wata(monkeypatch):
    ...
    wata_method = next(
        (method for method in response.methods if method.id == "wata"), None
    )
...
async def test_get_payment_methods_marks_mulenpay_iframe(monkeypatch):
    ...
    monkeypatch.setattr(
        settings, "MULENPAY_BASE_URL", "https://checkout.example/api", raising=False
    )
    monkeypatch.setattr(
        settings, "MULENPAY_IFRAME_EXPECTED_ORIGIN", None, raising=False
    )
    ...
    mulenpay_method = next(
        (method for method in response.methods if method.id == "mulenpay"), None
    )
    ...
    assert (
        str(mulenpay_method.iframe_config.expected_origin) == "https://checkout.example"
    )
