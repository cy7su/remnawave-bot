...
def locales():
    return {
        lang: json.loads((LOCALE_DIR / f"{lang}.json").read_text(encoding="utf-8"))
        for lang in LANGS
    }
...
def test_all_locales_have_identical_keys(locales):
    ...
    for lang in LANGS:
        ...
        assert (
            not missing
        ), f"{lang}.json is missing {len(missing)} keys present in ru: {missing[:15]}"
...
def test_placeholders_consistent_across_locales(locales):
    ...
    assert (
        not problems
    ), "Placeholder mismatches (would break .format()):\n" + "\n".join(problems[:25])
...
def test_t_calls_without_default_exist_in_ru(locales):
    ...
    assert (
        not offenders
    ), "texts.t(key) with no fallback for a key missing from ru.json:\n" + "\n".join(
        sorted(offenders)[:25]
    )
