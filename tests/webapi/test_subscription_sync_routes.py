...
async def test_users_subscription_trial_calls_remnawave_sync(
    ...
    monkeypatch.setattr(
        users, "_get_user_by_id_or_telegram_id", AsyncMock(return_value=fake_user)
    )
    monkeypatch.setattr(
        users, "get_subscription_by_user_id", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
        users, "create_trial_subscription", AsyncMock(return_value=created_subscription)
    )
    ...
    payload = UserSubscriptionCreateRequest(
        is_trial=True, duration_days=7, replace_existing=False
    )
    result = await users.create_user_subscription(
        user_id=1, payload=payload, _=None, db=SimpleNamespace()
    )
...
async def test_users_subscription_paid_calls_remnawave_sync(
    ...
    monkeypatch.setattr(
        users, "create_paid_subscription", AsyncMock(return_value=created_subscription)
    )
    monkeypatch.setattr(users, "SubscriptionService", lambda: service_instance)
    monkeypatch.setattr(users, "get_user_by_id", AsyncMock(return_value=fake_user))
    monkeypatch.setattr(users, "_serialize_user", lambda user: {"id": user.id})
...
async def test_subscriptions_extend_calls_remnawave_sync(
    ...
    monkeypatch.setattr(
        subscriptions, "extend_subscription", AsyncMock(return_value=subscription)
    )
    monkeypatch.setattr(subscriptions, "SubscriptionService", lambda: service_instance)
    monkeypatch.setattr(
        subscriptions, "_serialize_subscription", lambda sub: {"id": sub.id}
    )
...
async def test_subscriptions_extend_rolls_back_when_sync_fails(
    ...
    monkeypatch.setattr(
        subscriptions, "_get_subscription", AsyncMock(return_value=subscription)
    )
    monkeypatch.setattr(
        subscriptions, "extend_subscription", AsyncMock(return_value=subscription)
    )
...
async def test_users_patch_subscription_delegates_to_post(
    ...
    result = await users.patch_user_subscription(
        user_id=42, payload=payload, _=None, db=SimpleNamespace()
    )
...
def test_users_patch_subscription_route_returns_201() -> None:
    ...
    patch_route = next(
        ...
        if getattr(route, "path", None) == "/{user_id}/subscription"
...
async def test_users_subscription_replace_existing_restores_on_sync_failure(
    ...
    monkeypatch.setattr(
        users, "replace_subscription", AsyncMock(return_value=replaced_subscription)
    )
    ...
    payload = UserSubscriptionCreateRequest(
        is_trial=True, duration_days=7, replace_existing=True
    )

    with pytest.raises(HTTPException) as error:
        await users.create_user_subscription(
            user_id=7, payload=payload, _=None, db=SimpleNamespace()
        )
