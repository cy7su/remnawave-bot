...
async def test_spin_rechecks_daily_limit_under_lock() -> None:
    ...
    config = SimpleNamespace(
        id=1, daily_spin_limit=5, spin_cost_stars=10, spin_cost_stars_enabled=True
    )
    ...
    with ExitStack() as s:
        s.enter_context(
            patch.object(
                ...
                "check_availability",
                AsyncMock(
                    return_value=SpinAvailability(can_spin=True, can_pay_stars=True)
                ),
            )
        )
        s.enter_context(
...
async def test_stars_wheel_spin_idempotent_on_redelivery() -> None:
    ...
    with ExitStack() as s:
        ...
        s.enter_context(
            patch("app.database.crud.wheel.get_or_create_wheel_config", config_loader)
        )
